#!/usr/bin/env bash
# airdrop — Airdrop reference tool. Use when working with airdrop in blockchain contexts.
...
show_help() {
    cat << 'HELPEOF'
airdrop v$VERSION — Airdrop Reference Tool

Usage: airdrop <command>
...
## What is Airdrop?
Airdrop (airdrop) is a specialized tool/concept in the blockchain domain.
It provides essential capabilities for professionals working with airdrop.

## Key Concepts
- Core airdrop principles and fundamentals
- How airdrop fits into the broader blockchain ecosystem  
- Essential terminology every practitioner should know

## Why Airdrop Matters
Understanding airdrop is critical for:
...
## Getting Started
1. Understand the basic airdrop concepts
...
cmd_formulas() {
    cat << 'EOF'
# Airdrop — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Airdrop — Instruments & Tools Overview

## Primary Instruments
- Core tools used in airdrop operations
...
cmd_glossary() {
    cat << 'EOF'
# Airdrop — Key Terms & Definitions

## Core Terminology
- **Airdrop**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "airdrop v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: airdrop help"; exit 1 ;;
esac
