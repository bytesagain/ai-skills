#!/usr/bin/env bash
# alarm -- Track alarm personal activities and daily routines. Use when managing health goals, organizing tasks, or building productive habits.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ALARM_DIR:-$HOME/.alarm}"
...
show_help() {
    cat << EOF
alarm v$VERSION -- Track alarm personal activities and daily routines. Use when managing health goals, organizing tasks, or building productive habits.

Usage: alarm <command> [args]
...
cmd_add() {
    local value="${1:?Usage: alarm add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: alarm search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: alarm remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "alarm v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: alarm help"; exit 1 ;;
esac
