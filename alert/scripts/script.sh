#!/usr/bin/env bash
# alert -- Send and schedule alerts via multiple channels with flexible triggers. Use when monitoring servers, tracking thresholds, or setting reminders.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ALERT_DIR:-$HOME/.alert}"
...
show_help() {
    cat << EOF
alert v$VERSION -- Send and schedule alerts via multiple channels with flexible triggers. Use when monitoring servers, tracking thresholds, or setting reminders.
...
cmd_add() {
    local value="${1:?Usage: alert add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: alert remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "alert v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: alert help"; exit 1 ;;
esac
