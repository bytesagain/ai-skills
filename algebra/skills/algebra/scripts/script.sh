#!/usr/bin/env bash
# algebra -- Solve equations, simplify expressions, and factor polynomials. Use when doing algebra homework, checking proofs, or plotting functions.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ALGEBRA_DIR:-$HOME/.algebra}"
...
show_help() {
    cat << EOF
algebra v$VERSION -- Solve equations, simplify expressions, and factor polynomials. Use when doing algebra homework, checking proofs, or plotting functions.

Usage: algebra <command> [args]
...
cmd_add() {
    local value="${1:?Usage: algebra add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: algebra remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "algebra v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: algebra help"; exit 1 ;;
esac
