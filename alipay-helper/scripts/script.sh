#!/usr/bin/env bash
# alipay-helper - Financial tracking and analysis tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${ALIPAY_HELPER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/alipay-helper}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
alipay-helper v$VERSION

Financial tracking and analysis tool

Usage: alipay-helper <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "alipay-helper v$VERSION" ;;
