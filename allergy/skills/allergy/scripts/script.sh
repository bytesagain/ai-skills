#!/usr/bin/env bash
# allergy -- Log allergens, track reactions, and chart symptom patterns over time. Use when identifying triggers, preparing reports, or setting reminders.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ALLERGY_DIR:-$HOME/.allergy}"
...
show_help() {
    cat << EOF
allergy v$VERSION -- Log allergens, track reactions, and chart symptom patterns over time. Use when identifying triggers, preparing reports, or setting reminders.

Usage: allergy <command> [args]
...
cmd_add() {
    local value="${1:?Usage: allergy add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: allergy search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: allergy remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "allergy v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: allergy help"; exit 1 ;;
esac
