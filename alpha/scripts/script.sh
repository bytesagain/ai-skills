#!/usr/bin/env bash
# alpha — Alpha reference tool. Use when working with alpha in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
alpha v$VERSION — Alpha Reference Tool
...
## What is Alpha?
Alpha (alpha) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with alpha.

## Key Concepts
- Core alpha principles and fundamentals
- How alpha fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Alpha Matters
Understanding alpha is critical for:
...
## Getting Started
1. Understand the basic alpha concepts
...
cmd_formulas() {
    cat << 'EOF'
# Alpha — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Alpha — Instruments & Tools Overview

## Primary Instruments
- Core tools used in alpha operations
...
cmd_glossary() {
    cat << 'EOF'
# Alpha — Key Terms & Definitions

## Core Terminology
- **Alpha**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "alpha v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: alpha help"; exit 1 ;;
esac
