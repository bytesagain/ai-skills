#!/usr/bin/env bash
# antlr4 — Antlr4 reference tool. Use when working with antlr4 in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
antlr4 v$VERSION — Antlr4 Reference Tool

Usage: antlr4 <command>
...
## What is Antlr4?
Antlr4 (antlr4) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with antlr4.

## Key Concepts
- Core antlr4 principles and fundamentals
- How antlr4 fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Antlr4 Matters
Understanding antlr4 is critical for:
...
## Getting Started
1. Understand the basic antlr4 concepts
...
## Installation
1. Download or clone the antlr4 package
...
cmd_patterns() {
    cat << 'EOF'
# Antlr4 — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for antlr4
...
cmd_performance() {
    cat << 'EOF'
# Antlr4 — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Antlr4 — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Antlr4 — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "antlr4 v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: antlr4 help"; exit 1 ;;
esac
