#!/usr/bin/env bash
# api-router — Api Router reference tool. Use when working with api router in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
api-router v$VERSION — Api Router Reference Tool

Usage: api-router <command>
...
## What is Api Router?
Api Router (api-router) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with api router.

## Key Concepts
- Core api router principles and fundamentals
- How api router fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Api Router Matters
Understanding api router is critical for:
...
## Getting Started
1. Understand the basic api router concepts
...
## Installation
1. Download or clone the api router package
...
cmd_patterns() {
    cat << 'EOF'
# Api Router — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for api router
...
cmd_security() {
    cat << 'EOF'
# Api Router — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Api Router — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "api-router v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: api-router help"; exit 1 ;;
esac
