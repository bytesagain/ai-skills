#!/usr/bin/env bash
# appsmith — Appsmith reference tool. Use when working with appsmith in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
appsmith v$VERSION — Appsmith Reference Tool

Usage: appsmith <command>
...
## What is Appsmith?
Appsmith (appsmith) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with appsmith.

## Key Concepts
- Core appsmith principles and fundamentals
- How appsmith fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Appsmith Matters
Understanding appsmith is critical for:
...
## Getting Started
1. Understand the basic appsmith concepts
...
## Installation
1. Download or clone the appsmith package
...
cmd_patterns() {
    cat << 'EOF'
# Appsmith — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for appsmith
...
cmd_performance() {
    cat << 'EOF'
# Appsmith — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Appsmith — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Appsmith — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "appsmith v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: appsmith help"; exit 1 ;;
esac
