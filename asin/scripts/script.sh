#!/usr/bin/env bash
# asin -- Run asin operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ASIN_DIR:-$HOME/.asin}"
...
show_help() {
    cat << EOF
asin v$VERSION -- Run asin operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: asin <command> [args]
...
cmd_add() {
    local value="${1:?Usage: asin add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: asin search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: asin remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "asin v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: asin help"; exit 1 ;;
esac
