#!/usr/bin/env bash
# auth -- Manage authentication tokens and JWTs using bash and Python. Use when generating, verifying, decoding, or revoking auth tokens locally.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${AUTH_DIR:-$HOME/.auth}"
...
show_help() {
    cat << EOF
auth v$VERSION -- Manage authentication tokens and JWTs using bash and Python. Use when generating, verifying, decoding, or revoking auth tokens locally.
...
cmd_add() {
    local value="${1:?Usage: auth add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: auth search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: auth remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "auth v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: auth help"; exit 1 ;;
esac
