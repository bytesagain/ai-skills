#!/usr/bin/env bash
# backoff -- Integrate backoff operations. Use when you need to configure backoff endpoints, manage API connections, or handle service webhooks.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${BACKOFF_DIR:-$HOME/.backoff}"
...
show_help() {
    cat << EOF
backoff v$VERSION -- Integrate backoff operations. Use when you need to configure backoff endpoints, manage API connections, or handle service webhooks.

Usage: backoff <command> [args]
...
cmd_add() {
    local value="${1:?Usage: backoff add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: backoff search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: backoff remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "backoff v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: backoff help"; exit 1 ;;
esac
