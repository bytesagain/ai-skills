#!/usr/bin/env bash
# badge -- Generate badge UI elements and design assets. Use when building interfaces, creating visual components, or styling web pages.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${BADGE_DIR:-$HOME/.badge}"
...
show_help() {
    cat << EOF
badge v$VERSION -- Generate badge UI elements and design assets. Use when building interfaces, creating visual components, or styling web pages.

Usage: badge <command> [args]
...
cmd_add() {
    local value="${1:?Usage: badge add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: badge search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: badge remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "badge v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: badge help"; exit 1 ;;
esac
