#!/usr/bin/env bash
# base — Base reference tool. Use when working with base in blockchain contexts.
...
show_help() {
    cat << 'HELPEOF'
base v$VERSION — Base Reference Tool
...
## What is Base?
Base (base) is a specialized tool/concept in the blockchain domain.
It provides essential capabilities for professionals working with base.

## Key Concepts
- Core base principles and fundamentals
- How base fits into the broader blockchain ecosystem  
- Essential terminology every practitioner should know

## Why Base Matters
Understanding base is critical for:
...
## Getting Started
1. Understand the basic base concepts
...
cmd_formulas() {
    cat << 'EOF'
# Base — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Base — Instruments & Tools Overview

## Primary Instruments
- Core tools used in base operations
...
cmd_glossary() {
    cat << 'EOF'
# Base — Key Terms & Definitions

## Core Terminology
- **Base**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "base v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: base help"; exit 1 ;;
esac
