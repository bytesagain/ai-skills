#!/usr/bin/env bash
# base64 — Base64 reference tool. Use when working with base64 in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
base64 v$VERSION — Base64 Reference Tool

Usage: base64 <command>
...
## What is Base64?
Base64 (base64) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with base64.

## Key Concepts
- Core base64 principles and fundamentals
- How base64 fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Base64 Matters
Understanding base64 is critical for:
...
## Getting Started
1. Understand the basic base64 concepts
...
## Installation
1. Download or clone the base64 package
...
cmd_patterns() {
    cat << 'EOF'
# Base64 — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for base64
...
cmd_migration() {
    cat << 'EOF'
# Base64 — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "base64 v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: base64 help"; exit 1 ;;
esac
