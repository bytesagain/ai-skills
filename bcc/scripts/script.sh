#!/usr/bin/env bash
# bcc — Bcc reference tool. Use when working with bcc in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
bcc v$VERSION — Bcc Reference Tool
...
## What is Bcc?
Bcc (bcc) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with bcc.

## Key Concepts
- Core bcc principles and fundamentals
- How bcc fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Bcc Matters
Understanding bcc is critical for:
...
## Getting Started
1. Understand the basic bcc concepts
...
cmd_patterns() {
    cat << 'EOF'
# Bcc — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for bcc
...
case "$CMD" in
    ...
    version|--version|-v) echo "bcc v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: bcc help"; exit 1 ;;
esac
