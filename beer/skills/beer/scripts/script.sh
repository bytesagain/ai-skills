#!/usr/bin/env bash
# beer -- Catalog beers, log tasting notes, and track brewing recipes locally. Use when rating craft beers, recording homebrew batches, or browsing styles.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${BEER_DIR:-$HOME/.beer}"
...
show_help() {
    cat << EOF
beer v$VERSION -- Catalog beers, log tasting notes, and track brewing recipes locally. Use when rating craft beers, recording homebrew batches, or browsing styles.
...
cmd_add() {
    local value="${1:?Usage: beer add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: beer remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "beer v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: beer help"; exit 1 ;;
esac
