#!/usr/bin/env bash
# benchmark — Benchmark reference tool. Use when working with benchmark in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
benchmark v$VERSION — Benchmark Reference Tool
...
## What is Benchmark?
Benchmark (benchmark) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with benchmark.

## Key Concepts
- Core benchmark principles and fundamentals
- How benchmark fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Benchmark Matters
Understanding benchmark is critical for:
...
## Getting Started
1. Understand the basic benchmark concepts
...
## Installation
1. Download or clone the benchmark package
...
cmd_patterns() {
    cat << 'EOF'
# Benchmark — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for benchmark
...
cmd_security() {
    cat << 'EOF'
# Benchmark — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Benchmark — Migration & Upgrade Guide
