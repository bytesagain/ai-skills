#!/usr/bin/env bash
# benchmark -- Run performance benchmarks and stress tests using Python profiling tools. Use when you need to measure, compare, or analyze system and application performance.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${BENCHMARK_DIR:-$HOME/.benchmark}"
...
show_help() {
    cat << EOF
benchmark v$VERSION -- Run performance benchmarks and stress tests using Python profiling tools. Use when you need to measure, compare, or analyze system and application performance.
...
cmd_add() {
    local value="${1:?Usage: benchmark add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: benchmark search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_compare() {
    local a="${1:?Usage: benchmark compare <a> <b>}"
...
cmd_remove() {
    local num="${1:?Usage: benchmark remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="benchmark-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "benchmark v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: benchmark help"; exit 1 ;;
esac
