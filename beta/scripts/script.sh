#!/usr/bin/env bash
# beta -- Calculate beta financial metrics and business data. Use when tracking expenses, analyzing investments, or generating reports.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${BETA_DIR:-$HOME/.beta}"
...
show_help() {
    cat << EOF
beta v$VERSION -- Calculate beta financial metrics and business data. Use when tracking expenses, analyzing investments, or generating reports.

Usage: beta <command> [args]
...
cmd_add() {
    local value="${1:?Usage: beta add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: beta search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: beta remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: beta help"; exit 1 ;;
esac
