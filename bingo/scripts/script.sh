#!/usr/bin/env bash
# bingo -- Generate and play bingo cards with number calling and verification. Use when running bingo games.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${BINGO_DIR:-$HOME/.bingo}"
...
show_help() {
    cat << EOF
bingo v$VERSION -- Generate and play bingo cards with number calling and verification. Use when running bingo games.

Usage: bingo <command> [args]
...
cmd_add() {
    local value="${1:?Usage: bingo add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: bingo remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "bingo v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: bingo help"; exit 1 ;;
esac
