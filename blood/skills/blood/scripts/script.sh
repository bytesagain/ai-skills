#!/usr/bin/env bash
# blood -- Log blood pressure, glucose, and health metrics with trend charts. Use when recording vitals, reviewing health trends, or exporting medical logs.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${BLOOD_DIR:-$HOME/.blood}"
...
show_help() {
    cat << EOF
blood v$VERSION -- Log blood pressure, glucose, and health metrics with trend charts. Use when recording vitals, reviewing health trends, or exporting medical logs.

Usage: blood <command> [args]
...
cmd_add() {
    local value="${1:?Usage: blood add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: blood search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: blood remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "blood v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: blood help"; exit 1 ;;
esac
