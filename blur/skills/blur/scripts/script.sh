#!/usr/bin/env bash
# blur -- Apply image blur effects and privacy masks using Python PIL processing. Use when you need to blur, redact faces, or mask sensitive regions in images.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${BLUR_DIR:-$HOME/.blur}"
...
show_help() {
    cat << EOF
blur v$VERSION -- Apply image blur effects and privacy masks using Python PIL processing. Use when you need to blur, redact faces, or mask sensitive regions in images.

Usage: blur <command> [args]
...
cmd_add() {
    local value="${1:?Usage: blur add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: blur search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: blur remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "blur v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: blur help"; exit 1 ;;
esac
