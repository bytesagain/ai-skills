#!/usr/bin/env bash
# bmi -- Calculate BMI, log weight entries, and chart body composition trends. Use when tracking fitness progress, setting weight goals, or reviewing data.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${BMI_DIR:-$HOME/.bmi}"
...
show_help() {
    cat << EOF
bmi v$VERSION -- Calculate BMI, log weight entries, and chart body composition trends. Use when tracking fitness progress, setting weight goals, or reviewing data.

Usage: bmi <command> [args]
...
cmd_add() {
    local value="${1:?Usage: bmi add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: bmi search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: bmi remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "bmi v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: bmi help"; exit 1 ;;
esac
