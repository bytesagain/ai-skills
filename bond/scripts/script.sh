...
show_help() {
    cat << EOF
bond v$VERSION -- bond tool

Usage: bond <command> [args]
...
cmd_add() {
    local value="${1:?Usage: bond add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: bond search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: bond remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "bond v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: bond help"; exit 1 ;;
esac
