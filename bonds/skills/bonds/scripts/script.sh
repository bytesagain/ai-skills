#!/usr/bin/env bash
# bonds -- Record bond holdings, analyze yields, and track maturity dates. Use when managing fixed-income portfolios, comparing yields, or exporting data.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${BONDS_DIR:-$HOME/.bonds}"
...
show_help() {
    cat << EOF
bonds v$VERSION -- Record bond holdings, analyze yields, and track maturity dates. Use when managing fixed-income portfolios, comparing yields, or exporting data.

Usage: bonds <command> [args]
...
cmd_add() {
    local value="${1:?Usage: bonds add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: bonds search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: bonds remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "bonds v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: bonds help"; exit 1 ;;
esac
