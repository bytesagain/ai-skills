#!/usr/bin/env bash
# book-recommend - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${BOOK_RECOMMEND_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/book-recommend}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
book-recommend v$VERSION

Multi-purpose utility tool

Usage: book-recommend <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "book-recommend v$VERSION" ;;
