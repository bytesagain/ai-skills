#!/usr/bin/env bash
# border -- Generate CSS border styles including radius, shadow, and animations using templates. Use when you need to create, preview,
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${BORDER_DIR:-$HOME/.border}"
...
show_help() {
    cat << EOF
border v$VERSION -- Generate CSS border styles including radius, shadow, and animations using templates. Use when you need to create, preview,

Usage: border <command> [args]
...
cmd_add() {
    local value="${1:?Usage: border add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: border search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: border remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "border v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: border help"; exit 1 ;;
esac
