...
VERSION="1.0.0"
DATA_DIR="${BREADCRUMB_DIR:-$HOME/.breadcrumb}"
...
show_help() {
    cat << EOF
breadcrumb v$VERSION -- breadcrumb tool

Usage: breadcrumb <command> [args]
...
cmd_add() {
    local value="${1:?Usage: breadcrumb add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: breadcrumb search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: breadcrumb remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="breadcrumb-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "breadcrumb v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: breadcrumb help"; exit 1 ;;
esac
