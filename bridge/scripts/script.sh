#!/usr/bin/env bash
# bridge — Bridge reference tool. Use when working with bridge in blockchain contexts.
...
show_help() {
    cat << 'HELPEOF'
bridge v$VERSION — Bridge Reference Tool
...
## What is Bridge?
Bridge (bridge) is a specialized tool/concept in the blockchain domain.
It provides essential capabilities for professionals working with bridge.

## Key Concepts
- Core bridge principles and fundamentals
- How bridge fits into the broader blockchain ecosystem  
- Essential terminology every practitioner should know

## Why Bridge Matters
Understanding bridge is critical for:
...
## Getting Started
1. Understand the basic bridge concepts
...
cmd_formulas() {
    cat << 'EOF'
# Bridge — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Bridge — Instruments & Tools Overview

## Primary Instruments
- Core tools used in bridge operations
...
cmd_glossary() {
    cat << 'EOF'
# Bridge — Key Terms & Definitions

## Core Terminology
- **Bridge**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "bridge v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: bridge help"; exit 1 ;;
esac
