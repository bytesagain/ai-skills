#!/usr/bin/env bash
# brook — Brook reference tool. Use when working with brook in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
brook v$VERSION — Brook Reference Tool
...
## What is Brook?
Brook (brook) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with brook.

## Key Concepts
- Core brook principles and fundamentals
- How brook fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Brook Matters
Understanding brook is critical for:
...
## Getting Started
1. Understand the basic brook concepts
...
## Installation
1. Download or clone the brook package
...
cmd_patterns() {
    cat << 'EOF'
# Brook — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for brook
...
cmd_performance() {
    cat << 'EOF'
# Brook — Performance Optimization
...
case "$CMD" in
    ...
    version|--version|-v) echo "brook v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: brook help"; exit 1 ;;
esac
