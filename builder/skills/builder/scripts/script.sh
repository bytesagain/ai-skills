#!/usr/bin/env bash
# builder -- builder tool for creating outputs
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${BUILDER_DIR:-$HOME/.builder}"
...
show_help() {
    cat << EOF
builder v$VERSION -- builder tool for creating outputs

Usage: builder <command> [args]
...
cmd_add() {
    local value="${1:?Usage: builder add <entry>}"
    shift || true
...
cmd_generate() {
    local name="${1:?Usage: builder generate <name>}"
    local tmpl="${2:-default}"
    local out="$DATA_DIR/${name}.txt"
    echo "# Generated by builder" > "$out"
...
cmd_remove() {
    local num="${1:?Usage: builder remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
