#!/usr/bin/env bash
# bullet-journal — Bullet Journal reference tool. Use when working with bullet journal in life contexts.
...
show_help() {
    cat << 'HELPEOF'
bullet-journal v$VERSION — Bullet Journal Reference Tool

Usage: bullet-journal <command>
...
Bullet Journal (bullet-journal) is a specialized tool/concept in the life domain.
It provides essential capabilities for professionals working with bullet journal.

## Key Concepts
- Core bullet journal principles and fundamentals
- How bullet journal fits into the broader life ecosystem  
- Essential terminology every practitioner should know

## Why Bullet Journal Matters
Understanding bullet journal is critical for:
...
## Getting Started
1. Understand the basic bullet journal concepts
...
cmd_guide() {
    cat << 'EOF'
# Bullet Journal — Step-by-Step Guide

## Overview
This guide walks you through the essential bullet journal workflows.
...
cmd_tips() {
    cat << 'EOF'
# Bullet Journal — Pro Tips & Tricks
...
cmd_planning() {
    cat << 'EOF'
# Bullet Journal — Planning & Preparation
...
cmd_resources() {
    cat << 'EOF'
# Bullet Journal — Recommended Resources
...
cmd_mistakes() {
    cat << 'EOF'
# Bullet Journal — Common Mistakes to Avoid
...
cmd_examples() {
    cat << 'EOF'
# Bullet Journal — Real-World Examples

## Example 1: Basic Setup
A typical bullet journal setup for a small team:
- Standard configuration with defaults
- Basic monitoring enabled
- Manual backup schedule

## Example 2: Production Deployment
An enterprise bullet journal deployment:
...
## General
**Q: What is Bullet Journal?**
A: Bullet Journal is a reference tool for bullet journal in the life domain.

**Q: Who should use this?**
A: Anyone working with bullet journal who needs quick reference material.
...
case "$CMD" in
    ...
    version|--version|-v) echo "bullet-journal v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: bullet-journal help"; exit 1 ;;
esac
