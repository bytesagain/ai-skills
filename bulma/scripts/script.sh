#!/usr/bin/env bash
# bulma — Bulma reference tool. Use when working with bulma in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
bulma v$VERSION — Bulma Reference Tool
...
## What is Bulma?
Bulma (bulma) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with bulma.

## Key Concepts
- Core bulma principles and fundamentals
- How bulma fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Bulma Matters
Understanding bulma is critical for:
...
## Getting Started
1. Understand the basic bulma concepts
...
## Installation
1. Download or clone the bulma package
...
cmd_patterns() {
    cat << 'EOF'
# Bulma — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for bulma
...
cmd_performance() {
    cat << 'EOF'
# Bulma — Performance Optimization
...
case "$CMD" in
    ...
    version|--version|-v) echo "bulma v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: bulma help"; exit 1 ;;
esac
