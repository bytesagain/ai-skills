#!/usr/bin/env bash
# burndown — Burndown reference tool. Use when working with burndown in life contexts.
...
show_help() {
    cat << 'HELPEOF'
burndown v$VERSION — Burndown Reference Tool

Usage: burndown <command>
...
## What is Burndown?
Burndown (burndown) is a specialized tool/concept in the life domain.
It provides essential capabilities for professionals working with burndown.

## Key Concepts
- Core burndown principles and fundamentals
- How burndown fits into the broader life ecosystem  
- Essential terminology every practitioner should know

## Why Burndown Matters
Understanding burndown is critical for:
...
## Getting Started
1. Understand the basic burndown concepts
...
cmd_guide() {
    cat << 'EOF'
# Burndown — Step-by-Step Guide

## Overview
This guide walks you through the essential burndown workflows.
...
cmd_resources() {
    cat << 'EOF'
# Burndown — Recommended Resources
...
cmd_mistakes() {
    cat << 'EOF'
# Burndown — Common Mistakes to Avoid
...
cmd_examples() {
    cat << 'EOF'
# Burndown — Real-World Examples

## Example 1: Basic Setup
A typical burndown setup for a small team:
- Standard configuration with defaults
- Basic monitoring enabled
- Manual backup schedule

## Example 2: Production Deployment
An enterprise burndown deployment:
...
cmd_faq() {
    cat << 'EOF'
# Burndown — Frequently Asked Questions

## General
**Q: What is Burndown?**
A: Burndown is a reference tool for burndown in the life domain.

**Q: Who should use this?**
A: Anyone working with burndown who needs quick reference material.
...
case "$CMD" in
    ...
    version|--version|-v) echo "burndown v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: burndown help"; exit 1 ;;
esac
