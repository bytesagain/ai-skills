#!/usr/bin/env bash
# busbar -- Busbar current capacity calculator
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${BUSBAR_DIR:-$HOME/.busbar}"
...
show_help() {
    cat << EOF
busbar v$VERSION -- Busbar current capacity calculator

Usage: busbar <command> [args]
...
cmd_add() {
    local value="${1:?Usage: busbar add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: busbar search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: busbar remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "busbar v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: busbar help"; exit 1 ;;
esac
