#!/usr/bin/env bash
# cache -- Manage a local key-value cache store using bash and Python. Use when caching API responses, session data, or computed results with TTL support.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CACHE_DIR:-$HOME/.cache}"
...
show_help() {
    cat << EOF
cache v$VERSION -- Manage a local key-value cache store using bash and Python. Use when caching API responses, session data, or computed results with TTL support.
...
cmd_add() {
    local value="${1:?Usage: cache add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: cache search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: cache remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "cache v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: cache help"; exit 1 ;;
esac
