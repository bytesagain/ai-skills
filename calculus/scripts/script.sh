#!/usr/bin/env bash
# calculus -- Compute derivatives, integrals, limits, and series step by step. Use when solving calculus problems, plotting functions, or verifying integrals.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CALCULUS_DIR:-$HOME/.calculus}"
...
show_help() {
    cat << EOF
calculus v$VERSION -- Compute derivatives, integrals, limits, and series step by step. Use when solving calculus problems, plotting functions, or verifying integrals.

Usage: calculus <command> [args]
...
cmd_add() {
    local value="${1:?Usage: calculus add <entry>}"
    shift || true
...
cmd_calculate() {
    local expr="${*:?Usage: calculus calculate <expression>}"
...
cmd_remove() {
    local num="${1:?Usage: calculus remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
