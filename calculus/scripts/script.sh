#!/usr/bin/env bash
# calculus — Calculus reference tool. Use when working with calculus in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
calculus v$VERSION — Calculus Reference Tool

Usage: calculus <command>
...
## What is Calculus?
Calculus (calculus) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with calculus.

## Key Concepts
- Core calculus principles and fundamentals
- How calculus fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Calculus Matters
Understanding calculus is critical for:
...
## Getting Started
1. Understand the basic calculus concepts
...
## Installation
1. Download or clone the calculus package
...
cmd_patterns() {
    cat << 'EOF'
# Calculus — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for calculus
...
cmd_performance() {
    cat << 'EOF'
# Calculus — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Calculus — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Calculus — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "calculus v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: calculus help"; exit 1 ;;
esac
