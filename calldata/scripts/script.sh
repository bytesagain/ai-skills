#!/usr/bin/env bash
# calldata -- Analyze calldata operations. Use when you need to understand calldata mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CALLDATA_DIR:-$HOME/.calldata}"
...
show_help() {
    cat << EOF
calldata v$VERSION -- Analyze calldata operations. Use when you need to understand calldata mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: calldata <command> [args]
...
cmd_add() {
    local value="${1:?Usage: calldata add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: calldata search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: calldata remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "calldata v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: calldata help"; exit 1 ;;
esac
