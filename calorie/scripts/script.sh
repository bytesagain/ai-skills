#!/usr/bin/env bash
# calorie -- Track calorie personal activities and daily routines. Use when managing health goals, organizing tasks, or building habits.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CALORIE_DIR:-$HOME/.calorie}"
...
show_help() {
    cat << EOF
calorie v$VERSION -- Track calorie personal activities and daily routines. Use when managing health goals, organizing tasks, or building habits.

Usage: calorie <command> [args]
...
cmd_add() {
    local value="${1:?Usage: calorie add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: calorie search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: calorie remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "calorie v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: calorie help"; exit 1 ;;
esac
