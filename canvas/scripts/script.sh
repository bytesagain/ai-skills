#!/usr/bin/env bash
# canvas -- Generate HTML5 Canvas drawing code snippets using bash and Python. Use when creating shapes, gradients, text, or animated canvas scenes programmatically.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CANVAS_DIR:-$HOME/.canvas}"
...
show_help() {
    cat << EOF
canvas v$VERSION -- Generate HTML5 Canvas drawing code snippets using bash and Python. Use when creating shapes, gradients, text, or animated canvas scenes programmatically.
...
cmd_add() {
    local value="${1:?Usage: canvas add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: canvas search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: canvas remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "canvas v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: canvas help"; exit 1 ;;
esac
