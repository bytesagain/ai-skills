#!/usr/bin/env bash
# capistrano — Capistrano reference tool. Use when working with capistrano in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
capistrano v$VERSION — Capistrano Reference Tool

Usage: capistrano <command>
...
## What is Capistrano?
Capistrano (capistrano) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with capistrano.

## Key Concepts
- Core capistrano principles and fundamentals
- How capistrano fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Capistrano Matters
Understanding capistrano is critical for:
...
## Getting Started
1. Understand the basic capistrano concepts
...
## Installation
1. Download or clone the capistrano package
...
cmd_patterns() {
    cat << 'EOF'
# Capistrano — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for capistrano
...
cmd_performance() {
    cat << 'EOF'
# Capistrano — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Capistrano — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Capistrano — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "capistrano v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: capistrano help"; exit 1 ;;
esac
