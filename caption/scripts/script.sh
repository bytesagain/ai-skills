#!/usr/bin/env bash
# caption — Caption reference tool. Use when working with caption in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
caption v$VERSION — Caption Reference Tool

Usage: caption <command>
...
## What is Caption?
Caption (caption) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with caption.

## Key Concepts
- Core caption principles and fundamentals
- How caption fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Caption Matters
Understanding caption is critical for:
...
## Getting Started
1. Understand the basic caption concepts
...
## Installation
1. Download or clone the caption package
...
cmd_patterns() {
    cat << 'EOF'
# Caption — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for caption
...
cmd_migration() {
    cat << 'EOF'
# Caption — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "caption v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: caption help"; exit 1 ;;
esac
