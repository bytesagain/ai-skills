#!/usr/bin/env bash
# ccpa -- Run ccpa operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CCPA_DIR:-$HOME/.ccpa}"
...
show_help() {
    cat << EOF
ccpa v$VERSION -- Run ccpa operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: ccpa <command> [args]
...
cmd_add() {
    local value="${1:?Usage: ccpa add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: ccpa search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: ccpa remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "ccpa v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: ccpa help"; exit 1 ;;
esac
