#!/usr/bin/env bash
# chainlink -- Analyze chainlink operations. Use when you need to understand chainlink mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CHAINLINK_DIR:-$HOME/.chainlink}"
...
show_help() {
    cat << EOF
chainlink v$VERSION -- Analyze chainlink operations. Use when you need to understand chainlink mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: chainlink <command> [args]
...
cmd_add() {
    local value="${1:?Usage: chainlink add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: chainlink search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: chainlink remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="chainlink-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "chainlink v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: chainlink help"; exit 1 ;;
esac
