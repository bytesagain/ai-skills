#!/usr/bin/env bash
# changelog-writer - Data processing and analysis toolkit
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${CHANGELOG_WRITER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/changelog-writer}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
changelog-writer v$VERSION

Data processing and analysis toolkit

Usage: changelog-writer <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "changelog-writer v$VERSION" ;;
