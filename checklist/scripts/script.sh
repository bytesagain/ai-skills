#!/usr/bin/env bash
# checklist -- Track checklist personal activities and daily routines. Use when managing health goals, organizing tasks, or building productive habits.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CHECKLIST_DIR:-$HOME/.checklist}"
...
show_help() {
    cat << EOF
checklist v$VERSION -- Track checklist personal activities and daily routines. Use when managing health goals, organizing tasks, or building productive habits.

Usage: checklist <command> [args]
...
cmd_add() {
    local value="${1:?Usage: checklist add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: checklist search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_check() {
    local target="${1:?Usage: checklist check <file-or-value>}"
    if [ -f "$target" ]; then
...
cmd_remove() {
    local num="${1:?Usage: checklist remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="checklist-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "checklist v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: checklist help"; exit 1 ;;
esac
