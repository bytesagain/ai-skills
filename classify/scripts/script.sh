#!/usr/bin/env bash
# classify — Classify reference tool. Use when working with classify in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
classify v$VERSION — Classify Reference Tool
...
## What is Classify?
Classify (classify) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with classify.

## Key Concepts
- Core classify principles and fundamentals
- How classify fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Classify Matters
Understanding classify is critical for:
...
## Getting Started
1. Understand the basic classify concepts
...
## Installation
1. Download or clone the classify package
...
cmd_patterns() {
    cat << 'EOF'
# Classify — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for classify
...
cmd_performance() {
    cat << 'EOF'
# Classify — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Classify — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Classify — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "classify v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: classify help"; exit 1 ;;
esac
