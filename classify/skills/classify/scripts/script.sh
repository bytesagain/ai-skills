#!/usr/bin/env bash
# classify -- Train and run text classification models using keyword-based and TF-IDF methods. Use when you need to categorize, label,
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CLASSIFY_DIR:-$HOME/.classify}"
...
show_help() {
    cat << EOF
classify v$VERSION -- Train and run text classification models using keyword-based and TF-IDF methods. Use when you need to categorize, label,
...
cmd_add() {
    local value="${1:?Usage: classify add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: classify search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: classify remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
