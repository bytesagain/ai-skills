#!/usr/bin/env bash
# clean — Clean reference tool. Use when working with clean in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
clean v$VERSION — Clean Reference Tool
...
## What is Clean?
Clean (clean) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with clean.

## Key Concepts
- Core clean principles and fundamentals
- How clean fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Clean Matters
Understanding clean is critical for:
...
## Getting Started
1. Understand the basic clean concepts
...
## Installation
1. Download or clone the clean package
...
cmd_patterns() {
    cat << 'EOF'
# Clean — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for clean
