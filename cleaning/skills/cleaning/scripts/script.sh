#!/usr/bin/env bash
# cleaning -- Schedule and track household cleaning tasks with inventory management. Use when planning cleaning rotations, restocking supplies, or making checklists.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CLEANING_DIR:-$HOME/.cleaning}"
...
show_help() {
    cat << EOF
cleaning v$VERSION -- Schedule and track household cleaning tasks with inventory management. Use when planning cleaning rotations, restocking supplies, or making checklists.

Usage: cleaning <command> [args]
...
cmd_add() {
    local value="${1:?Usage: cleaning add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: cleaning remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "cleaning v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: cleaning help"; exit 1 ;;
esac
