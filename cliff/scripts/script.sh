#!/usr/bin/env bash
# cliff -- Analyze cliff operations. Use when you need to understand cliff mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CLIFF_DIR:-$HOME/.cliff}"
...
show_help() {
    cat << EOF
cliff v$VERSION -- Analyze cliff operations. Use when you need to understand cliff mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: cliff <command> [args]
...
cmd_add() {
    local value="${1:?Usage: cliff add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: cliff search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: cliff remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "cliff v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: cliff help"; exit 1 ;;
esac
