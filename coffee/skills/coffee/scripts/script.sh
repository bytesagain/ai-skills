#!/usr/bin/env bash
# coffee -- Track coffee brewing logs and tasting notes locally. Use when logging espresso shots, comparing brew methods, reviewing bean origins.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${COFFEE_DIR:-$HOME/.coffee}"
...
show_help() {
    cat << EOF
coffee v$VERSION -- Track coffee brewing logs and tasting notes locally. Use when logging espresso shots, comparing brew methods, reviewing bean origins.
...
cmd_add() {
    local value="${1:?Usage: coffee add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: coffee remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "coffee v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: coffee help"; exit 1 ;;
esac
