#!/usr/bin/env bash
# color -- Convert colors between formats and generate palettes using bash and Python. Use when working with hex, RGB, HSL color values or building design palettes.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${COLOR_DIR:-$HOME/.color}"
...
show_help() {
    cat << EOF
color v$VERSION -- Convert colors between formats and generate palettes using bash and Python. Use when working with hex, RGB, HSL color values or building design palettes.
...
cmd_add() {
    local value="${1:?Usage: color add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: color search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: color remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "color v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: color help"; exit 1 ;;
esac
