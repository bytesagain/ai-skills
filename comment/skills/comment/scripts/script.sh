#!/usr/bin/env bash
# comment -- Organize comment data with filtering, analysis, and export tools. Use when collecting feedback, analyzing sentiment, exporting threads.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${COMMENT_DIR:-$HOME/.comment}"
...
show_help() {
    cat << EOF
comment v$VERSION -- Organize comment data with filtering, analysis, and export tools. Use when collecting feedback, analyzing sentiment, exporting threads.

Usage: comment <command> [args]
...
cmd_add() {
    local value="${1:?Usage: comment add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: comment remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "comment v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: comment help"; exit 1 ;;
esac
