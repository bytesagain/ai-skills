#!/usr/bin/env bash
# comparison-table - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${COMPARISON_TABLE_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/comparison-table}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
comparison-table v$VERSION

Multi-purpose utility tool

Usage: comparison-table <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "comparison-table v$VERSION" ;;
