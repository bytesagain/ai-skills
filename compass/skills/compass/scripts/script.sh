#!/usr/bin/env bash
# compass -- Audit security posture and manage credential rotation schedules. Use when auditing passwords, rotating credentials, generating secure tokens.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${COMPASS_DIR:-$HOME/.compass}"
...
show_help() {
    cat << EOF
compass v$VERSION -- Audit security posture and manage credential rotation schedules. Use when auditing passwords, rotating credentials, generating secure tokens.
...
cmd_add() {
    local value="${1:?Usage: compass add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: compass remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
