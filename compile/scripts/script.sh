#!/usr/bin/env bash
# compile — Compile reference tool. Use when working with compile in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
compile v$VERSION — Compile Reference Tool
...
## What is Compile?
Compile (compile) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with compile.

## Key Concepts
- Core compile principles and fundamentals
- How compile fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Compile Matters
Understanding compile is critical for:
...
## Getting Started
1. Understand the basic compile concepts
...
## Installation
1. Download or clone the compile package
...
cmd_patterns() {
    cat << 'EOF'
# Compile — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for compile
...
cmd_security() {
    cat << 'EOF'
# Compile — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Compile — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "compile v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: compile help"; exit 1 ;;
esac
