#!/usr/bin/env bash
# compose — Compose reference tool. Use when working with compose in data contexts.
...
show_help() {
    cat << 'HELPEOF'
compose v$VERSION — Compose Reference Tool
...
## What is Compose?
Compose (compose) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with compose.

## Key Concepts
- Core compose principles and fundamentals
- How compose fits into the broader data ecosystem  
- Essential terminology every practitioner should know

## Why Compose Matters
Understanding compose is critical for:
...
## Getting Started
1. Understand the basic compose concepts
...
## Installation
1. Download or clone the compose package
...
cmd_patterns() {
    cat << 'EOF'
# Compose — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for compose
...
cmd_migration() {
    cat << 'EOF'
# Compose — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "compose v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: compose help"; exit 1 ;;
esac
