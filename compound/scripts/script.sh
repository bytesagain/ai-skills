#!/usr/bin/env bash
# compound -- Calculate compound interest and investment growth using financial formulas. Use when you need to plan savings, compare rates, or project wealth.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${COMPOUND_DIR:-$HOME/.compound}"
...
show_help() {
    cat << EOF
compound v$VERSION -- Calculate compound interest and investment growth using financial formulas. Use when you need to plan savings, compare rates, or project wealth.

Usage: compound <command> [args]
...
cmd_add() {
    local value="${1:?Usage: compound add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: compound search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: compound remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "compound v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: compound help"; exit 1 ;;
esac
