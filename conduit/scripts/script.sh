#!/usr/bin/env bash
# conduit — Conduit reference tool. Use when working with conduit in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
conduit v$VERSION — Conduit Reference Tool
...
## What is Conduit?
Conduit (conduit) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with conduit.

## Key Concepts
- Core conduit principles and fundamentals
- How conduit fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Conduit Matters
Understanding conduit is critical for:
...
## Getting Started
1. Understand the basic conduit concepts
...
## Installation
1. Download or clone the conduit package
...
cmd_patterns() {
    cat << 'EOF'
# Conduit — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for conduit
...
cmd_performance() {
    cat << 'EOF'
# Conduit — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Conduit — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Conduit — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "conduit v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: conduit help"; exit 1 ;;
esac
