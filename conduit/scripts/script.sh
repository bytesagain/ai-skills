#!/usr/bin/env bash
# conduit -- Conduit fill and sizing calculator
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CONDUIT_DIR:-$HOME/.conduit}"
...
show_help() {
    cat << EOF
conduit v$VERSION -- Conduit fill and sizing calculator

Usage: conduit <command> [args]
...
cmd_add() {
    local value="${1:?Usage: conduit add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: conduit search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: conduit remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
