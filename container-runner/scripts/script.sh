#!/usr/bin/env bash
# container-runner — Container Runner reference tool. Use when working with container runner in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
container-runner v$VERSION — Container Runner Reference Tool

Usage: container-runner <command>
...
cmd_intro() {
    cat << 'EOF'
# Container Runner — Overview

## What is Container Runner?
Container Runner (container-runner) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with container runner.

## Key Concepts
- Core container runner principles and fundamentals
- How container runner fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Container Runner Matters
Understanding container runner is critical for:
...
## Getting Started
1. Understand the basic container runner concepts
...
cmd_quickstart() {
    cat << 'EOF'
# Container Runner — Quick Start Guide
...
## Installation
1. Download or clone the container runner package
...
cmd_patterns() {
    cat << 'EOF'
# Container Runner — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for container runner
...
cmd_debugging() {
    cat << 'EOF'
# Container Runner — Debugging Guide
...
cmd_performance() {
    cat << 'EOF'
# Container Runner — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Container Runner — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Container Runner — Migration & Upgrade Guide
...
cmd_cheatsheet() {
    cat << 'EOF'
# Container Runner — Quick Reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "container-runner v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: container-runner help"; exit 1 ;;
esac
