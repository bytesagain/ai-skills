#!/usr/bin/env bash
# convert — Convert reference tool. Use when working with convert in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
convert v$VERSION — Convert Reference Tool
...
## What is Convert?
Convert (convert) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with convert.

## Key Concepts
- Core convert principles and fundamentals
- How convert fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Convert Matters
Understanding convert is critical for:
...
## Getting Started
1. Understand the basic convert concepts
...
## Installation
1. Download or clone the convert package
...
cmd_patterns() {
    cat << 'EOF'
# Convert — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for convert
...
cmd_performance() {
    cat << 'EOF'
# Convert — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Convert — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Convert — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "convert v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: convert help"; exit 1 ;;
esac
