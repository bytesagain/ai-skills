#!/usr/bin/env bash
# convert -- Convert between file formats, units, currencies, encodings, and color codes using Python. Use when you need universal data transformation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CONVERT_DIR:-$HOME/.convert}"
...
show_help() {
    cat << EOF
convert v$VERSION -- Convert between file formats, units, currencies, encodings, and color codes using Python. Use when you need universal data transformation.
...
cmd_add() {
    local value="${1:?Usage: convert add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: convert search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_convert() {
    local input="${1:?Usage: convert convert <input> [format]}"
    local fmt="${2:-base64}"
    case "$fmt" in
        base64) echo "$input" | base64 ;;
        hex) echo "$input" | xxd -p ;;
        upper) echo "$input" | tr "[:lower:]" "[:upper:]" ;;
        lower) echo "$input" | tr "[:upper:]" "[:lower:]" ;;
        reverse) echo "$input" | rev ;;
        *) echo "Formats: base64, hex, upper, lower, reverse" ;;
    esac
    _save_entry "convert" "$input|$fmt"
}

cmd_remove() {
    local num="${1:?Usage: convert remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
