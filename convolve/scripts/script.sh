...
VERSION="1.0.0"
DATA_DIR="${CONVOLVE_DIR:-$HOME/.convolve}"
...
show_help() {
    cat << EOF
convolve v$VERSION -- Compute convolve operations. Use when you need to solve convolve equations, run statistical analysis, or simulate numerical models.

Usage: convolve <command> [args]
...
cmd_add() {
    local value="${1:?Usage: convolve add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: convolve search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_calculate() {
    local expr="${*:?Usage: convolve calculate <expression>}"
...
cmd_remove() {
    local num="${1:?Usage: convolve remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
