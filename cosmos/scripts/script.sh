#!/usr/bin/env bash
# cosmos — Cosmos reference tool. Use when working with cosmos in blockchain contexts.
...
show_help() {
    cat << 'HELPEOF'
cosmos v$VERSION — Cosmos Reference Tool
...
## What is Cosmos?
Cosmos (cosmos) is a specialized tool/concept in the blockchain domain.
It provides essential capabilities for professionals working with cosmos.

## Key Concepts
- Core cosmos principles and fundamentals
- How cosmos fits into the broader blockchain ecosystem  
- Essential terminology every practitioner should know

## Why Cosmos Matters
Understanding cosmos is critical for:
...
## Getting Started
1. Understand the basic cosmos concepts
...
cmd_formulas() {
    cat << 'EOF'
# Cosmos — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Cosmos — Instruments & Tools Overview

## Primary Instruments
- Core tools used in cosmos operations
...
cmd_glossary() {
    cat << 'EOF'
# Cosmos — Key Terms & Definitions

## Core Terminology
- **Cosmos**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "cosmos v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: cosmos help"; exit 1 ;;
esac
