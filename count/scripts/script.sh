#!/usr/bin/env bash
# count -- Count words, lines, and characters with frequency analysis tools. Use when counting frequencies, analyzing text length, aggregating file stats.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${COUNT_DIR:-$HOME/.count}"
...
show_help() {
    cat << EOF
count v$VERSION -- Count words, lines, and characters with frequency analysis tools. Use when counting frequencies, analyzing text length, aggregating file stats.
...
cmd_add() {
    local value="${1:?Usage: count add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: count search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: count remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "count v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: count help"; exit 1 ;;
esac
