#!/usr/bin/env bash
# coupon -- Calculate coupon financial metrics and business data. Use when tracking expenses, analyzing investments, or generating reports.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${COUPON_DIR:-$HOME/.coupon}"
...
show_help() {
    cat << EOF
coupon v$VERSION -- Calculate coupon financial metrics and business data. Use when tracking expenses, analyzing investments, or generating reports.

Usage: coupon <command> [args]
...
cmd_add() {
    local value="${1:?Usage: coupon add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: coupon search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: coupon remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "coupon v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: coupon help"; exit 1 ;;
esac
