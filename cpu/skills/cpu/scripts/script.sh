#!/usr/bin/env bash
# cpu -- Monitor CPU load, per-core usage, and rank top resource-consuming processes. Use when checking temperatures, ranking processes, tracking load.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CPU_DIR:-$HOME/.cpu}"
...
show_help() {
    cat << EOF
cpu v$VERSION -- Monitor CPU load, per-core usage, and rank top resource-consuming processes. Use when checking temperatures, ranking processes, tracking load.

Usage: cpu <command> [args]
...
cmd_add() {
    local value="${1:?Usage: cpu add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: cpu search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: cpu remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "cpu v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: cpu help"; exit 1 ;;
esac
