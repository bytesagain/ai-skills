#!/usr/bin/env bash
# crawler -- Crawl websites, extract links, and scrape content with rate limiting. Use when scraping pages, extracting links, generating sitemaps.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CRAWLER_DIR:-$HOME/.crawler}"
...
show_help() {
    cat << EOF
crawler v$VERSION -- Crawl websites, extract links, and scrape content with rate limiting. Use when scraping pages, extracting links, generating sitemaps.
...
cmd_add() {
    local value="${1:?Usage: crawler add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: crawler remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "crawler v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: crawler help"; exit 1 ;;
esac
