#!/usr/bin/env bash
# crl — Crl reference tool. Use when working with crl in security contexts.
...
show_help() {
    cat << 'HELPEOF'
crl v$VERSION — Crl Reference Tool
...
## What is Crl?
Crl (crl) is a specialized tool/concept in the security domain.
It provides essential capabilities for professionals working with crl.

## Key Concepts
- Core crl principles and fundamentals
- How crl fits into the broader security ecosystem  
- Essential terminology every practitioner should know

## Why Crl Matters
Understanding crl is critical for:
...
## Getting Started
1. Understand the basic crl concepts
...
## Installation
1. Download or clone the crl package
...
cmd_patterns() {
    cat << 'EOF'
# Crl — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for crl
...
case "$CMD" in
    ...
    version|--version|-v) echo "crl v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: crl help"; exit 1 ;;
esac
