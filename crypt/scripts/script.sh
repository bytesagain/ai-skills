#!/usr/bin/env bash
# crypt -- Encrypt, decrypt, hash, and sign data using standard cryptographic algorithms. Use when you need data protection or integrity verification.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CRYPT_DIR:-$HOME/.crypt}"
...
show_help() {
    cat << EOF
crypt v$VERSION -- Encrypt, decrypt, hash, and sign data using standard cryptographic algorithms. Use when you need data protection or integrity verification.

Usage: crypt <command> [args]
...
cmd_add() {
    local value="${1:?Usage: crypt add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: crypt search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: crypt remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "crypt v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: crypt help"; exit 1 ;;
esac
