#!/usr/bin/env bash
# crypto-tax-calc - Financial tracking and analysis tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${CRYPTO_TAX_CALC_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/crypto-tax-calc}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
crypto-tax-calc v$VERSION

Financial tracking and analysis tool

Usage: crypto-tax-calc <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "crypto-tax-calc v$VERSION" ;;
