#!/usr/bin/env bash
# css-helper - Design reference and helper tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${CSS_HELPER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/css-helper}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
css-helper v$VERSION

Design reference and helper tool

Usage: css-helper <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "css-helper v$VERSION" ;;
