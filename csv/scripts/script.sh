#!/usr/bin/env bash
# csv -- Handle csv format data for parsing, conversion, and analysis. Use when working with data pipelines or format migration.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CSV_DIR:-$HOME/.csv}"
...
show_help() {
    cat << EOF
csv v$VERSION -- Handle csv format data for parsing, conversion, and analysis. Use when working with data pipelines or format migration.

Usage: csv <command> [args]
...
cmd_add() {
    local value="${1:?Usage: csv add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: csv search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: csv remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "csv v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: csv help"; exit 1 ;;
esac
