#!/usr/bin/env bash
# curl -- Send HTTP requests and test API endpoints using Python urllib. Use when you need to debug, test, or interact with web services.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CURL_DIR:-$HOME/.curl}"
...
show_help() {
    cat << EOF
curl v$VERSION -- Send HTTP requests and test API endpoints using Python urllib. Use when you need to debug, test, or interact with web services.

Usage: curl <command> [args]
...
cmd_add() {
    local value="${1:?Usage: curl add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: curl remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "curl v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: curl help"; exit 1 ;;
esac
