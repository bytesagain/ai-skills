#!/usr/bin/env bash
# currency -- Plan travel budgets with currency conversion and expense tracking. Use when converting travel money, budgeting trips, comparing destination costs.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CURRENCY_DIR:-$HOME/.currency}"
...
show_help() {
    cat << EOF
currency v$VERSION -- Plan travel budgets with currency conversion and expense tracking. Use when converting travel money, budgeting trips, comparing destination costs.

Usage: currency <command> [args]
...
cmd_add() {
    local value="${1:?Usage: currency add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: currency remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "currency v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: currency help"; exit 1 ;;
esac
