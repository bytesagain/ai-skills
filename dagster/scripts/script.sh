#!/usr/bin/env bash
# dagster — Dagster reference tool. Use when working with dagster in data contexts.
...
show_help() {
    cat << 'HELPEOF'
dagster v$VERSION — Dagster Reference Tool

Usage: dagster <command>
...
## What is Dagster?
Dagster (dagster) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with dagster.

## Key Concepts
- Core dagster principles and fundamentals
- How dagster fits into the broader data ecosystem  
- Essential terminology every practitioner should know

## Why Dagster Matters
Understanding dagster is critical for:
...
## Getting Started
1. Understand the basic dagster concepts
...
## Installation
1. Download or clone the dagster package
...
cmd_patterns() {
    cat << 'EOF'
# Dagster — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for dagster
...
cmd_performance() {
    cat << 'EOF'
# Dagster — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Dagster — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Dagster — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "dagster v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: dagster help"; exit 1 ;;
esac
