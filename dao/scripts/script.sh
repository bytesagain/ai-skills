#!/usr/bin/env bash
# dao — Dao reference tool. Use when working with dao in blockchain contexts.
...
show_help() {
    cat << 'HELPEOF'
dao v$VERSION — Dao Reference Tool
...
## What is Dao?
Dao (dao) is a specialized tool/concept in the blockchain domain.
It provides essential capabilities for professionals working with dao.

## Key Concepts
- Core dao principles and fundamentals
- How dao fits into the broader blockchain ecosystem  
- Essential terminology every practitioner should know

## Why Dao Matters
Understanding dao is critical for:
...
## Getting Started
1. Understand the basic dao concepts
...
cmd_instruments() {
    cat << 'EOF'
# Dao — Instruments & Tools Overview

## Primary Instruments
- Core tools used in dao operations
...
cmd_glossary() {
    cat << 'EOF'
# Dao — Key Terms & Definitions

## Core Terminology
- **Dao**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "dao v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: dao help"; exit 1 ;;
esac
