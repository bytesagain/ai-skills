#!/usr/bin/env bash
# dbt — Dbt reference tool. Use when working with dbt in data contexts.
...
show_help() {
    cat << 'HELPEOF'
dbt v$VERSION — Dbt Reference Tool
...
## What is Dbt?
Dbt (dbt) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with dbt.

## Key Concepts
- Core dbt principles and fundamentals
- How dbt fits into the broader data ecosystem  
- Essential terminology every practitioner should know

## Why Dbt Matters
Understanding dbt is critical for:
...
## Getting Started
1. Understand the basic dbt concepts
...
## Installation
1. Download or clone the dbt package
...
cmd_patterns() {
    cat << 'EOF'
# Dbt — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for dbt
...
case "$CMD" in
    ...
    version|--version|-v) echo "dbt v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: dbt help"; exit 1 ;;
esac
