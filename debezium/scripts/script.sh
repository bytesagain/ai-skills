#!/usr/bin/env bash
# debezium — Debezium reference tool. Use when working with debezium in data contexts.
...
show_help() {
    cat << 'HELPEOF'
debezium v$VERSION — Debezium Reference Tool

Usage: debezium <command>
...
## What is Debezium?
Debezium (debezium) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with debezium.

## Key Concepts
- Core debezium principles and fundamentals
- How debezium fits into the broader data ecosystem  
- Essential terminology every practitioner should know

## Why Debezium Matters
Understanding debezium is critical for:
...
## Getting Started
1. Understand the basic debezium concepts
...
## Installation
1. Download or clone the debezium package
...
cmd_patterns() {
    cat << 'EOF'
# Debezium — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for debezium
...
cmd_security() {
    cat << 'EOF'
# Debezium — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Debezium — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "debezium v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: debezium help"; exit 1 ;;
esac
