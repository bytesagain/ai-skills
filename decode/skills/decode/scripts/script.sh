#!/usr/bin/env bash
# decode -- Decode base64, URLs, JWTs, and encoded formats into readable text. Use when decoding base64, parsing JWT tokens, inspecting encoded payloads.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${DECODE_DIR:-$HOME/.decode}"
...
show_help() {
    cat << EOF
decode v$VERSION -- Decode base64, URLs, JWTs, and encoded formats into readable text. Use when decoding base64, parsing JWT tokens, inspecting encoded payloads.

Usage: decode <command> [args]
...
cmd_add() {
    local value="${1:?Usage: decode add <entry>}"
    shift || true
...
cmd_convert() {
    local input="${1:?Usage: decode convert <input> [format]}"
    local fmt="${2:-base64}"
    case "$fmt" in
...
cmd_remove() {
    local num="${1:?Usage: decode remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "decode v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: decode help"; exit 1 ;;
esac
