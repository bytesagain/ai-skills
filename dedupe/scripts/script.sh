#!/usr/bin/env bash
# dedupe -- Process and transform dedupe data with fast CLI operations. Use when you need to manipulate, convert, or analyze content.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${DEDUPE_DIR:-$HOME/.dedupe}"
...
show_help() {
    cat << EOF
dedupe v$VERSION -- Process and transform dedupe data with fast CLI operations. Use when you need to manipulate, convert, or analyze content.

Usage: dedupe <command> [args]
...
cmd_add() {
    local value="${1:?Usage: dedupe add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: dedupe search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: dedupe remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: dedupe help"; exit 1 ;;
esac
