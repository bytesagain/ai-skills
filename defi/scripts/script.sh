#!/usr/bin/env bash
# defi — Defi reference tool. Use when working with defi in blockchain contexts.
...
show_help() {
    cat << 'HELPEOF'
defi v$VERSION — Defi Reference Tool
...
## What is Defi?
Defi (defi) is a specialized tool/concept in the blockchain domain.
It provides essential capabilities for professionals working with defi.

## Key Concepts
- Core defi principles and fundamentals
- How defi fits into the broader blockchain ecosystem  
- Essential terminology every practitioner should know

## Why Defi Matters
Understanding defi is critical for:
...
## Getting Started
1. Understand the basic defi concepts
...
cmd_formulas() {
    cat << 'EOF'
# Defi — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Defi — Instruments & Tools Overview

## Primary Instruments
- Core tools used in defi operations
...
cmd_glossary() {
    cat << 'EOF'
# Defi — Key Terms & Definitions

## Core Terminology
- **Defi**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "defi v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: defi help"; exit 1 ;;
esac
