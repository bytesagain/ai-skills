#!/usr/bin/env bash
# deno — Deno reference tool. Use when working with deno in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
deno v$VERSION — Deno Reference Tool
...
## What is Deno?
Deno (deno) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with deno.

## Key Concepts
- Core deno principles and fundamentals
- How deno fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Deno Matters
Understanding deno is critical for:
...
## Getting Started
1. Understand the basic deno concepts
...
## Installation
1. Download or clone the deno package
...
cmd_patterns() {
    cat << 'EOF'
# Deno — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for deno
...
case "$CMD" in
    ...
    version|--version|-v) echo "deno v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: deno help"; exit 1 ;;
esac
