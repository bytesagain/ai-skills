#!/usr/bin/env bash
# depreciation -- Calculate depreciation financial metrics and business data. Use when tracking expenses, analyzing investments, or generating financial reports.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${DEPRECIATION_DIR:-$HOME/.depreciation}"
...
show_help() {
    cat << EOF
depreciation v$VERSION -- Calculate depreciation financial metrics and business data. Use when tracking expenses, analyzing investments, or generating financial reports.

Usage: depreciation <command> [args]
...
cmd_add() {
    local value="${1:?Usage: depreciation add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: depreciation search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: depreciation remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="depreciation-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "depreciation v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: depreciation help"; exit 1 ;;
esac
