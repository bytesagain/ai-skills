#!/usr/bin/env bash
# dice -- Roll dice, track scores, and manage game stats for tabletop gaming. Use when rolling dice, tracking scores, ranking players, reviewing history.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${DICE_DIR:-$HOME/.dice}"
...
show_help() {
    cat << EOF
dice v$VERSION -- Roll dice, track scores, and manage game stats for tabletop gaming. Use when rolling dice, tracking scores, ranking players, reviewing history.
...
cmd_add() {
    local value="${1:?Usage: dice add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: dice search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: dice remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "dice v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: dice help"; exit 1 ;;
esac
