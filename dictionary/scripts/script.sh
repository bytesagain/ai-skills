#!/usr/bin/env bash
# dictionary -- Look up definitions, synonyms, etymology, and usage examples fast. Use when checking meanings, finding synonyms, exploring etymology.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${DICTIONARY_DIR:-$HOME/.dictionary}"
...
show_help() {
    cat << EOF
dictionary v$VERSION -- Look up definitions, synonyms, etymology, and usage examples fast. Use when checking meanings, finding synonyms, exploring etymology.

Usage: dictionary <command> [args]
...
cmd_add() {
    local value="${1:?Usage: dictionary add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: dictionary search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: dictionary remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="dictionary-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "dictionary v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: dictionary help"; exit 1 ;;
esac
