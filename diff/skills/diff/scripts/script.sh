#!/usr/bin/env bash
# diff -- Compare files and directories, merge changes, and generate patches. Use when comparing versions, merging changes, generating patch files.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${DIFF_DIR:-$HOME/.diff}"
...
show_help() {
    cat << EOF
diff v$VERSION -- Compare files and directories, merge changes, and generate patches. Use when comparing versions, merging changes, generating patch files.

Usage: diff <command> [args]
...
cmd_add() {
    local value="${1:?Usage: diff add <entry>}"
    shift || true
...
cmd_compare() {
    local a="${1:?Usage: diff compare <a> <b>}"
...
cmd_remove() {
    local num="${1:?Usage: diff remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "diff v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: diff help"; exit 1 ;;
esac
