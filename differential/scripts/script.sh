#!/usr/bin/env bash
# differential -- Diagnose differential operations. Use when you need to inspect differential components, plan maintenance schedules, or troubleshoot vehicle systems.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${DIFFERENTIAL_DIR:-$HOME/.differential}"
...
show_help() {
    cat << EOF
differential v$VERSION -- Diagnose differential operations. Use when you need to inspect differential components, plan maintenance schedules, or troubleshoot vehicle systems.

Usage: differential <command> [args]
...
cmd_add() {
    local value="${1:?Usage: differential add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: differential search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_compare() {
    local a="${1:?Usage: differential compare <a> <b>}"
    local b="${2:?Missing second argument}"
    if [ -f "$a" ] && [ -f "$b" ]; then
        echo "A: $a ($(wc -c < "$a")b)"
        echo "B: $b ($(wc -c < "$b")b)"
        diff "$a" "$b" > /dev/null 2>&1 && echo "IDENTICAL" || echo "DIFFERENT"
    else
        [ "$a" = "$b" ] && echo "IDENTICAL" || echo "DIFFERENT"
    fi
    _save_entry "compare" "$a|$b"
}

cmd_remove() {
    local num="${1:?Usage: differential remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="differential-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "differential v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: differential help"; exit 1 ;;
esac
