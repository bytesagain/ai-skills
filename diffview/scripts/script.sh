#!/usr/bin/env bash
# diffview — Diffview reference tool. Use when working with diffview in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
diffview v$VERSION — Diffview Reference Tool

Usage: diffview <command>
...
## What is Diffview?
Diffview (diffview) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with diffview.

## Key Concepts
- Core diffview principles and fundamentals
- How diffview fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Diffview Matters
Understanding diffview is critical for:
...
## Getting Started
1. Understand the basic diffview concepts
...
## Installation
1. Download or clone the diffview package
...
cmd_patterns() {
    cat << 'EOF'
# Diffview — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for diffview
...
cmd_performance() {
    cat << 'EOF'
# Diffview — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Diffview — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Diffview — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "diffview v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: diffview help"; exit 1 ;;
esac
