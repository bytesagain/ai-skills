#!/usr/bin/env bash
# dig — Dig reference tool. Use when working with dig in network contexts.
...
show_help() {
    cat << 'HELPEOF'
dig v$VERSION — Dig Reference Tool
...
## What is Dig?
Dig (dig) is a specialized tool/concept in the network domain.
It provides essential capabilities for professionals working with dig.

## Key Concepts
- Core dig principles and fundamentals
- How dig fits into the broader network ecosystem  
- Essential terminology every practitioner should know

## Why Dig Matters
Understanding dig is critical for:
...
## Getting Started
1. Understand the basic dig concepts
...
## Installation
1. Download or clone the dig package
...
cmd_patterns() {
    cat << 'EOF'
# Dig — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for dig
...
case "$CMD" in
    ...
    version|--version|-v) echo "dig v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: dig help"; exit 1 ;;
esac
