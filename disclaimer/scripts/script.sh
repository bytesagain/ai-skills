#!/usr/bin/env bash
# disclaimer -- Run disclaimer operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${DISCLAIMER_DIR:-$HOME/.disclaimer}"
...
show_help() {
    cat << EOF
disclaimer v$VERSION -- Run disclaimer operations with simple CLI commands. Use when you need quick data processing or automation.
...
cmd_add() {
    local value="${1:?Usage: disclaimer add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: disclaimer search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: disclaimer remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="disclaimer-export.$fmt"
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: disclaimer help"; exit 1 ;;
esac
