#!/usr/bin/env bash
# disk -- Monitor disk usage, find space hogs, and get cleanup suggestions. Use when checking space, finding large files, monitoring partitions.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${DISK_DIR:-$HOME/.disk}"
...
show_help() {
    cat << EOF
disk v$VERSION -- Monitor disk usage, find space hogs, and get cleanup suggestions. Use when checking space, finding large files, monitoring partitions.
...
cmd_add() {
    local value="${1:?Usage: disk add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: disk remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "disk v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: disk help"; exit 1 ;;
esac
