#!/usr/bin/env bash
# dividend -- Track dividend income, view yield stats, and export payment history. Use when logging payments, calculating yields, tracking payout schedules.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${DIVIDEND_DIR:-$HOME/.dividend}"
...
show_help() {
    cat << EOF
dividend v$VERSION -- Track dividend income, view yield stats, and export payment history. Use when logging payments, calculating yields, tracking payout schedules.

Usage: dividend <command> [args]
...
cmd_add() {
    local value="${1:?Usage: dividend add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: dividend search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: dividend remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "dividend v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: dividend help"; exit 1 ;;
esac
