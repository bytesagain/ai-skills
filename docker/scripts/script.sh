#!/usr/bin/env bash
# docker — Docker reference tool. Use when working with docker in sysops contexts.
...
show_help() {
    cat << 'HELPEOF'
docker v$VERSION — Docker Reference Tool
...
## What is Docker?
Docker (docker) is a specialized tool/concept in the sysops domain.
It provides essential capabilities for professionals working with docker.

## Key Concepts
- Core docker principles and fundamentals
- How docker fits into the broader sysops ecosystem  
- Essential terminology every practitioner should know

## Why Docker Matters
Understanding docker is critical for:
...
## Getting Started
1. Understand the basic docker concepts
...
## Installation
1. Download or clone the docker package
...
cmd_patterns() {
    cat << 'EOF'
# Docker — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for docker
...
cmd_migration() {
    cat << 'EOF'
# Docker — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "docker v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: docker help"; exit 1 ;;
esac
