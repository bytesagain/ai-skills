#!/usr/bin/env bash
# dose -- Track dose personal activities and daily routines. Use when managing health goals, organizing tasks, or building habits.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${DOSE_DIR:-$HOME/.dose}"
...
show_help() {
    cat << EOF
dose v$VERSION -- Track dose personal activities and daily routines. Use when managing health goals, organizing tasks, or building habits.

Usage: dose <command> [args]
...
cmd_add() {
    local value="${1:?Usage: dose add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: dose search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: dose remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "dose v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: dose help"; exit 1 ;;
esac
