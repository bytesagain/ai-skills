#!/usr/bin/env bash
# drain -- Integrate drain operations. Use when you need to configure drain endpoints, manage API connections, or handle service webhooks.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${DRAIN_DIR:-$HOME/.drain}"
...
show_help() {
    cat << EOF
drain v$VERSION -- Integrate drain operations. Use when you need to configure drain endpoints, manage API connections, or handle service webhooks.

Usage: drain <command> [args]
...
cmd_add() {
    local value="${1:?Usage: drain add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: drain search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: drain remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "drain v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: drain help"; exit 1 ;;
esac
