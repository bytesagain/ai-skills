#!/usr/bin/env bash
# drawdown -- Calculate drawdown financial metrics and business data. Use when tracking expenses, analyzing investments, or generating reports.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${DRAWDOWN_DIR:-$HOME/.drawdown}"
...
show_help() {
    cat << EOF
drawdown v$VERSION -- Calculate drawdown financial metrics and business data. Use when tracking expenses, analyzing investments, or generating reports.

Usage: drawdown <command> [args]
...
cmd_add() {
    local value="${1:?Usage: drawdown add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: drawdown search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: drawdown remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: drawdown help"; exit 1 ;;
esac
