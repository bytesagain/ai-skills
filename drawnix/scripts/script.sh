...
DATA_DIR="${HOME}/.local/share/drawnix"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "drawnix v2.0.0"; }

_help() {
    echo "Drawnix v2.0.0 — design toolkit"
    echo ""
    echo "Usage: drawnix <command> [args]"
    echo ""
    echo "Commands:"
...
_search() {
    local term="${1:?Usage: drawnix search <term>}"
...
case "${1:-help}" in
    palette)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/palette.log" 2>/dev/null || echo "  No entries yet. Use: drawnix palette <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/palette.log"
            local total=$(wc -l < "$DATA_DIR/palette.log")
            echo "  [Drawnix] palette: $input"
    ...
    preview)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/preview.log" 2>/dev/null || echo "  No entries yet. Use: drawnix preview <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/preview.log"
            local total=$(wc -l < "$DATA_DIR/preview.log")
            echo "  [Drawnix] preview: $input"
    ...
    generate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/generate.log" 2>/dev/null || echo "  No entries yet. Use: drawnix generate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/generate.log"
            local total=$(wc -l < "$DATA_DIR/generate.log")
            echo "  [Drawnix] generate: $input"
    ...
    convert)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/convert.log" 2>/dev/null || echo "  No entries yet. Use: drawnix convert <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/convert.log"
            local total=$(wc -l < "$DATA_DIR/convert.log")
            echo "  [Drawnix] convert: $input"
    ...
    harmonize)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/harmonize.log" 2>/dev/null || echo "  No entries yet. Use: drawnix harmonize <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/harmonize.log"
            local total=$(wc -l < "$DATA_DIR/harmonize.log")
            echo "  [Drawnix] harmonize: $input"
    ...
    contrast)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/contrast.log" 2>/dev/null || echo "  No entries yet. Use: drawnix contrast <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/contrast.log"
            local total=$(wc -l < "$DATA_DIR/contrast.log")
            echo "  [Drawnix] contrast: $input"
    ...
    export)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/export.log" 2>/dev/null || echo "  No entries yet. Use: drawnix export <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/export.log"
            local total=$(wc -l < "$DATA_DIR/export.log")
            echo "  [Drawnix] export: $input"
    ...
    random)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/random.log" 2>/dev/null || echo "  No entries yet. Use: drawnix random <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/random.log"
            local total=$(wc -l < "$DATA_DIR/random.log")
            echo "  [Drawnix] random: $input"
    ...
    browse)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/browse.log" 2>/dev/null || echo "  No entries yet. Use: drawnix browse <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/browse.log"
            local total=$(wc -l < "$DATA_DIR/browse.log")
            echo "  [Drawnix] browse: $input"
    ...
    mix)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/mix.log" 2>/dev/null || echo "  No entries yet. Use: drawnix mix <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/mix.log"
            local total=$(wc -l < "$DATA_DIR/mix.log")
            echo "  [Drawnix] mix: $input"
    ...
    gradient)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/gradient.log" 2>/dev/null || echo "  No entries yet. Use: drawnix gradient <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/gradient.log"
            local total=$(wc -l < "$DATA_DIR/gradient.log")
            echo "  [Drawnix] gradient: $input"
    ...
    swatch)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/swatch.log" 2>/dev/null || echo "  No entries yet. Use: drawnix swatch <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/swatch.log"
            local total=$(wc -l < "$DATA_DIR/swatch.log")
            echo "  [Drawnix] swatch: $input"
    ...
    *)
        echo "Unknown: $1 — run 'drawnix help'"
        exit 1
        ;;
esac