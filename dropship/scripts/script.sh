#!/usr/bin/env bash
# dropship -- Run dropship operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${DROPSHIP_DIR:-$HOME/.dropship}"
...
show_help() {
    cat << EOF
dropship v$VERSION -- Run dropship operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: dropship <command> [args]
...
cmd_add() {
    local value="${1:?Usage: dropship add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: dropship search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: dropship remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "dropship v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: dropship help"; exit 1 ;;
esac
