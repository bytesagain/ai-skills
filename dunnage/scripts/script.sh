#!/usr/bin/env bash
# dunnage -- Run dunnage operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${DUNNAGE_DIR:-$HOME/.dunnage}"
...
show_help() {
    cat << EOF
dunnage v$VERSION -- Run dunnage operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: dunnage <command> [args]
...
cmd_add() {
    local value="${1:?Usage: dunnage add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: dunnage search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: dunnage remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "dunnage v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: dunnage help"; exit 1 ;;
esac
