#!/usr/bin/env bash
# ebook -- Manage ebook collections, track reading progress, and export highlights using bash and Python. Use when cataloging books, logging reading sessions,
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${EBOOK_DIR:-$HOME/.ebook}"
...
show_help() {
    cat << EOF
ebook v$VERSION -- Manage ebook collections, track reading progress, and export highlights using bash and Python. Use when cataloging books, logging reading sessions,

Usage: ebook <command> [args]
...
cmd_add() {
    local value="${1:?Usage: ebook add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: ebook search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: ebook remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "ebook v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: ebook help"; exit 1 ;;
esac
