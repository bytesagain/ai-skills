#!/usr/bin/env bash
# ec -- Run ec operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${EC_DIR:-$HOME/.ec}"
...
show_help() {
    cat << EOF
ec v$VERSION -- Run ec operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: ec <command> [args]
...
cmd_add() {
    local value="${1:?Usage: ec add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: ec search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: ec remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "ec v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: ec help"; exit 1 ;;
esac
