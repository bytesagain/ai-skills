#!/usr/bin/env bash
# embed -- Generate and search text embedding vectors using TF-IDF and cosine similarity. Use when you need semantic search or text similarity analysis.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${EMBED_DIR:-$HOME/.embed}"
...
show_help() {
    cat << EOF
embed v$VERSION -- Generate and search text embedding vectors using TF-IDF and cosine similarity. Use when you need semantic search or text similarity analysis.

Usage: embed <command> [args]
...
cmd_add() {
    local value="${1:?Usage: embed add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: embed search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: embed remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "embed v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: embed help"; exit 1 ;;
esac
