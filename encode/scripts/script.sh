#!/usr/bin/env bash
# encode -- Encode text to base64, URL-safe strings, and HTML entity formats. Use when encoding payloads, escaping URLs, converting HTML entities.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ENCODE_DIR:-$HOME/.encode}"
...
show_help() {
    cat << EOF
encode v$VERSION -- Encode text to base64, URL-safe strings, and HTML entity formats. Use when encoding payloads, escaping URLs, converting HTML entities.

Usage: encode <command> [args]
...
cmd_add() {
    local value="${1:?Usage: encode add <entry>}"
    shift || true
...
cmd_convert() {
    local input="${1:?Usage: encode convert <input> [format]}"
    local fmt="${2:-base64}"
    case "$fmt" in
...
cmd_remove() {
    local num="${1:?Usage: encode remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
