#!/usr/bin/env bash
# endeffector -- End effector design and selection tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ENDEFFECTOR_DIR:-$HOME/.endeffector}"
...
show_help() {
    cat << EOF
endeffector v$VERSION -- End effector design and selection tool

Usage: endeffector <command> [args]
...
cmd_add() {
    local value="${1:?Usage: endeffector add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: endeffector search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: endeffector remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="endeffector-export.$fmt"
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: endeffector help"; exit 1 ;;
esac
