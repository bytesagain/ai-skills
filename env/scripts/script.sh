#!/usr/bin/env bash
# env -- Manage environment variables with profiles, validation, and templating using bash. Use when you need to organize, switch, or audit env configs.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ENV_DIR:-$HOME/.env}"
...
show_help() {
    cat << EOF
env v$VERSION -- Manage environment variables with profiles, validation, and templating using bash. Use when you need to organize, switch, or audit env configs.
...
cmd_add() {
    local value="${1:?Usage: env add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: env search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: env remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
