#!/usr/bin/env bash
# eplan -- Electrical planning and schematic design tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${EPLAN_DIR:-$HOME/.eplan}"
...
show_help() {
    cat << EOF
eplan v$VERSION -- Electrical planning and schematic design tool

Usage: eplan <command> [args]
...
cmd_add() {
    local value="${1:?Usage: eplan add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: eplan search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: eplan remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "eplan v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: eplan help"; exit 1 ;;
esac
