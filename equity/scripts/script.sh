#!/usr/bin/env bash
# equity -- Model cap tables, dilution scenarios, and vesting schedules for startups. Use when planning fundraising, pricing options, or tracking equity.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${EQUITY_DIR:-$HOME/.equity}"
...
show_help() {
    cat << EOF
equity v$VERSION -- Model cap tables, dilution scenarios, and vesting schedules for startups. Use when planning fundraising, pricing options, or tracking equity.

Usage: equity <command> [args]
...
cmd_add() {
    local value="${1:?Usage: equity add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: equity search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: equity remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "equity v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: equity help"; exit 1 ;;
esac
