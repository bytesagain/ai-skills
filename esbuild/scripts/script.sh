#!/usr/bin/env bash
# esbuild — Esbuild reference tool. Use when working with esbuild in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
esbuild v$VERSION — Esbuild Reference Tool

Usage: esbuild <command>
...
## What is Esbuild?
Esbuild (esbuild) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with esbuild.

## Key Concepts
- Core esbuild principles and fundamentals
- How esbuild fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Esbuild Matters
Understanding esbuild is critical for:
...
## Getting Started
1. Understand the basic esbuild concepts
...
## Installation
1. Download or clone the esbuild package
...
cmd_patterns() {
    cat << 'EOF'
# Esbuild — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for esbuild
...
cmd_security() {
    cat << 'EOF'
# Esbuild — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Esbuild — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "esbuild v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: esbuild help"; exit 1 ;;
esac
