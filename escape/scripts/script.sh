#!/usr/bin/env bash
# escape — Escape reference tool. Use when working with escape in security contexts.
...
show_help() {
    cat << 'HELPEOF'
escape v$VERSION — Escape Reference Tool
...
## What is Escape?
Escape (escape) is a specialized tool/concept in the security domain.
It provides essential capabilities for professionals working with escape.

## Key Concepts
- Core escape principles and fundamentals
- How escape fits into the broader security ecosystem  
- Essential terminology every practitioner should know

## Why Escape Matters
Understanding escape is critical for:
...
## Getting Started
1. Understand the basic escape concepts
...
## Installation
1. Download or clone the escape package
...
cmd_patterns() {
    cat << 'EOF'
# Escape — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for escape
...
cmd_performance() {
    cat << 'EOF'
# Escape — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Escape — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Escape — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: escape help"; exit 1 ;;
esac
