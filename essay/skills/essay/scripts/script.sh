#!/usr/bin/env bash
# essay -- Draft and polish essays with outlines, argument structure, and paragraph edits. Use when writing papers, structuring arguments, or editing prose.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ESSAY_DIR:-$HOME/.essay}"
...
show_help() {
    cat << EOF
essay v$VERSION -- Draft and polish essays with outlines, argument structure, and paragraph edits. Use when writing papers, structuring arguments, or editing prose.

Usage: essay <command> [args]
...
cmd_add() {
    local value="${1:?Usage: essay add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: essay remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "essay v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: essay help"; exit 1 ;;
esac
