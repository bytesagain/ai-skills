#!/usr/bin/env bash
# estoppel -- Run estoppel operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ESTOPPEL_DIR:-$HOME/.estoppel}"
...
show_help() {
    cat << EOF
estoppel v$VERSION -- Run estoppel operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: estoppel <command> [args]
...
cmd_add() {
    local value="${1:?Usage: estoppel add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: estoppel search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: estoppel remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "estoppel v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: estoppel help"; exit 1 ;;
esac
