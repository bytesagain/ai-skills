#!/usr/bin/env bash
# etf -- Analyze ETF holdings, compare fund metrics, and review sector allocation data. Use when screening ETFs, comparing ratios, or tracking allocations.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ETF_DIR:-$HOME/.etf}"
...
show_help() {
    cat << EOF
etf v$VERSION -- Analyze ETF holdings, compare fund metrics, and review sector allocation data. Use when screening ETFs, comparing ratios, or tracking allocations.

Usage: etf <command> [args]
...
cmd_add() {
    local value="${1:?Usage: etf add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: etf search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: etf remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "etf v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: etf help"; exit 1 ;;
esac
