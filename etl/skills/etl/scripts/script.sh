#!/usr/bin/env bash
# etl -- Build ETL pipelines with data ingestion, cleaning, and validation steps. Use when ingesting sources, transforming formats, validating data, or scheduling loads.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ETL_DIR:-$HOME/.etl}"
...
show_help() {
    cat << EOF
etl v$VERSION -- Build ETL pipelines with data ingestion, cleaning, and validation steps. Use when ingesting sources, transforming formats, validating data, or scheduling loads.
...
cmd_add() {
    local value="${1:?Usage: etl add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: etl search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: etl remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "etl v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: etl help"; exit 1 ;;
esac
