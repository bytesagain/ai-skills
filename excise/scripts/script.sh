#!/usr/bin/env bash
# excise -- Run excise operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${EXCISE_DIR:-$HOME/.excise}"
...
show_help() {
    cat << EOF
excise v$VERSION -- Run excise operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: excise <command> [args]
...
cmd_add() {
    local value="${1:?Usage: excise add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: excise search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: excise remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "excise v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: excise help"; exit 1 ;;
esac
