...
VERSION="1.0.0"
DATA_DIR="${EXERCISE_DIR:-$HOME/.exercise}"
...
show_help() {
    cat << EOF
exercise v$VERSION -- exercise tool

Usage: exercise <command> [args]
...
cmd_add() {
    local value="${1:?Usage: exercise add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: exercise search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: exercise remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "exercise v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: exercise help"; exit 1 ;;
esac
