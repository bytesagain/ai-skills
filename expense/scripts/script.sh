#!/usr/bin/env bash
# expense -- Calculate expense financial metrics and business data. Use when tracking expenses, analyzing investments, or generating financial reports.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${EXPENSE_DIR:-$HOME/.expense}"
...
show_help() {
    cat << EOF
expense v$VERSION -- Calculate expense financial metrics and business data. Use when tracking expenses, analyzing investments, or generating financial reports.

Usage: expense <command> [args]
...
cmd_add() {
    local value="${1:?Usage: expense add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: expense search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: expense remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "expense v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: expense help"; exit 1 ;;
esac
