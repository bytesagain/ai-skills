#!/usr/bin/env bash
# feedback -- Collect user feedback, categorize by theme, and track issue resolution. Use when gathering opinions, prioritizing bugs, or tracking fix progress.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${FEEDBACK_DIR:-$HOME/.feedback}"
...
show_help() {
    cat << EOF
feedback v$VERSION -- Collect user feedback, categorize by theme, and track issue resolution. Use when gathering opinions, prioritizing bugs, or tracking fix progress.

Usage: feedback <command> [args]
...
cmd_add() {
    local value="${1:?Usage: feedback add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: feedback remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "feedback v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: feedback help"; exit 1 ;;
esac
