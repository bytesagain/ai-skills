#!/usr/bin/env bash
# finetune -- Manage model fine-tuning datasets using CLI tools. Use when preparing, validating, or transforming training data for LLM fine-tuning.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${FINETUNE_DIR:-$HOME/.finetune}"
...
show_help() {
    cat << EOF
finetune v$VERSION -- Manage model fine-tuning datasets using CLI tools. Use when preparing, validating, or transforming training data for LLM fine-tuning.

Usage: finetune <command> [args]
...
cmd_add() {
    local value="${1:?Usage: finetune add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: finetune search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: finetune remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
