#!/usr/bin/env bash
# firewall-manager — Firewall Manager reference tool. Use when working with firewall manager in security contexts.
...
show_help() {
    cat << 'HELPEOF'
firewall-manager v$VERSION — Firewall Manager Reference Tool

Usage: firewall-manager <command>
...
Firewall Manager (firewall-manager) is a specialized tool/concept in the security domain.
It provides essential capabilities for professionals working with firewall manager.

## Key Concepts
- Core firewall manager principles and fundamentals
- How firewall manager fits into the broader security ecosystem  
- Essential terminology every practitioner should know

## Why Firewall Manager Matters
Understanding firewall manager is critical for:
...
## Getting Started
1. Understand the basic firewall manager concepts
...
cmd_quickstart() {
    cat << 'EOF'
# Firewall Manager — Quick Start Guide
...
## Installation
1. Download or clone the firewall manager package
...
cmd_patterns() {
    cat << 'EOF'
# Firewall Manager — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for firewall manager
...
cmd_debugging() {
    cat << 'EOF'
# Firewall Manager — Debugging Guide
...
cmd_performance() {
    cat << 'EOF'
# Firewall Manager — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Firewall Manager — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Firewall Manager — Migration & Upgrade Guide
...
cmd_cheatsheet() {
    cat << 'EOF'
# Firewall Manager — Quick Reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "firewall-manager v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: firewall-manager help"; exit 1 ;;
esac
