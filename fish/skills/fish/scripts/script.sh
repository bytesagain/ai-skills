#!/usr/bin/env bash
# fish -- Configure Fish shell abbreviations, completions, and prompt themes easily. Use when setting aliases, writing functions, customizing prompts, or adding plugins.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${FISH_DIR:-$HOME/.fish}"
...
show_help() {
    cat << EOF
fish v$VERSION -- Configure Fish shell abbreviations, completions, and prompt themes easily. Use when setting aliases, writing functions, customizing prompts, or adding plugins.

Usage: fish <command> [args]
...
cmd_add() {
    local value="${1:?Usage: fish add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: fish search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: fish remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "fish v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: fish help"; exit 1 ;;
esac
