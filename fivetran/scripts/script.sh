#!/usr/bin/env bash
# fivetran — Fivetran reference tool. Use when working with fivetran in data contexts.
...
show_help() {
    cat << 'HELPEOF'
fivetran v$VERSION — Fivetran Reference Tool

Usage: fivetran <command>
...
## What is Fivetran?
Fivetran (fivetran) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with fivetran.

## Key Concepts
- Core fivetran principles and fundamentals
- How fivetran fits into the broader data ecosystem  
- Essential terminology every practitioner should know

## Why Fivetran Matters
Understanding fivetran is critical for:
...
## Getting Started
1. Understand the basic fivetran concepts
...
## Installation
1. Download or clone the fivetran package
...
cmd_patterns() {
    cat << 'EOF'
# Fivetran — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for fivetran
...
cmd_performance() {
    cat << 'EOF'
# Fivetran — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Fivetran — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Fivetran — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "fivetran v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: fivetran help"; exit 1 ;;
esac
