#!/usr/bin/env bash
# flatten -- Process and transform flatten data with fast CLI operations. Use when you need to manipulate, convert, or analyze content.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${FLATTEN_DIR:-$HOME/.flatten}"
...
show_help() {
    cat << EOF
flatten v$VERSION -- Process and transform flatten data with fast CLI operations. Use when you need to manipulate, convert, or analyze content.

Usage: flatten <command> [args]
...
cmd_add() {
    local value="${1:?Usage: flatten add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: flatten search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: flatten remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: flatten help"; exit 1 ;;
esac
