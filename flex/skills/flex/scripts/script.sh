#!/usr/bin/env bash
# flex -- Generate CSS Flexbox layouts using interactive CLI commands. Use when building responsive flex containers, rows, columns, or alignment configurations.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${FLEX_DIR:-$HOME/.flex}"
...
show_help() {
    cat << EOF
flex v$VERSION -- Generate CSS Flexbox layouts using interactive CLI commands. Use when building responsive flex containers, rows, columns, or alignment configurations.

Usage: flex <command> [args]
...
cmd_add() {
    local value="${1:?Usage: flex add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: flex search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: flex remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "flex v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: flex help"; exit 1 ;;
esac
