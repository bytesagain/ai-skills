#!/usr/bin/env bash
# flink — Flink reference tool. Use when working with flink in data contexts.
...
show_help() {
    cat << 'HELPEOF'
flink v$VERSION — Flink Reference Tool
...
## What is Flink?
Flink (flink) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with flink.

## Key Concepts
- Core flink principles and fundamentals
- How flink fits into the broader data ecosystem  
- Essential terminology every practitioner should know

## Why Flink Matters
Understanding flink is critical for:
...
## Getting Started
1. Understand the basic flink concepts
...
## Installation
1. Download or clone the flink package
...
cmd_patterns() {
    cat << 'EOF'
# Flink — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for flink
...
case "$CMD" in
    ...
    version|--version|-v) echo "flink v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: flink help"; exit 1 ;;
esac
