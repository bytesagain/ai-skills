#!/usr/bin/env bash
# flip — Flip reference tool. Use when working with flip in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
flip v$VERSION — Flip Reference Tool
...
## What is Flip?
Flip (flip) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with flip.

## Key Concepts
- Core flip principles and fundamentals
- How flip fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Flip Matters
Understanding flip is critical for:
...
## Getting Started
1. Understand the basic flip concepts
...
## Installation
1. Download or clone the flip package
...
cmd_patterns() {
    cat << 'EOF'
# Flip — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for flip
...
case "$CMD" in
    ...
    version|--version|-v) echo "flip v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: flip help"; exit 1 ;;
esac
