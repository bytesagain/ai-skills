#!/usr/bin/env bash
# flip -- Generate flip UI elements and design assets. Use when building interfaces, creating visual components, or styling web pages.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${FLIP_DIR:-$HOME/.flip}"
...
show_help() {
    cat << EOF
flip v$VERSION -- Generate flip UI elements and design assets. Use when building interfaces, creating visual components, or styling web pages.

Usage: flip <command> [args]
...
cmd_add() {
    local value="${1:?Usage: flip add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: flip search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: flip remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
