...
show_help() {
    cat << EOF
fmea v$VERSION -- Failure mode and effects analysis tool

Usage: fmea <command> [args]
...
cmd_add() {
    local value="${1:?Usage: fmea add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: fmea search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: fmea remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "fmea v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: fmea help"; exit 1 ;;
esac
