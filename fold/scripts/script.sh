#!/usr/bin/env bash
# fold -- Handle fold operations. Use when you need to parse fold structures, migrate data formats, or run analytics pipelines.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${FOLD_DIR:-$HOME/.fold}"
...
show_help() {
    cat << EOF
fold v$VERSION -- Handle fold operations. Use when you need to parse fold structures, migrate data formats, or run analytics pipelines.

Usage: fold <command> [args]
...
cmd_add() {
    local value="${1:?Usage: fold add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: fold search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: fold remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "fold v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: fold help"; exit 1 ;;
esac
