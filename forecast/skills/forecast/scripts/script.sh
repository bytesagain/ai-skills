#!/usr/bin/env bash
# forecast -- Calculate forecast financial metrics and business data. Use when tracking expenses, analyzing investments, or generating financial reports.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${FORECAST_DIR:-$HOME/.forecast}"
...
show_help() {
    cat << EOF
forecast v$VERSION -- Calculate forecast financial metrics and business data. Use when tracking expenses, analyzing investments, or generating financial reports.

Usage: forecast <command> [args]
...
cmd_add() {
    local value="${1:?Usage: forecast add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: forecast search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: forecast remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "forecast v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: forecast help"; exit 1 ;;
esac
