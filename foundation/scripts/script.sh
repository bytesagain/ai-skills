#!/usr/bin/env bash
# foundation — Foundation reference tool. Use when working with foundation in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
foundation v$VERSION — Foundation Reference Tool
...
## What is Foundation?
Foundation (foundation) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with foundation.

## Key Concepts
- Core foundation principles and fundamentals
- How foundation fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Foundation Matters
Understanding foundation is critical for:
...
## Getting Started
1. Understand the basic foundation concepts
...
## Installation
1. Download or clone the foundation package
...
cmd_patterns() {
    cat << 'EOF'
# Foundation — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for foundation
...
case "$CMD" in
    ...
    version|--version|-v) echo "foundation v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: foundation help"; exit 1 ;;
esac
