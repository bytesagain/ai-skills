#!/usr/bin/env bash
# freeport -- Run freeport operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${FREEPORT_DIR:-$HOME/.freeport}"
...
show_help() {
    cat << EOF
freeport v$VERSION -- Run freeport operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: freeport <command> [args]
...
cmd_add() {
    local value="${1:?Usage: freeport add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: freeport search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: freeport remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: freeport help"; exit 1 ;;
esac
