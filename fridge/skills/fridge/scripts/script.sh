#!/usr/bin/env bash
# fridge -- Track fridge and pantry inventory with expiry reminders and grocery lists. Use when logging groceries, checking expiry dates, or building shopping lists.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${FRIDGE_DIR:-$HOME/.fridge}"
...
show_help() {
    cat << EOF
fridge v$VERSION -- Track fridge and pantry inventory with expiry reminders and grocery lists. Use when logging groceries, checking expiry dates, or building shopping lists.

Usage: fridge <command> [args]
...
cmd_add() {
    local value="${1:?Usage: fridge add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: fridge remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "fridge v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: fridge help"; exit 1 ;;
esac
