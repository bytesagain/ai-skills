#!/usr/bin/env bash
# fuel -- Track fuel purchases, calculate consumption, and manage refueling records. Use when logging fill-ups, computing fuel economy, or comparing usage trends.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${FUEL_DIR:-$HOME/.fuel}"
...
show_help() {
    cat << EOF
fuel v$VERSION -- Track fuel purchases, calculate consumption, and manage refueling records. Use when logging fill-ups, computing fuel economy, or comparing usage trends.

Usage: fuel <command> [args]
...
cmd_add() {
    local value="${1:?Usage: fuel add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: fuel remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "fuel v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: fuel help"; exit 1 ;;
esac
