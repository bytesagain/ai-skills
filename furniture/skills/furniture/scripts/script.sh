#!/usr/bin/env bash
# furniture -- Track home furniture, schedule maintenance, and manage warranty details. Use when cataloging furniture, scheduling cleaning, or tracking warranty expiry dates.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${FURNITURE_DIR:-$HOME/.furniture}"
...
show_help() {
    cat << EOF
furniture v$VERSION -- Track home furniture, schedule maintenance, and manage warranty details. Use when cataloging furniture, scheduling cleaning, or tracking warranty expiry dates.

Usage: furniture <command> [args]
...
cmd_add() {
    local value="${1:?Usage: furniture add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: furniture search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: furniture remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="furniture-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "furniture v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: furniture help"; exit 1 ;;
esac
