#!/usr/bin/env bash
# gamma — Gamma reference tool. Use when working with gamma in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
gamma v$VERSION — Gamma Reference Tool
...
## What is Gamma?
Gamma (gamma) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with gamma.

## Key Concepts
- Core gamma principles and fundamentals
- How gamma fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Gamma Matters
Understanding gamma is critical for:
...
## Getting Started
1. Understand the basic gamma concepts
...
cmd_formulas() {
    cat << 'EOF'
# Gamma — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Gamma — Instruments & Tools Overview

## Primary Instruments
- Core tools used in gamma operations
...
cmd_glossary() {
    cat << 'EOF'
# Gamma — Key Terms & Definitions

## Core Terminology
- **Gamma**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "gamma v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: gamma help"; exit 1 ;;
esac
