#!/usr/bin/env bash
# gatsby — Gatsby reference tool. Use when working with gatsby in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
gatsby v$VERSION — Gatsby Reference Tool

Usage: gatsby <command>
...
## What is Gatsby?
Gatsby (gatsby) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with gatsby.

## Key Concepts
- Core gatsby principles and fundamentals
- How gatsby fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Gatsby Matters
Understanding gatsby is critical for:
...
## Getting Started
1. Understand the basic gatsby concepts
...
## Installation
1. Download or clone the gatsby package
...
cmd_patterns() {
    cat << 'EOF'
# Gatsby — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for gatsby
...
cmd_security() {
    cat << 'EOF'
# Gatsby — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Gatsby — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "gatsby v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: gatsby help"; exit 1 ;;
esac
