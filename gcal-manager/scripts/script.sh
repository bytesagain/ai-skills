#!/usr/bin/env bash
# gcal-manager — Gcal Manager reference tool. Use when working with gcal manager in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
gcal-manager v$VERSION — Gcal Manager Reference Tool

Usage: gcal-manager <command>
...
cmd_intro() {
    cat << 'EOF'
# Gcal Manager — Overview

## What is Gcal Manager?
Gcal Manager (gcal-manager) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with gcal manager.

## Key Concepts
- Core gcal manager principles and fundamentals
- How gcal manager fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Gcal Manager Matters
Understanding gcal manager is critical for:
...
## Getting Started
1. Understand the basic gcal manager concepts
...
cmd_quickstart() {
    cat << 'EOF'
# Gcal Manager — Quick Start Guide
...
## Installation
1. Download or clone the gcal manager package
...
cmd_patterns() {
    cat << 'EOF'
# Gcal Manager — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for gcal manager
...
cmd_performance() {
    cat << 'EOF'
# Gcal Manager — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Gcal Manager — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Gcal Manager — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "gcal-manager v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: gcal-manager help"; exit 1 ;;
esac
