#!/usr/bin/env bash
# gearbox -- Diagnose gearbox operations. Use when you need to inspect gearbox components, plan maintenance schedules, or troubleshoot vehicle systems.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${GEARBOX_DIR:-$HOME/.gearbox}"
...
show_help() {
    cat << EOF
gearbox v$VERSION -- Diagnose gearbox operations. Use when you need to inspect gearbox components, plan maintenance schedules, or troubleshoot vehicle systems.

Usage: gearbox <command> [args]
...
cmd_add() {
    local value="${1:?Usage: gearbox add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: gearbox search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: gearbox remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "gearbox v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: gearbox help"; exit 1 ;;
esac
