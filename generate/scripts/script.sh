#!/usr/bin/env bash
# generate -- generate tool for creating outputs
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${GENERATE_DIR:-$HOME/.generate}"
...
show_help() {
    cat << EOF
generate v$VERSION -- generate tool for creating outputs
...
cmd_add() {
    local value="${1:?Usage: generate add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: generate search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_generate() {
    local name="${1:?Usage: generate generate <name>}"
    local tmpl="${2:-default}"
    local out="$DATA_DIR/${name}.txt"
    echo "# Generated by generate" > "$out"
...
cmd_remove() {
    local num="${1:?Usage: generate remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "generate v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: generate help"; exit 1 ;;
esac
