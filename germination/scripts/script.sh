#!/usr/bin/env bash
# germination -- Run germination operations with simple CLI commands. Use when you need quick data processing or automation.
...
show_help() {
    cat << EOF
germination v$VERSION -- Run germination operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: germination <command> [args]
...
cmd_add() {
    local value="${1:?Usage: germination add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: germination search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: germination remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="germination-export.$fmt"
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: germination help"; exit 1 ;;
esac
