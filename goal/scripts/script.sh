#!/usr/bin/env bash
# goal -- Track goal personal activities and daily routines. Use when managing health goals, organizing tasks, or building productive habits.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${GOAL_DIR:-$HOME/.goal}"
...
show_help() {
    cat << EOF
goal v$VERSION -- Track goal personal activities and daily routines. Use when managing health goals, organizing tasks, or building productive habits.

Usage: goal <command> [args]
...
cmd_add() {
    local value="${1:?Usage: goal add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: goal search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: goal remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: goal help"; exit 1 ;;
esac
