#!/usr/bin/env bash
# golang -- Build, test, lint, and format Go projects with integrated dev tooling. Use when compiling binaries, running tests, linting code, or formatting files.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${GOLANG_DIR:-$HOME/.golang}"
...
show_help() {
    cat << EOF
golang v$VERSION -- Build, test, lint, and format Go projects with integrated dev tooling. Use when compiling binaries, running tests, linting code, or formatting files.

Usage: golang <command> [args]
...
cmd_add() {
    local value="${1:?Usage: golang add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: golang search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: golang remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "golang v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: golang help"; exit 1 ;;
esac
