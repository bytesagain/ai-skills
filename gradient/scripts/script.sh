#!/usr/bin/env bash
# gradient -- Generate CSS gradient code, preview gradient combinations, and build gradient palettes using bash and Python. Use when creating linear, radial,
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${GRADIENT_DIR:-$HOME/.gradient}"
...
show_help() {
    cat << EOF
gradient v$VERSION -- Generate CSS gradient code, preview gradient combinations, and build gradient palettes using bash and Python. Use when creating linear, radial,

Usage: gradient <command> [args]
...
cmd_add() {
    local value="${1:?Usage: gradient add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: gradient search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: gradient remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "gradient v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: gradient help"; exit 1 ;;
esac
