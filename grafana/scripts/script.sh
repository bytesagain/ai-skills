#!/usr/bin/env bash
# grafana — Grafana reference tool. Use when working with grafana in sysops contexts.
...
show_help() {
    cat << 'HELPEOF'
grafana v$VERSION — Grafana Reference Tool

Usage: grafana <command>
...
## What is Grafana?
Grafana (grafana) is a specialized tool/concept in the sysops domain.
It provides essential capabilities for professionals working with grafana.

## Key Concepts
- Core grafana principles and fundamentals
- How grafana fits into the broader sysops ecosystem  
- Essential terminology every practitioner should know

## Why Grafana Matters
Understanding grafana is critical for:
...
## Getting Started
1. Understand the basic grafana concepts
...
## Installation
1. Download or clone the grafana package
...
cmd_patterns() {
    cat << 'EOF'
# Grafana — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for grafana
...
cmd_security() {
    cat << 'EOF'
# Grafana — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Grafana — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "grafana v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: grafana help"; exit 1 ;;
esac
