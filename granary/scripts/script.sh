#!/usr/bin/env bash
# granary -- Run granary operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${GRANARY_DIR:-$HOME/.granary}"
...
show_help() {
    cat << EOF
granary v$VERSION -- Run granary operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: granary <command> [args]
...
cmd_add() {
    local value="${1:?Usage: granary add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: granary search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: granary remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "granary v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: granary help"; exit 1 ;;
esac
