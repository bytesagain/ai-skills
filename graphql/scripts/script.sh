#!/usr/bin/env bash
# graphql — Graphql reference tool. Use when working with graphql in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
graphql v$VERSION — Graphql Reference Tool
...
## What is Graphql?
Graphql (graphql) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with graphql.

## Key Concepts
- Core graphql principles and fundamentals
- How graphql fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Graphql Matters
Understanding graphql is critical for:
...
## Getting Started
1. Understand the basic graphql concepts
...
## Installation
1. Download or clone the graphql package
...
cmd_patterns() {
    cat << 'EOF'
# Graphql — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for graphql
...
cmd_migration() {
    cat << 'EOF'
# Graphql — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "graphql v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: graphql help"; exit 1 ;;
esac
