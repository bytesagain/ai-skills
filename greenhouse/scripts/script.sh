#!/usr/bin/env bash
# greenhouse -- Run greenhouse operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${GREENHOUSE_DIR:-$HOME/.greenhouse}"
...
show_help() {
    cat << EOF
greenhouse v$VERSION -- Run greenhouse operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: greenhouse <command> [args]
...
cmd_add() {
    local value="${1:?Usage: greenhouse add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: greenhouse search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: greenhouse remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="greenhouse-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "greenhouse v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: greenhouse help"; exit 1 ;;
esac
