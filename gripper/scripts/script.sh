#!/usr/bin/env bash
# gripper -- Robot gripper selection and force calculator
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${GRIPPER_DIR:-$HOME/.gripper}"
...
show_help() {
    cat << EOF
gripper v$VERSION -- Robot gripper selection and force calculator

Usage: gripper <command> [args]
...
cmd_add() {
    local value="${1:?Usage: gripper add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: gripper search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: gripper remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: gripper help"; exit 1 ;;
esac
