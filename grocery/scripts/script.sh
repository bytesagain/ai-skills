#!/usr/bin/env bash
# grocery -- Track grocery personal activities and daily routines. Use when managing health goals, organizing tasks, or building productive habits.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${GROCERY_DIR:-$HOME/.grocery}"
...
show_help() {
    cat << EOF
grocery v$VERSION -- Track grocery personal activities and daily routines. Use when managing health goals, organizing tasks, or building productive habits.

Usage: grocery <command> [args]
...
cmd_add() {
    local value="${1:?Usage: grocery add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: grocery search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: grocery remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "grocery v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: grocery help"; exit 1 ;;
esac
