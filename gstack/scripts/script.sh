#!/usr/bin/env bash
# gstack -- Apply Garry Tan's Claude Code setup with 10 tools for structured AI dev. Use when enforcing reviews, managing releases, or running QA audit passes.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${GSTACK_DIR:-$HOME/.gstack}"
...
show_help() {
    cat << EOF
gstack v$VERSION -- Apply Garry Tan's Claude Code setup with 10 tools for structured AI dev. Use when enforcing reviews, managing releases, or running QA audit passes.

Usage: gstack <command> [args]
...
cmd_add() {
    local value="${1:?Usage: gstack add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: gstack search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: gstack remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: gstack help"; exit 1 ;;
esac
