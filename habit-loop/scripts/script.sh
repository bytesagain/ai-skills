#!/usr/bin/env bash
# habit-loop — Habit Loop reference tool. Use when working with habit loop in life contexts.
...
show_help() {
    cat << 'HELPEOF'
habit-loop v$VERSION — Habit Loop Reference Tool

Usage: habit-loop <command>
...
## What is Habit Loop?
Habit Loop (habit-loop) is a specialized tool/concept in the life domain.
It provides essential capabilities for professionals working with habit loop.

## Key Concepts
- Core habit loop principles and fundamentals
- How habit loop fits into the broader life ecosystem  
- Essential terminology every practitioner should know

## Why Habit Loop Matters
Understanding habit loop is critical for:
...
## Getting Started
1. Understand the basic habit loop concepts
...
cmd_guide() {
    cat << 'EOF'
# Habit Loop — Step-by-Step Guide

## Overview
This guide walks you through the essential habit loop workflows.
...
cmd_planning() {
    cat << 'EOF'
# Habit Loop — Planning & Preparation
...
cmd_resources() {
    cat << 'EOF'
# Habit Loop — Recommended Resources
...
cmd_mistakes() {
    cat << 'EOF'
# Habit Loop — Common Mistakes to Avoid
...
cmd_examples() {
    cat << 'EOF'
# Habit Loop — Real-World Examples

## Example 1: Basic Setup
A typical habit loop setup for a small team:
- Standard configuration with defaults
- Basic monitoring enabled
- Manual backup schedule

## Example 2: Production Deployment
An enterprise habit loop deployment:
...
cmd_faq() {
    cat << 'EOF'
# Habit Loop — Frequently Asked Questions

## General
**Q: What is Habit Loop?**
A: Habit Loop is a reference tool for habit loop in the life domain.

**Q: Who should use this?**
A: Anyone working with habit loop who needs quick reference material.
...
case "$CMD" in
    ...
    version|--version|-v) echo "habit-loop v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: habit-loop help"; exit 1 ;;
esac
