#!/usr/bin/env bash
# hash -- Generate MD5 and SHA checksums, verify integrity, and compare hash values. Use when computing checksums, verifying downloads, or comparing hash outputs.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${HASH_DIR:-$HOME/.hash}"
...
show_help() {
    cat << EOF
hash v$VERSION -- Generate MD5 and SHA checksums, verify integrity, and compare hash values. Use when computing checksums, verifying downloads, or comparing hash outputs.

Usage: hash <command> [args]
...
cmd_add() {
    local value="${1:?Usage: hash add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: hash remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "hash v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: hash help"; exit 1 ;;
esac
