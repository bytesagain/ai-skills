#!/usr/bin/env bash
# headline -- Create and optimize marketing headlines using scoring and A/B testing tools. Use when writing blog titles, ad copy, or comparing headline variations.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${HEADLINE_DIR:-$HOME/.headline}"
...
show_help() {
    cat << EOF
headline v$VERSION -- Create and optimize marketing headlines using scoring and A/B testing tools. Use when writing blog titles, ad copy, or comparing headline variations.

Usage: headline <command> [args]
...
cmd_add() {
    local value="${1:?Usage: headline add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: headline search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: headline remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "headline v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: headline help"; exit 1 ;;
esac
