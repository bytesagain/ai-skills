#!/usr/bin/env bash
# heartrate -- Log heart rate readings, track BPM trends, and set cardiovascular goals. Use when recording heart rate, tracking trends, or charting weekly averages.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${HEARTRATE_DIR:-$HOME/.heartrate}"
...
show_help() {
    cat << EOF
heartrate v$VERSION -- Log heart rate readings, track BPM trends, and set cardiovascular goals. Use when recording heart rate, tracking trends, or charting weekly averages.

Usage: heartrate <command> [args]
...
cmd_add() {
    local value="${1:?Usage: heartrate add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: heartrate search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: heartrate remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="heartrate-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "heartrate v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: heartrate help"; exit 1 ;;
esac
