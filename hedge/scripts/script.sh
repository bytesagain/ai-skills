...
show_help() {
    cat << EOF
hedge v$VERSION -- hedge tool

Usage: hedge <command> [args]
...
cmd_add() {
    local value="${1:?Usage: hedge add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: hedge search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: hedge remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "hedge v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: hedge help"; exit 1 ;;
esac
