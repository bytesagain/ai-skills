#!/usr/bin/env bash
# helm -- Create, lint, template, and package Kubernetes Helm charts with checks. Use when scaffolding charts, linting templates, or packaging chart releases.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${HELM_DIR:-$HOME/.helm}"
...
show_help() {
    cat << EOF
helm v$VERSION -- Create, lint, template, and package Kubernetes Helm charts with checks. Use when scaffolding charts, linting templates, or packaging chart releases.
...
cmd_add() {
    local value="${1:?Usage: helm add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: helm search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: helm remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "helm v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: helm help"; exit 1 ;;
esac
