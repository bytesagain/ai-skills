#!/usr/bin/env bash
# herbicide -- Run herbicide operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${HERBICIDE_DIR:-$HOME/.herbicide}"
...
show_help() {
    cat << EOF
herbicide v$VERSION -- Run herbicide operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: herbicide <command> [args]
...
cmd_add() {
    local value="${1:?Usage: herbicide add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: herbicide search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: herbicide remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="herbicide-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "herbicide v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: herbicide help"; exit 1 ;;
esac
