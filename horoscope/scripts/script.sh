#!/usr/bin/env bash
# horoscope - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${HOROSCOPE_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/horoscope}"
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "horoscope v$VERSION" ;;
