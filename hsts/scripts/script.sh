#!/usr/bin/env bash
# hsts — Hsts reference tool. Use when working with hsts in security contexts.
...
show_help() {
    cat << 'HELPEOF'
hsts v$VERSION — Hsts Reference Tool
...
## What is Hsts?
Hsts (hsts) is a specialized tool/concept in the security domain.
It provides essential capabilities for professionals working with hsts.

## Key Concepts
- Core hsts principles and fundamentals
- How hsts fits into the broader security ecosystem  
- Essential terminology every practitioner should know

## Why Hsts Matters
Understanding hsts is critical for:
...
## Getting Started
1. Understand the basic hsts concepts
...
## Installation
1. Download or clone the hsts package
...
cmd_patterns() {
    cat << 'EOF'
# Hsts — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for hsts
...
case "$CMD" in
    ...
    version|--version|-v) echo "hsts v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: hsts help"; exit 1 ;;
esac
