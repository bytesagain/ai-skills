#!/usr/bin/env bash
# html-builder - Design reference and helper tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${HTML_BUILDER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/html-builder}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
html-builder v$VERSION

Design reference and helper tool

Usage: html-builder <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "html-builder v$VERSION" ;;
