#!/usr/bin/env bash
# htmx — Htmx reference tool. Use when working with htmx in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
htmx v$VERSION — Htmx Reference Tool
...
## What is Htmx?
Htmx (htmx) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with htmx.

## Key Concepts
- Core htmx principles and fundamentals
- How htmx fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Htmx Matters
Understanding htmx is critical for:
...
## Getting Started
1. Understand the basic htmx concepts
...
## Installation
1. Download or clone the htmx package
...
cmd_patterns() {
    cat << 'EOF'
# Htmx — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for htmx
...
case "$CMD" in
    ...
    version|--version|-v) echo "htmx v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: htmx help"; exit 1 ;;
esac
