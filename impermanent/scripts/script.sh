#!/usr/bin/env bash
# impermanent -- Analyze impermanent operations. Use when you need to understand impermanent mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${IMPERMANENT_DIR:-$HOME/.impermanent}"
...
show_help() {
    cat << EOF
impermanent v$VERSION -- Analyze impermanent operations. Use when you need to understand impermanent mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: impermanent <command> [args]
...
cmd_add() {
    local value="${1:?Usage: impermanent add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: impermanent search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: impermanent remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="impermanent-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "impermanent v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: impermanent help"; exit 1 ;;
esac
