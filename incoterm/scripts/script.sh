#!/usr/bin/env bash
# incoterm -- Run incoterm operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${INCOTERM_DIR:-$HOME/.incoterm}"
...
show_help() {
    cat << EOF
incoterm v$VERSION -- Run incoterm operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: incoterm <command> [args]
...
cmd_add() {
    local value="${1:?Usage: incoterm add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: incoterm search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: incoterm remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "incoterm v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: incoterm help"; exit 1 ;;
esac
