#!/usr/bin/env bash
# indemnity -- Run indemnity operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${INDEMNITY_DIR:-$HOME/.indemnity}"
...
show_help() {
    cat << EOF
indemnity v$VERSION -- Run indemnity operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: indemnity <command> [args]
...
cmd_add() {
    local value="${1:?Usage: indemnity add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: indemnity search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: indemnity remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="indemnity-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "indemnity v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: indemnity help"; exit 1 ;;
esac
