#!/usr/bin/env bash
# injunction -- Run injunction operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${INJUNCTION_DIR:-$HOME/.injunction}"
...
show_help() {
    cat << EOF
injunction v$VERSION -- Run injunction operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: injunction <command> [args]
...
cmd_add() {
    local value="${1:?Usage: injunction add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: injunction search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: injunction remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="injunction-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "injunction v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: injunction help"; exit 1 ;;
esac
