#!/usr/bin/env bash
# ink — Ink reference tool. Use when working with ink in blockchain contexts.
...
show_help() {
    cat << 'HELPEOF'
ink v$VERSION — Ink Reference Tool
...
## What is Ink?
Ink (ink) is a specialized tool/concept in the blockchain domain.
It provides essential capabilities for professionals working with ink.

## Key Concepts
- Core ink principles and fundamentals
- How ink fits into the broader blockchain ecosystem  
- Essential terminology every practitioner should know

## Why Ink Matters
Understanding ink is critical for:
...
## Getting Started
1. Understand the basic ink concepts
...
cmd_instruments() {
    cat << 'EOF'
# Ink — Instruments & Tools Overview

## Primary Instruments
- Core tools used in ink operations
...
cmd_glossary() {
    cat << 'EOF'
# Ink — Key Terms & Definitions

## Core Terminology
- **Ink**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "ink v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: ink help"; exit 1 ;;
esac
