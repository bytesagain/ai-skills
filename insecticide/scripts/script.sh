#!/usr/bin/env bash
# insecticide -- Run insecticide operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${INSECTICIDE_DIR:-$HOME/.insecticide}"
...
show_help() {
    cat << EOF
insecticide v$VERSION -- Run insecticide operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: insecticide <command> [args]
...
cmd_add() {
    local value="${1:?Usage: insecticide add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: insecticide search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: insecticide remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="insecticide-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "insecticide v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: insecticide help"; exit 1 ;;
esac
