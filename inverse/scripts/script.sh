...
VERSION="1.0.0"
DATA_DIR="${INVERSE_DIR:-$HOME/.inverse}"
...
show_help() {
    cat << EOF
inverse v$VERSION -- Compute inverse operations. Use when you need to solve inverse equations, run statistical analysis, or simulate numerical models.

Usage: inverse <command> [args]
...
cmd_add() {
    local value="${1:?Usage: inverse add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: inverse search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_calculate() {
    local expr="${*:?Usage: inverse calculate <expression>}"
...
cmd_remove() {
    local num="${1:?Usage: inverse remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
