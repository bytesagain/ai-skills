#!/usr/bin/env bash
# invoice -- Create, manage, and track invoices with line items, tax calculations, and payment status using bash and Python. Use when generating invoices,
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${INVOICE_DIR:-$HOME/.invoice}"
...
show_help() {
    cat << EOF
invoice v$VERSION -- Create, manage, and track invoices with line items, tax calculations, and payment status using bash and Python. Use when generating invoices,

Usage: invoice <command> [args]
...
cmd_add() {
    local value="${1:?Usage: invoice add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: invoice search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: invoice remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "invoice v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: invoice help"; exit 1 ;;
esac
