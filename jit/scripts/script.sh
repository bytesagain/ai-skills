#!/usr/bin/env bash
# jit -- Just-in-time inventory optimizer
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${JIT_DIR:-$HOME/.jit}"
...
show_help() {
    cat << EOF
jit v$VERSION -- Just-in-time inventory optimizer
...
cmd_add() {
    local value="${1:?Usage: jit add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: jit search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: jit remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: jit help"; exit 1 ;;
esac
