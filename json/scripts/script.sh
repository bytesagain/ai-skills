#!/usr/bin/env bash
# json -- Handle json format data for parsing, conversion, and analysis. Use when working with data pipelines or format migration.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${JSON_DIR:-$HOME/.json}"
...
show_help() {
    cat << EOF
json v$VERSION -- Handle json format data for parsing, conversion, and analysis. Use when working with data pipelines or format migration.
...
cmd_add() {
    local value="${1:?Usage: json add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: json search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: json remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "json v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: json help"; exit 1 ;;
esac
