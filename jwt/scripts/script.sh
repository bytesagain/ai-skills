#!/usr/bin/env bash
# jwt — Jwt reference tool. Use when working with jwt in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
jwt v$VERSION — Jwt Reference Tool
...
## What is Jwt?
Jwt (jwt) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with jwt.

## Key Concepts
- Core jwt principles and fundamentals
- How jwt fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Jwt Matters
Understanding jwt is critical for:
...
## Getting Started
1. Understand the basic jwt concepts
...
## Installation
1. Download or clone the jwt package
...
cmd_patterns() {
    cat << 'EOF'
# Jwt — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for jwt
...
case "$CMD" in
    ...
    version|--version|-v) echo "jwt v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: jwt help"; exit 1 ;;
esac
