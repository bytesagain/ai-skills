#!/usr/bin/env bash
# kafka-connect — Kafka Connect reference tool. Use when working with kafka connect in data contexts.
...
show_help() {
    cat << 'HELPEOF'
kafka-connect v$VERSION — Kafka Connect Reference Tool

Usage: kafka-connect <command>
...
Kafka Connect (kafka-connect) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with kafka connect.

## Key Concepts
- Core kafka connect principles and fundamentals
- How kafka connect fits into the broader data ecosystem  
- Essential terminology every practitioner should know

## Why Kafka Connect Matters
Understanding kafka connect is critical for:
...
## Getting Started
1. Understand the basic kafka connect concepts
...
## Installation
1. Download or clone the kafka connect package
...
cmd_patterns() {
    cat << 'EOF'
# Kafka Connect — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for kafka connect
...
cmd_security() {
    cat << 'EOF'
# Kafka Connect — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Kafka Connect — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "kafka-connect v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: kafka-connect help"; exit 1 ;;
esac
