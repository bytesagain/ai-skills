#!/usr/bin/env bash
# kafka -- Produce, consume, and manage Kafka topics with lag monitoring and data export. Use when publishing messages, consuming topics, monitoring consumer lag.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${KAFKA_DIR:-$HOME/.kafka}"
...
show_help() {
    cat << EOF
kafka v$VERSION -- Produce, consume, and manage Kafka topics with lag monitoring and data export. Use when publishing messages, consuming topics, monitoring consumer lag.

Usage: kafka <command> [args]
...
cmd_add() {
    local value="${1:?Usage: kafka add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: kafka search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: kafka remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "kafka v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: kafka help"; exit 1 ;;
esac
