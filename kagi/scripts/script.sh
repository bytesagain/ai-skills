#!/usr/bin/env bash
# kagi — Kagi reference tool. Use when working with kagi in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
kagi v$VERSION — Kagi Reference Tool
...
## What is Kagi?
Kagi (kagi) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with kagi.

## Key Concepts
- Core kagi principles and fundamentals
- How kagi fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Kagi Matters
Understanding kagi is critical for:
...
## Getting Started
1. Understand the basic kagi concepts
...
cmd_formulas() {
    cat << 'EOF'
# Kagi — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Kagi — Instruments & Tools Overview

## Primary Instruments
- Core tools used in kagi operations
...
cmd_glossary() {
    cat << 'EOF'
# Kagi — Key Terms & Definitions

## Core Terminology
- **Kagi**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "kagi v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: kagi help"; exit 1 ;;
esac
