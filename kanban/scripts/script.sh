#!/usr/bin/env bash
# kanban -- Kanban board and workflow manager
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${KANBAN_DIR:-$HOME/.kanban}"
...
show_help() {
    cat << EOF
kanban v$VERSION -- Kanban board and workflow manager
...
cmd_add() {
    local value="${1:?Usage: kanban add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: kanban search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: kanban remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
