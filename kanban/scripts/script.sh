#!/usr/bin/env bash
# kanban — Kanban reference tool. Use when working with kanban in life contexts.
...
show_help() {
    cat << 'HELPEOF'
kanban v$VERSION — Kanban Reference Tool
...
## What is Kanban?
Kanban (kanban) is a specialized tool/concept in the life domain.
It provides essential capabilities for professionals working with kanban.

## Key Concepts
- Core kanban principles and fundamentals
- How kanban fits into the broader life ecosystem  
- Essential terminology every practitioner should know

## Why Kanban Matters
Understanding kanban is critical for:
...
## Getting Started
1. Understand the basic kanban concepts
...
cmd_guide() {
    cat << 'EOF'
# Kanban — Step-by-Step Guide

## Overview
This guide walks you through the essential kanban workflows.
...
cmd_mistakes() {
    cat << 'EOF'
# Kanban — Common Mistakes to Avoid
...
cmd_examples() {
    cat << 'EOF'
# Kanban — Real-World Examples

## Example 1: Basic Setup
A typical kanban setup for a small team:
- Standard configuration with defaults
- Basic monitoring enabled
- Manual backup schedule

## Example 2: Production Deployment
An enterprise kanban deployment:
...
## General
**Q: What is Kanban?**
A: Kanban is a reference tool for kanban in the life domain.

**Q: Who should use this?**
A: Anyone working with kanban who needs quick reference material.
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: kanban help"; exit 1 ;;
esac
