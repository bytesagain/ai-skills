#!/usr/bin/env bash
# keyset -- Integrate keyset operations. Use when you need to configure keyset endpoints, manage API connections, or handle service webhooks.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${KEYSET_DIR:-$HOME/.keyset}"
...
show_help() {
    cat << EOF
keyset v$VERSION -- Integrate keyset operations. Use when you need to configure keyset endpoints, manage API connections, or handle service webhooks.

Usage: keyset <command> [args]
...
cmd_add() {
    local value="${1:?Usage: keyset add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: keyset search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: keyset remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "keyset v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: keyset help"; exit 1 ;;
esac
