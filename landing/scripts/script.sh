...
_version() { echo "landing v2.0.0"; }

_help() {
    echo "Landing v2.0.0 — social toolkit"
    echo ""
    echo "Usage: landing <command> [args]"
    echo ""
    echo "Commands:"
...
_search() {
    local term="${1:?Usage: landing search <term>}"
...
case "${1:-help}" in
    connect)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/connect.log" 2>/dev/null || echo "  No entries yet. Use: landing connect <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/connect.log"
            local total=$(wc -l < "$DATA_DIR/connect.log")
            echo "  [Landing] connect: $input"
    ...
    sync)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/sync.log" 2>/dev/null || echo "  No entries yet. Use: landing sync <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/sync.log"
            local total=$(wc -l < "$DATA_DIR/sync.log")
            echo "  [Landing] sync: $input"
    ...
    monitor)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/monitor.log" 2>/dev/null || echo "  No entries yet. Use: landing monitor <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/monitor.log"
            local total=$(wc -l < "$DATA_DIR/monitor.log")
            echo "  [Landing] monitor: $input"
    ...
    automate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/automate.log" 2>/dev/null || echo "  No entries yet. Use: landing automate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/automate.log"
            local total=$(wc -l < "$DATA_DIR/automate.log")
            echo "  [Landing] automate: $input"
    ...
    notify)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/notify.log" 2>/dev/null || echo "  No entries yet. Use: landing notify <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/notify.log"
            local total=$(wc -l < "$DATA_DIR/notify.log")
            echo "  [Landing] notify: $input"
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: landing report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [Landing] report: $input"
    ...
    schedule)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/schedule.log" 2>/dev/null || echo "  No entries yet. Use: landing schedule <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/schedule.log"
            local total=$(wc -l < "$DATA_DIR/schedule.log")
            echo "  [Landing] schedule: $input"
    ...
    template)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/template.log" 2>/dev/null || echo "  No entries yet. Use: landing template <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/template.log"
            local total=$(wc -l < "$DATA_DIR/template.log")
            echo "  [Landing] template: $input"
    ...
    webhook)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/webhook.log" 2>/dev/null || echo "  No entries yet. Use: landing webhook <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/webhook.log"
            local total=$(wc -l < "$DATA_DIR/webhook.log")
            echo "  [Landing] webhook: $input"
    ...
    status)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/status.log" 2>/dev/null || echo "  No entries yet. Use: landing status <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/status.log"
            local total=$(wc -l < "$DATA_DIR/status.log")
            echo "  [Landing] status: $input"
    ...
    analytics)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/analytics.log" 2>/dev/null || echo "  No entries yet. Use: landing analytics <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/analytics.log"
            local total=$(wc -l < "$DATA_DIR/analytics.log")
            echo "  [Landing] analytics: $input"
    ...
    export)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/export.log" 2>/dev/null || echo "  No entries yet. Use: landing export <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/export.log"
            local total=$(wc -l < "$DATA_DIR/export.log")
            echo "  [Landing] export: $input"
    ...
    *)
        ...
        echo "Run 'landing help' for available commands."
        exit 1
        ;;
esac