#!/usr/bin/env bash
# laundry -- Track laundry loads, view usage stats, and export reports in multiple formats. Use when logging wash cycles, reviewing patterns, exporting schedules.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${LAUNDRY_DIR:-$HOME/.laundry}"
...
show_help() {
    cat << EOF
laundry v$VERSION -- Track laundry loads, view usage stats, and export reports in multiple formats. Use when logging wash cycles, reviewing patterns, exporting schedules.
...
cmd_add() {
    local value="${1:?Usage: laundry add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: laundry search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: laundry remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "laundry v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: laundry help"; exit 1 ;;
esac
