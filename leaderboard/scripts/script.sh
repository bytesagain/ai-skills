#!/usr/bin/env bash
# leaderboard -- Record scores, rank players, and analyze game stats with terminal leaderboards. Use when tracking scores, ranking competitors, reviewing performance.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${LEADERBOARD_DIR:-$HOME/.leaderboard}"
...
show_help() {
    cat << EOF
leaderboard v$VERSION -- Record scores, rank players, and analyze game stats with terminal leaderboards. Use when tracking scores, ranking competitors, reviewing performance.

Usage: leaderboard <command> [args]
...
cmd_add() {
    local value="${1:?Usage: leaderboard add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: leaderboard remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="leaderboard-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "leaderboard v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: leaderboard help"; exit 1 ;;
esac
