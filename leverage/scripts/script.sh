...
VERSION="1.0.0"
DATA_DIR="${LEVERAGE_DIR:-$HOME/.leverage}"
...
show_help() {
    cat << EOF
leverage v$VERSION -- leverage tool

Usage: leverage <command> [args]
...
cmd_add() {
    local value="${1:?Usage: leverage add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: leverage search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: leverage remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "leverage v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: leverage help"; exit 1 ;;
esac
