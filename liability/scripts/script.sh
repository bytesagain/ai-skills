#!/usr/bin/env bash
# liability -- Run liability operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${LIABILITY_DIR:-$HOME/.liability}"
...
show_help() {
    cat << EOF
liability v$VERSION -- Run liability operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: liability <command> [args]
...
cmd_add() {
    local value="${1:?Usage: liability add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: liability search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: liability remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="liability-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "liability v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: liability help"; exit 1 ;;
esac
