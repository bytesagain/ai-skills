#!/usr/bin/env bash
# lidar -- LiDAR point cloud data processor
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${LIDAR_DIR:-$HOME/.lidar}"
...
show_help() {
    cat << EOF
lidar v$VERSION -- LiDAR point cloud data processor

Usage: lidar <command> [args]
...
cmd_add() {
    local value="${1:?Usage: lidar add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: lidar search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: lidar remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: lidar help"; exit 1 ;;
esac
