...
VERSION="1.0.0"
DATA_DIR="${LIGHTBOX_DIR:-$HOME/.lightbox}"
...
show_help() {
    cat << EOF
lightbox v$VERSION -- lightbox tool
...
cmd_add() {
    local value="${1:?Usage: lightbox add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: lightbox search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: lightbox remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "lightbox v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: lightbox help"; exit 1 ;;
esac
