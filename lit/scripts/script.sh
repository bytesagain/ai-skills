#!/usr/bin/env bash
# lit — Lit reference tool. Use when working with lit in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
lit v$VERSION — Lit Reference Tool
...
## What is Lit?
Lit (lit) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with lit.

## Key Concepts
- Core lit principles and fundamentals
- How lit fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Lit Matters
Understanding lit is critical for:
...
## Getting Started
1. Understand the basic lit concepts
...
## Installation
1. Download or clone the lit package
...
cmd_patterns() {
    cat << 'EOF'
# Lit — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for lit
...
case "$CMD" in
    ...
    version|--version|-v) echo "lit v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: lit help"; exit 1 ;;
esac
