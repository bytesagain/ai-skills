#!/usr/bin/env bash
# load-balancer -- Manage load balancer configurations and server pools using CLI tools. Use when you need to create, test, or rotate backend server pools.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${LOAD_BALANCER_DIR:-$HOME/.load-balancer}"
...
show_help() {
    cat << EOF
load-balancer v$VERSION -- Manage load balancer configurations and server pools using CLI tools. Use when you need to create, test, or rotate backend server pools.

Usage: load-balancer <command> [args]
...
cmd_add() {
    local value="${1:?Usage: load-balancer add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: load-balancer search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: load-balancer remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="load-balancer-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "load-balancer v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: load-balancer help"; exit 1 ;;
esac
