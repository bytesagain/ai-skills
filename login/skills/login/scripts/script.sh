#!/usr/bin/env bash
# login -- Store, retrieve, and manage login credentials securely using bash and Python with local encryption. Use when saving website logins, API keys,
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${LOGIN_DIR:-$HOME/.login}"
...
show_help() {
    cat << EOF
login v$VERSION -- Store, retrieve, and manage login credentials securely using bash and Python with local encryption. Use when saving website logins, API keys,

Usage: login <command> [args]
...
cmd_add() {
    local value="${1:?Usage: login add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: login search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: login remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "login v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: login help"; exit 1 ;;
esac
