#!/usr/bin/env bash
# looker — Looker reference tool. Use when working with looker in data contexts.
...
show_help() {
    cat << 'HELPEOF'
looker v$VERSION — Looker Reference Tool

Usage: looker <command>
...
## What is Looker?
Looker (looker) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with looker.

## Key Concepts
- Core looker principles and fundamentals
- How looker fits into the broader data ecosystem  
- Essential terminology every practitioner should know

## Why Looker Matters
Understanding looker is critical for:
...
## Getting Started
1. Understand the basic looker concepts
...
## Installation
1. Download or clone the looker package
...
cmd_patterns() {
    cat << 'EOF'
# Looker — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for looker
...
cmd_security() {
    cat << 'EOF'
# Looker — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Looker — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "looker v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: looker help"; exit 1 ;;
esac
