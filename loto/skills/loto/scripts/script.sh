#!/usr/bin/env bash
# loto -- Lockout-tagout procedure generator
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${LOTO_DIR:-$HOME/.loto}"
...
show_help() {
    cat << EOF
loto v$VERSION -- Lockout-tagout procedure generator
...
cmd_add() {
    local value="${1:?Usage: loto add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: loto search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: loto remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "loto v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: loto help"; exit 1 ;;
esac
