#!/usr/bin/env bash
# lottery -- Generate lottery numbers and check results. Use when picking numbers for draws.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${LOTTERY_DIR:-$HOME/.lottery}"
...
show_help() {
    cat << EOF
lottery v$VERSION -- Generate lottery numbers and check results. Use when picking numbers for draws.
...
cmd_add() {
    local value="${1:?Usage: lottery add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: lottery remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "lottery v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: lottery help"; exit 1 ;;
esac
