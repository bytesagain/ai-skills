#!/usr/bin/env bash
# luigi — Luigi reference tool. Use when working with luigi in data contexts.
...
show_help() {
    cat << 'HELPEOF'
luigi v$VERSION — Luigi Reference Tool
...
## What is Luigi?
Luigi (luigi) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with luigi.

## Key Concepts
- Core luigi principles and fundamentals
- How luigi fits into the broader data ecosystem  
- Essential terminology every practitioner should know

## Why Luigi Matters
Understanding luigi is critical for:
...
## Getting Started
1. Understand the basic luigi concepts
...
## Installation
1. Download or clone the luigi package
...
cmd_patterns() {
    cat << 'EOF'
# Luigi — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for luigi
...
cmd_performance() {
    cat << 'EOF'
# Luigi — Performance Optimization
...
case "$CMD" in
    ...
    version|--version|-v) echo "luigi v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: luigi help"; exit 1 ;;
esac
