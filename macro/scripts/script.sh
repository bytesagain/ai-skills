#!/usr/bin/env bash
# macro -- Track macro personal activities and daily routines. Use when managing health goals, organizing tasks, or building habits.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${MACRO_DIR:-$HOME/.macro}"
...
show_help() {
    cat << EOF
macro v$VERSION -- Track macro personal activities and daily routines. Use when managing health goals, organizing tasks, or building habits.

Usage: macro <command> [args]
...
cmd_add() {
    local value="${1:?Usage: macro add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: macro search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: macro remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "macro v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: macro help"; exit 1 ;;
esac
