#!/usr/bin/env bash
# mantine — Mantine reference tool. Use when working with mantine in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
mantine v$VERSION — Mantine Reference Tool

Usage: mantine <command>
...
## What is Mantine?
Mantine (mantine) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with mantine.

## Key Concepts
- Core mantine principles and fundamentals
- How mantine fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Mantine Matters
Understanding mantine is critical for:
...
## Getting Started
1. Understand the basic mantine concepts
...
## Installation
1. Download or clone the mantine package
...
cmd_patterns() {
    cat << 'EOF'
# Mantine — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for mantine
...
cmd_performance() {
    cat << 'EOF'
# Mantine — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Mantine — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Mantine — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "mantine v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: mantine help"; exit 1 ;;
esac
