#!/usr/bin/env bash
# margin -- Calculate margin financial metrics and business data. Use when tracking expenses, analyzing investments, or generating financial reports.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${MARGIN_DIR:-$HOME/.margin}"
...
show_help() {
    cat << EOF
margin v$VERSION -- Calculate margin financial metrics and business data. Use when tracking expenses, analyzing investments, or generating financial reports.

Usage: margin <command> [args]
...
cmd_add() {
    local value="${1:?Usage: margin add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: margin search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: margin remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "margin v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: margin help"; exit 1 ;;
esac
