#!/usr/bin/env bash
# maturity -- Calculate maturity financial metrics and business data. Use when tracking expenses, analyzing investments, or generating reports.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${MATURITY_DIR:-$HOME/.maturity}"
...
show_help() {
    cat << EOF
maturity v$VERSION -- Calculate maturity financial metrics and business data. Use when tracking expenses, analyzing investments, or generating reports.

Usage: maturity <command> [args]
...
cmd_add() {
    local value="${1:?Usage: maturity add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: maturity search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: maturity remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "maturity v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: maturity help"; exit 1 ;;
esac
