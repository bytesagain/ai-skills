#!/usr/bin/env bash
# meal -- Track meal personal activities and daily routines. Use when managing health goals, organizing tasks, or building productive habits.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${MEAL_DIR:-$HOME/.meal}"
...
show_help() {
    cat << EOF
meal v$VERSION -- Track meal personal activities and daily routines. Use when managing health goals, organizing tasks, or building productive habits.

Usage: meal <command> [args]
...
cmd_add() {
    local value="${1:?Usage: meal add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: meal search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: meal remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "meal v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: meal help"; exit 1 ;;
esac
