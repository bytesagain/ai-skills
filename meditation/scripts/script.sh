#!/usr/bin/env bash
# meditation - Health and wellness tracker
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${MEDITATION_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/meditation}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
meditation v$VERSION

Health and wellness tracker

Usage: meditation <command> [args]
