#!/usr/bin/env bash
# merge -- Combine files, resolve conflicts, concatenate data, and deduplicate across sources. Use when merging files, resolving conflicts, deduplicating datasets.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${MERGE_DIR:-$HOME/.merge}"
...
show_help() {
    cat << EOF
merge v$VERSION -- Combine files, resolve conflicts, concatenate data, and deduplicate across sources. Use when merging files, resolving conflicts, deduplicating datasets.
...
cmd_add() {
    local value="${1:?Usage: merge add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: merge remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "merge v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: merge help"; exit 1 ;;
esac
