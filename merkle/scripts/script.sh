#!/usr/bin/env bash
# merkle -- Analyze merkle operations. Use when you need to understand merkle mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${MERKLE_DIR:-$HOME/.merkle}"
...
show_help() {
    cat << EOF
merkle v$VERSION -- Analyze merkle operations. Use when you need to understand merkle mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: merkle <command> [args]
...
cmd_add() {
    local value="${1:?Usage: merkle add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: merkle search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: merkle remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "merkle v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: merkle help"; exit 1 ;;
esac
