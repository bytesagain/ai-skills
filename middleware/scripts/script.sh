#!/usr/bin/env bash
# middleware — Middleware reference tool. Use when working with middleware in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
middleware v$VERSION — Middleware Reference Tool

Usage: middleware <command>
...
## What is Middleware?
Middleware (middleware) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with middleware.

## Key Concepts
- Core middleware principles and fundamentals
- How middleware fits into the broader devtools ecosystem  
...
## Getting Started
1. Understand the basic middleware concepts
...
## Installation
1. Download or clone the middleware package
...
cmd_patterns() {
    cat << 'EOF'
# Middleware — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for middleware
...
cmd_migration() {
    cat << 'EOF'
# Middleware — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: middleware help"; exit 1 ;;
esac
