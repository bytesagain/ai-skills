#!/usr/bin/env bash
# mileage -- Log vehicle mileage, review driving trends, and export trip reports for tracking. Use when logging trips, tracking fuel economy, exporting mileage data.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${MILEAGE_DIR:-$HOME/.mileage}"
...
show_help() {
    cat << EOF
mileage v$VERSION -- Log vehicle mileage, review driving trends, and export trip reports for tracking. Use when logging trips, tracking fuel economy, exporting mileage data.

Usage: mileage <command> [args]
...
cmd_add() {
    local value="${1:?Usage: mileage add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: mileage search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: mileage remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "mileage v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: mileage help"; exit 1 ;;
esac
