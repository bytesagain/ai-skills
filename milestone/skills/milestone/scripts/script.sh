#!/usr/bin/env bash
# milestone -- Track project milestones, log progress, and export completion reports locally. Use when marking milestones, reviewing progress, exporting timeline data.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${MILESTONE_DIR:-$HOME/.milestone}"
...
show_help() {
    cat << EOF
milestone v$VERSION -- Track project milestones, log progress, and export completion reports locally. Use when marking milestones, reviewing progress, exporting timeline data.

Usage: milestone <command> [args]
...
cmd_add() {
    local value="${1:?Usage: milestone add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: milestone search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: milestone remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="milestone-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "milestone v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: milestone help"; exit 1 ;;
esac
