#!/usr/bin/env bash
# milligram — Milligram reference tool. Use when working with milligram in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
milligram v$VERSION — Milligram Reference Tool

Usage: milligram <command>
...
## What is Milligram?
Milligram (milligram) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with milligram.

## Key Concepts
- Core milligram principles and fundamentals
- How milligram fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Milligram Matters
Understanding milligram is critical for:
...
## Getting Started
1. Understand the basic milligram concepts
...
## Installation
1. Download or clone the milligram package
...
cmd_patterns() {
    cat << 'EOF'
# Milligram — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for milligram
...
cmd_performance() {
    cat << 'EOF'
# Milligram — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Milligram — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Milligram — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "milligram v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: milligram help"; exit 1 ;;
esac
