#!/usr/bin/env bash
# mitmproxy — Mitmproxy reference tool. Use when working with mitmproxy in network contexts.
...
show_help() {
    cat << 'HELPEOF'
mitmproxy v$VERSION — Mitmproxy Reference Tool

Usage: mitmproxy <command>
...
## What is Mitmproxy?
Mitmproxy (mitmproxy) is a specialized tool/concept in the network domain.
It provides essential capabilities for professionals working with mitmproxy.

## Key Concepts
- Core mitmproxy principles and fundamentals
- How mitmproxy fits into the broader network ecosystem  
- Essential terminology every practitioner should know

## Why Mitmproxy Matters
Understanding mitmproxy is critical for:
...
## Getting Started
1. Understand the basic mitmproxy concepts
...
## Installation
1. Download or clone the mitmproxy package
...
cmd_patterns() {
    cat << 'EOF'
# Mitmproxy — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for mitmproxy
...
cmd_performance() {
    cat << 'EOF'
# Mitmproxy — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Mitmproxy — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Mitmproxy — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "mitmproxy v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: mitmproxy help"; exit 1 ;;
esac
