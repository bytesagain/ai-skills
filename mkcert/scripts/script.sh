#!/usr/bin/env bash
# mkcert — Mkcert reference tool. Use when working with mkcert in security contexts.
...
show_help() {
    cat << 'HELPEOF'
mkcert v$VERSION — Mkcert Reference Tool

Usage: mkcert <command>
...
## What is Mkcert?
Mkcert (mkcert) is a specialized tool/concept in the security domain.
It provides essential capabilities for professionals working with mkcert.

## Key Concepts
- Core mkcert principles and fundamentals
- How mkcert fits into the broader security ecosystem  
- Essential terminology every practitioner should know

## Why Mkcert Matters
Understanding mkcert is critical for:
...
## Getting Started
1. Understand the basic mkcert concepts
...
## Installation
1. Download or clone the mkcert package
...
cmd_patterns() {
    cat << 'EOF'
# Mkcert — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for mkcert
...
cmd_performance() {
    cat << 'EOF'
# Mkcert — Performance Optimization
...
cmd_migration() {
    cat << 'EOF'
# Mkcert — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "mkcert v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: mkcert help"; exit 1 ;;
esac
