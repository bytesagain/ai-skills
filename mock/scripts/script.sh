#!/usr/bin/env bash
# mock -- Generate mock data and simulate API endpoints using CLI tools. Use when prototyping APIs or testing integrations.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${MOCK_DIR:-$HOME/.mock}"
...
show_help() {
    cat << EOF
mock v$VERSION -- Generate mock data and simulate API endpoints using CLI tools. Use when prototyping APIs or testing integrations.

Usage: mock <command> [args]
...
cmd_add() {
    local value="${1:?Usage: mock add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: mock search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: mock remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "mock v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: mock help"; exit 1 ;;
esac
