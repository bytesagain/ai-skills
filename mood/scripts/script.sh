...
show_help() {
    cat << EOF
mood v$VERSION -- mood tool

Usage: mood <command> [args]
...
cmd_add() {
    local value="${1:?Usage: mood add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: mood search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: mood remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: mood help"; exit 1 ;;
esac
