#!/usr/bin/env bash
# mtr — Mtr reference tool. Use when working with mtr in network contexts.
...
show_help() {
    cat << 'HELPEOF'
mtr v$VERSION — Mtr Reference Tool
...
## What is Mtr?
Mtr (mtr) is a specialized tool/concept in the network domain.
It provides essential capabilities for professionals working with mtr.

## Key Concepts
- Core mtr principles and fundamentals
- How mtr fits into the broader network ecosystem  
- Essential terminology every practitioner should know

## Why Mtr Matters
Understanding mtr is critical for:
...
## Getting Started
1. Understand the basic mtr concepts
...
## Installation
1. Download or clone the mtr package
...
cmd_patterns() {
    cat << 'EOF'
# Mtr — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for mtr
...
case "$CMD" in
    ...
    version|--version|-v) echo "mtr v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: mtr help"; exit 1 ;;
esac
