#!/usr/bin/env bash
# muffler -- Diagnose muffler operations. Use when you need to inspect muffler components, plan maintenance schedules, or troubleshoot vehicle systems.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${MUFFLER_DIR:-$HOME/.muffler}"
...
show_help() {
    cat << EOF
muffler v$VERSION -- Diagnose muffler operations. Use when you need to inspect muffler components, plan maintenance schedules, or troubleshoot vehicle systems.

Usage: muffler <command> [args]
...
cmd_add() {
    local value="${1:?Usage: muffler add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: muffler search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: muffler remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "muffler v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: muffler help"; exit 1 ;;
esac
