#!/usr/bin/env bash
# mulch -- Run mulch operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${MULCH_DIR:-$HOME/.mulch}"
...
show_help() {
    cat << EOF
mulch v$VERSION -- Run mulch operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: mulch <command> [args]
...
cmd_add() {
    local value="${1:?Usage: mulch add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: mulch search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: mulch remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "mulch v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: mulch help"; exit 1 ;;
esac
