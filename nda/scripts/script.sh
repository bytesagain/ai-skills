#!/usr/bin/env bash
# nda -- Run nda operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${NDA_DIR:-$HOME/.nda}"
...
show_help() {
    cat << EOF
nda v$VERSION -- Run nda operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: nda <command> [args]
...
cmd_add() {
    local value="${1:?Usage: nda add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: nda search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: nda remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "nda v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: nda help"; exit 1 ;;
esac
