#!/usr/bin/env bash
# netcat — Netcat reference tool. Use when working with netcat in network contexts.
...
show_help() {
    cat << 'HELPEOF'
netcat v$VERSION — Netcat Reference Tool

Usage: netcat <command>
...
## What is Netcat?
Netcat (netcat) is a specialized tool/concept in the network domain.
It provides essential capabilities for professionals working with netcat.

## Key Concepts
- Core netcat principles and fundamentals
- How netcat fits into the broader network ecosystem  
- Essential terminology every practitioner should know

## Why Netcat Matters
Understanding netcat is critical for:
...
## Getting Started
1. Understand the basic netcat concepts
...
## Installation
1. Download or clone the netcat package
...
cmd_patterns() {
    cat << 'EOF'
# Netcat — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for netcat
...
cmd_performance() {
    cat << 'EOF'
# Netcat — Performance Optimization
...
cmd_migration() {
    cat << 'EOF'
# Netcat — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "netcat v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: netcat help"; exit 1 ;;
esac
