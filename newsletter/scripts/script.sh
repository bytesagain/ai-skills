#!/usr/bin/env bash
# newsletter -- Manage newsletter content, subscribers, and scheduling using CLI tools. Use when you need to draft, preview, and track newsletter campaigns.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${NEWSLETTER_DIR:-$HOME/.newsletter}"
...
show_help() {
    cat << EOF
newsletter v$VERSION -- Manage newsletter content, subscribers, and scheduling using CLI tools. Use when you need to draft, preview, and track newsletter campaigns.

Usage: newsletter <command> [args]
...
cmd_add() {
    local value="${1:?Usage: newsletter add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: newsletter search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: newsletter remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="newsletter-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "newsletter v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: newsletter help"; exit 1 ;;
esac
