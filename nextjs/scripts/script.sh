#!/usr/bin/env bash
# nextjs — Nextjs reference tool. Use when working with nextjs in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
nextjs v$VERSION — Nextjs Reference Tool

Usage: nextjs <command>
...
## What is Nextjs?
Nextjs (nextjs) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with nextjs.

## Key Concepts
- Core nextjs principles and fundamentals
- How nextjs fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Nextjs Matters
Understanding nextjs is critical for:
...
## Getting Started
1. Understand the basic nextjs concepts
...
## Installation
1. Download or clone the nextjs package
...
cmd_patterns() {
    cat << 'EOF'
# Nextjs — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for nextjs
...
cmd_security() {
    cat << 'EOF'
# Nextjs — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Nextjs — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "nextjs v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: nextjs help"; exit 1 ;;
esac
