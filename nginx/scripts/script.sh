#!/usr/bin/env bash
# nginx — Nginx reference tool. Use when working with nginx in network contexts.
...
show_help() {
    cat << 'HELPEOF'
nginx v$VERSION — Nginx Reference Tool
...
## What is Nginx?
Nginx (nginx) is a specialized tool/concept in the network domain.
It provides essential capabilities for professionals working with nginx.

## Key Concepts
- Core nginx principles and fundamentals
- How nginx fits into the broader network ecosystem  
- Essential terminology every practitioner should know

## Why Nginx Matters
Understanding nginx is critical for:
...
## Getting Started
1. Understand the basic nginx concepts
...
## Installation
1. Download or clone the nginx package
...
cmd_patterns() {
    cat << 'EOF'
# Nginx — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for nginx
...
case "$CMD" in
    ...
    version|--version|-v) echo "nginx v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: nginx help"; exit 1 ;;
esac
