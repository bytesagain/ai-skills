#!/usr/bin/env bash
# nonce -- Analyze nonce operations. Use when you need to understand nonce mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${NONCE_DIR:-$HOME/.nonce}"
...
show_help() {
    cat << EOF
nonce v$VERSION -- Analyze nonce operations. Use when you need to understand nonce mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: nonce <command> [args]
...
cmd_add() {
    local value="${1:?Usage: nonce add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: nonce search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: nonce remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "nonce v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: nonce help"; exit 1 ;;
esac
