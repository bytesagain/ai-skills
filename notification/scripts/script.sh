#!/usr/bin/env bash
# notification -- Manage terminal notifications with scheduling, filtering, and delivery tracking. Use when sending alerts, filtering notifications, confirming delivery.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${NOTIFICATION_DIR:-$HOME/.notification}"
...
show_help() {
    cat << EOF
notification v$VERSION -- Manage terminal notifications with scheduling, filtering, and delivery tracking. Use when sending alerts, filtering notifications, confirming delivery.

Usage: notification <command> [args]
...
cmd_add() {
    local value="${1:?Usage: notification add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: notification search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: notification remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="notification-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "notification v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: notification help"; exit 1 ;;
esac
