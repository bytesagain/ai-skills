...
show_help() {
    cat << EOF
novel v$VERSION -- Manage novel data — chapters, characters, plots — from the terminal fast. Use when outlining chapters, tracking characters, organizing plot timelines.

Usage: novel <command> [args]
...
cmd_add() {
    local value="${1:?Usage: novel add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: novel search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: novel remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "novel v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: novel help"; exit 1 ;;
esac
