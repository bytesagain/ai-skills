#!/usr/bin/env bash
# npk -- Run npk operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${NPK_DIR:-$HOME/.npk}"
...
show_help() {
    cat << EOF
npk v$VERSION -- Run npk operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: npk <command> [args]
...
cmd_add() {
    local value="${1:?Usage: npk add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: npk search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: npk remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "npk v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: npk help"; exit 1 ;;
esac
