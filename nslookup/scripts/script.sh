#!/usr/bin/env bash
# nslookup — Nslookup reference tool. Use when working with nslookup in network contexts.
...
show_help() {
    cat << 'HELPEOF'
nslookup v$VERSION — Nslookup Reference Tool

Usage: nslookup <command>
...
## What is Nslookup?
Nslookup (nslookup) is a specialized tool/concept in the network domain.
It provides essential capabilities for professionals working with nslookup.

## Key Concepts
- Core nslookup principles and fundamentals
- How nslookup fits into the broader network ecosystem  
- Essential terminology every practitioner should know

## Why Nslookup Matters
Understanding nslookup is critical for:
...
## Getting Started
1. Understand the basic nslookup concepts
...
## Installation
1. Download or clone the nslookup package
...
cmd_patterns() {
    cat << 'EOF'
# Nslookup — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for nslookup
...
cmd_performance() {
    cat << 'EOF'
# Nslookup — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Nslookup — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Nslookup — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "nslookup v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: nslookup help"; exit 1 ;;
esac
