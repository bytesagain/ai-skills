#!/usr/bin/env bash
# numpy — Numpy reference tool. Use when working with numpy in data contexts.
...
show_help() {
    cat << 'HELPEOF'
numpy v$VERSION — Numpy Reference Tool
...
## What is Numpy?
Numpy (numpy) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with numpy.

## Key Concepts
- Core numpy principles and fundamentals
- How numpy fits into the broader data ecosystem  
...
## Getting Started
1. Understand the basic numpy concepts
...
## Installation
1. Download or clone the numpy package
...
cmd_patterns() {
    cat << 'EOF'
# Numpy — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for numpy
...
case "$CMD" in
    ...
    version|--version|-v) echo "numpy v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: numpy help"; exit 1 ;;
esac
