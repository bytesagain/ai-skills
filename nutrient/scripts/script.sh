#!/usr/bin/env bash
# nutrient -- Track nutrient personal activities and daily routines. Use when managing health goals, organizing tasks, or building habits.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${NUTRIENT_DIR:-$HOME/.nutrient}"
...
show_help() {
    cat << EOF
nutrient v$VERSION -- Track nutrient personal activities and daily routines. Use when managing health goals, organizing tasks, or building habits.

Usage: nutrient <command> [args]
...
cmd_add() {
    local value="${1:?Usage: nutrient add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: nutrient search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: nutrient remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "nutrient v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: nutrient help"; exit 1 ;;
esac
