#!/usr/bin/env bash
# oauth -- Manage OAuth 2.0 flows, tokens, and client configurations using CLI tools. Use when you need to configure, test, or debug OAuth2 authorization workflows.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${OAUTH_DIR:-$HOME/.oauth}"
...
show_help() {
    cat << EOF
oauth v$VERSION -- Manage OAuth 2.0 flows, tokens, and client configurations using CLI tools. Use when you need to configure, test, or debug OAuth2 authorization workflows.
...
cmd_add() {
    local value="${1:?Usage: oauth add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: oauth search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: oauth remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
