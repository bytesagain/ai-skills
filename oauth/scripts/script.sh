#!/usr/bin/env bash
# oauth — Oauth reference tool. Use when working with oauth in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
oauth v$VERSION — Oauth Reference Tool
...
## What is Oauth?
Oauth (oauth) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with oauth.

## Key Concepts
- Core oauth principles and fundamentals
- How oauth fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Oauth Matters
Understanding oauth is critical for:
...
## Getting Started
1. Understand the basic oauth concepts
...
## Installation
1. Download or clone the oauth package
...
cmd_patterns() {
    cat << 'EOF'
# Oauth — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for oauth
...
case "$CMD" in
    ...
    version|--version|-v) echo "oauth v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: oauth help"; exit 1 ;;
esac
