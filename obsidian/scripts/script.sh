#!/usr/bin/env bash
# obsidian — Obsidian reference tool. Use when working with obsidian in life contexts.
...
show_help() {
    cat << 'HELPEOF'
obsidian v$VERSION — Obsidian Reference Tool
...
## What is Obsidian?
Obsidian (obsidian) is a specialized tool/concept in the life domain.
It provides essential capabilities for professionals working with obsidian.

## Key Concepts
- Core obsidian principles and fundamentals
- How obsidian fits into the broader life ecosystem  
- Essential terminology every practitioner should know

## Why Obsidian Matters
Understanding obsidian is critical for:
...
## Getting Started
1. Understand the basic obsidian concepts
...
cmd_guide() {
    cat << 'EOF'
# Obsidian — Step-by-Step Guide

## Overview
This guide walks you through the essential obsidian workflows.
...
cmd_resources() {
    cat << 'EOF'
# Obsidian — Recommended Resources
...
cmd_mistakes() {
    cat << 'EOF'
# Obsidian — Common Mistakes to Avoid
...
cmd_examples() {
    cat << 'EOF'
# Obsidian — Real-World Examples

## Example 1: Basic Setup
A typical obsidian setup for a small team:
- Standard configuration with defaults
- Basic monitoring enabled
- Manual backup schedule

## Example 2: Production Deployment
An enterprise obsidian deployment:
...
## General
**Q: What is Obsidian?**
A: Obsidian is a reference tool for obsidian in the life domain.

**Q: Who should use this?**
A: Anyone working with obsidian who needs quick reference material.
...
case "$CMD" in
    ...
    version|--version|-v) echo "obsidian v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: obsidian help"; exit 1 ;;
esac
