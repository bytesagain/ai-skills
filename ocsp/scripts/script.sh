#!/usr/bin/env bash
# ocsp — Ocsp reference tool. Use when working with ocsp in security contexts.
...
show_help() {
    cat << 'HELPEOF'
ocsp v$VERSION — Ocsp Reference Tool
...
## What is Ocsp?
Ocsp (ocsp) is a specialized tool/concept in the security domain.
It provides essential capabilities for professionals working with ocsp.

## Key Concepts
- Core ocsp principles and fundamentals
- How ocsp fits into the broader security ecosystem  
- Essential terminology every practitioner should know

## Why Ocsp Matters
Understanding ocsp is critical for:
...
## Getting Started
1. Understand the basic ocsp concepts
...
## Installation
1. Download or clone the ocsp package
...
cmd_patterns() {
    cat << 'EOF'
# Ocsp — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for ocsp
...
case "$CMD" in
    ...
    version|--version|-v) echo "ocsp v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: ocsp help"; exit 1 ;;
esac
