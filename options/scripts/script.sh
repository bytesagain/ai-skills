#!/usr/bin/env bash
# options -- Log and review options trades with trend analysis and exportable position reports. Use when tracking trades, reviewing positions, exporting trade summaries.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${OPTIONS_DIR:-$HOME/.options}"
...
show_help() {
    cat << EOF
options v$VERSION -- Log and review options trades with trend analysis and exportable position reports. Use when tracking trades, reviewing positions, exporting trade summaries.
...
cmd_add() {
    local value="${1:?Usage: options add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: options search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: options remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "options v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: options help"; exit 1 ;;
esac
