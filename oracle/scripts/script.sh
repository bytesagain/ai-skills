#!/usr/bin/env bash
# oracle — Oracle reference tool. Use when working with oracle in blockchain contexts.
...
show_help() {
    cat << 'HELPEOF'
oracle v$VERSION — Oracle Reference Tool
...
## What is Oracle?
Oracle (oracle) is a specialized tool/concept in the blockchain domain.
It provides essential capabilities for professionals working with oracle.

## Key Concepts
- Core oracle principles and fundamentals
- How oracle fits into the broader blockchain ecosystem  
- Essential terminology every practitioner should know

## Why Oracle Matters
Understanding oracle is critical for:
...
## Getting Started
1. Understand the basic oracle concepts
...
cmd_formulas() {
    cat << 'EOF'
# Oracle — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Oracle — Instruments & Tools Overview

## Primary Instruments
- Core tools used in oracle operations
...
cmd_glossary() {
    cat << 'EOF'
# Oracle — Key Terms & Definitions

## Core Terminology
- **Oracle**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "oracle v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: oracle help"; exit 1 ;;
esac
