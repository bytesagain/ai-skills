#!/usr/bin/env bash
# oven -- Track oven usage and cooking schedules. Use when logging bake sessions, setting cook reminders, checking inventory, reviewing stats.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${OVEN_DIR:-$HOME/.oven}"
...
show_help() {
    cat << EOF
oven v$VERSION -- Track oven usage and cooking schedules. Use when logging bake sessions, setting cook reminders, checking inventory, reviewing stats.

Usage: oven <command> [args]
...
cmd_add() {
    local value="${1:?Usage: oven add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: oven search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: oven remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "oven v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: oven help"; exit 1 ;;
esac
