...
DATA_DIR="${HOME}/.local/share/package"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "package v2.0.0"; }

_help() {
    echo "Package v2.0.0 — travel toolkit"
    echo ""
    echo "Usage: package <command> [args]"
    echo ""
    echo "Commands:"
    echo "  plan               Plan"
    echo "  search             Search"
...
_search() {
    local term="${1:?Usage: package search <term>}"
...
case "${1:-help}" in
    plan)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/plan.log" 2>/dev/null || echo "  No entries yet. Use: package plan <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/plan.log"
            local total=$(wc -l < "$DATA_DIR/plan.log")
            echo "  [Package] plan: $input"
    ...
    search)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/search.log" 2>/dev/null || echo "  No entries yet. Use: package search <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/search.log"
            local total=$(wc -l < "$DATA_DIR/search.log")
            echo "  [Package] search: $input"
    ...
    book)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/book.log" 2>/dev/null || echo "  No entries yet. Use: package book <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/book.log"
            local total=$(wc -l < "$DATA_DIR/book.log")
            echo "  [Package] book: $input"
    ...
    pack-list)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/pack-list.log" 2>/dev/null || echo "  No entries yet. Use: package pack-list <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/pack-list.log"
            local total=$(wc -l < "$DATA_DIR/pack-list.log")
            echo "  [Package] pack-list: $input"
    ...
    budget)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/budget.log" 2>/dev/null || echo "  No entries yet. Use: package budget <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/budget.log"
            local total=$(wc -l < "$DATA_DIR/budget.log")
            echo "  [Package] budget: $input"
    ...
    convert)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/convert.log" 2>/dev/null || echo "  No entries yet. Use: package convert <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/convert.log"
            local total=$(wc -l < "$DATA_DIR/convert.log")
            echo "  [Package] convert: $input"
    ...
    weather)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/weather.log" 2>/dev/null || echo "  No entries yet. Use: package weather <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/weather.log"
            local total=$(wc -l < "$DATA_DIR/weather.log")
            echo "  [Package] weather: $input"
    ...
    route)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/route.log" 2>/dev/null || echo "  No entries yet. Use: package route <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/route.log"
            local total=$(wc -l < "$DATA_DIR/route.log")
            echo "  [Package] route: $input"
    ...
    checklist)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/checklist.log" 2>/dev/null || echo "  No entries yet. Use: package checklist <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/checklist.log"
            local total=$(wc -l < "$DATA_DIR/checklist.log")
            echo "  [Package] checklist: $input"
    ...
    journal)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/journal.log" 2>/dev/null || echo "  No entries yet. Use: package journal <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/journal.log"
            local total=$(wc -l < "$DATA_DIR/journal.log")
            echo "  [Package] journal: $input"
    ...
    compare)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/compare.log" 2>/dev/null || echo "  No entries yet. Use: package compare <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/compare.log"
            local total=$(wc -l < "$DATA_DIR/compare.log")
            echo "  [Package] compare: $input"
    ...
    remind)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/remind.log" 2>/dev/null || echo "  No entries yet. Use: package remind <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/remind.log"
            local total=$(wc -l < "$DATA_DIR/remind.log")
            echo "  [Package] remind: $input"
    ...
    *)
        echo "Unknown: $1 — run 'package help'"
        exit 1
        ;;
esac