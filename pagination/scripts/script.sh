...
VERSION="1.0.0"
DATA_DIR="${PAGINATION_DIR:-$HOME/.pagination}"
...
show_help() {
    cat << EOF
pagination v$VERSION -- pagination tool

Usage: pagination <command> [args]
...
cmd_add() {
    local value="${1:?Usage: pagination add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: pagination search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: pagination remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="pagination-export.$fmt"
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: pagination help"; exit 1 ;;
esac
