#!/usr/bin/env bash
# palette -- Create and manage color palettes using color theory algorithms. Use when designing UIs or building brand color systems.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PALETTE_DIR:-$HOME/.palette}"
...
show_help() {
    cat << EOF
palette v$VERSION -- Create and manage color palettes using color theory algorithms. Use when designing UIs or building brand color systems.
...
cmd_add() {
    local value="${1:?Usage: palette add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: palette search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: palette remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "palette v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: palette help"; exit 1 ;;
esac
