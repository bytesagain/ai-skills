#!/usr/bin/env bash
# pallet -- Run pallet operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PALLET_DIR:-$HOME/.pallet}"
...
show_help() {
    cat << EOF
pallet v$VERSION -- Run pallet operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: pallet <command> [args]
...
cmd_add() {
    local value="${1:?Usage: pallet add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: pallet search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: pallet remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "pallet v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: pallet help"; exit 1 ;;
esac
