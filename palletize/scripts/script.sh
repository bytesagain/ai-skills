#!/usr/bin/env bash
# palletize -- Run palletize operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PALLETIZE_DIR:-$HOME/.palletize}"
...
show_help() {
    cat << EOF
palletize v$VERSION -- Run palletize operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: palletize <command> [args]
...
cmd_add() {
    local value="${1:?Usage: palletize add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: palletize search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: palletize remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="palletize-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "palletize v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: palletize help"; exit 1 ;;
esac
