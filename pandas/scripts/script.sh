#!/usr/bin/env bash
# pandas — Pandas reference tool. Use when working with pandas in data contexts.
...
show_help() {
    cat << 'HELPEOF'
pandas v$VERSION — Pandas Reference Tool

Usage: pandas <command>
...
## What is Pandas?
Pandas (pandas) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with pandas.

## Key Concepts
- Core pandas principles and fundamentals
- How pandas fits into the broader data ecosystem  
- Essential terminology every practitioner should know

## Why Pandas Matters
Understanding pandas is critical for:
...
## Getting Started
1. Understand the basic pandas concepts
...
## Installation
1. Download or clone the pandas package
...
cmd_patterns() {
    cat << 'EOF'
# Pandas — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for pandas
...
cmd_security() {
    cat << 'EOF'
# Pandas — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Pandas — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "pandas v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: pandas help"; exit 1 ;;
esac
