#!/usr/bin/env bash
# pantry -- Organize pantry stock, expiry dates, and shopping lists. Use when adding items, checking inventory, scheduling restocks, setting expiry reminders.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PANTRY_DIR:-$HOME/.pantry}"
...
show_help() {
    cat << EOF
pantry v$VERSION -- Organize pantry stock, expiry dates, and shopping lists. Use when adding items, checking inventory, scheduling restocks, setting expiry reminders.

Usage: pantry <command> [args]
...
cmd_add() {
    local value="${1:?Usage: pantry add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: pantry search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: pantry remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "pantry v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: pantry help"; exit 1 ;;
esac
