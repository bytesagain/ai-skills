#!/usr/bin/env bash
# pareto — Pareto reference tool. Use when working with pareto in life contexts.
...
show_help() {
    cat << 'HELPEOF'
pareto v$VERSION — Pareto Reference Tool

Usage: pareto <command>
...
## What is Pareto?
Pareto (pareto) is a specialized tool/concept in the life domain.
It provides essential capabilities for professionals working with pareto.

## Key Concepts
- Core pareto principles and fundamentals
- How pareto fits into the broader life ecosystem  
- Essential terminology every practitioner should know

## Why Pareto Matters
Understanding pareto is critical for:
...
## Getting Started
1. Understand the basic pareto concepts
...
cmd_guide() {
    cat << 'EOF'
# Pareto — Step-by-Step Guide

## Overview
This guide walks you through the essential pareto workflows.
...
cmd_mistakes() {
    cat << 'EOF'
# Pareto — Common Mistakes to Avoid
...
cmd_examples() {
    cat << 'EOF'
# Pareto — Real-World Examples

## Example 1: Basic Setup
A typical pareto setup for a small team:
- Standard configuration with defaults
- Basic monitoring enabled
- Manual backup schedule

## Example 2: Production Deployment
An enterprise pareto deployment:
...
cmd_faq() {
    cat << 'EOF'
# Pareto — Frequently Asked Questions

## General
**Q: What is Pareto?**
A: Pareto is a reference tool for pareto in the life domain.

**Q: Who should use this?**
A: Anyone working with pareto who needs quick reference material.
...
case "$CMD" in
    ...
    version|--version|-v) echo "pareto v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: pareto help"; exit 1 ;;
esac
