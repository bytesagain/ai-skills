#!/usr/bin/env bash
# parquet — Parquet reference tool. Use when working with parquet in data contexts.
...
show_help() {
    cat << 'HELPEOF'
parquet v$VERSION — Parquet Reference Tool

Usage: parquet <command>
...
## What is Parquet?
Parquet (parquet) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with parquet.

## Key Concepts
- Core parquet principles and fundamentals
- How parquet fits into the broader data ecosystem  
- Essential terminology every practitioner should know

## Why Parquet Matters
Understanding parquet is critical for:
...
## Getting Started
1. Understand the basic parquet concepts
...
## Installation
1. Download or clone the parquet package
...
cmd_patterns() {
    cat << 'EOF'
# Parquet — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for parquet
...
cmd_security() {
    cat << 'EOF'
# Parquet — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Parquet — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "parquet v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: parquet help"; exit 1 ;;
esac
