#!/usr/bin/env bash
# parquet -- Handle parquet operations. Use when you need to parse parquet structures, migrate data formats, or run analytics pipelines.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PARQUET_DIR:-$HOME/.parquet}"
...
show_help() {
    cat << EOF
parquet v$VERSION -- Handle parquet operations. Use when you need to parse parquet structures, migrate data formats, or run analytics pipelines.

Usage: parquet <command> [args]
...
cmd_add() {
    local value="${1:?Usage: parquet add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: parquet search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: parquet remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
