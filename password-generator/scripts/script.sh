...
DATA_DIR="${HOME}/.local/share/password-generator"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "password-generator v2.0.0"; }

_help() {
    echo "Password Generator v2.0.0 — security toolkit"
    echo ""
    echo "Usage: password-generator <command> [args]"
    echo ""
    echo "Commands:"
...
_export() {
    ...
    case "$fmt" in
        ...
        csv)
            ...
            ;;
        txt)
            echo "=== Password Generator Export ===" > "$out"
            for f in "$DATA_DIR"/*.log; do
                [ -f "$f" ] || continue
...
_status() {
    echo "=== Password Generator Status ==="
    echo "  Version: v2.0.0"
...
_search() {
    local term="${1:?Usage: password-generator search <term>}"
...
case "${1:-help}" in
    generate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/generate.log" 2>/dev/null || echo "  No entries yet. Use: password-generator generate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/generate.log"
            local total=$(wc -l < "$DATA_DIR/generate.log")
            echo "  [Password Generator] generate: $input"
    ...
    check-strength)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/check-strength.log" 2>/dev/null || echo "  No entries yet. Use: password-generator check-strength <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/check-strength.log"
            local total=$(wc -l < "$DATA_DIR/check-strength.log")
            echo "  [Password Generator] check-strength: $input"
    ...
    rotate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/rotate.log" 2>/dev/null || echo "  No entries yet. Use: password-generator rotate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/rotate.log"
            local total=$(wc -l < "$DATA_DIR/rotate.log")
            echo "  [Password Generator] rotate: $input"
    ...
    audit)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/audit.log" 2>/dev/null || echo "  No entries yet. Use: password-generator audit <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/audit.log"
            local total=$(wc -l < "$DATA_DIR/audit.log")
            echo "  [Password Generator] audit: $input"
    ...
    store)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/store.log" 2>/dev/null || echo "  No entries yet. Use: password-generator store <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/store.log"
            local total=$(wc -l < "$DATA_DIR/store.log")
            echo "  [Password Generator] store: $input"
    ...
    retrieve)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/retrieve.log" 2>/dev/null || echo "  No entries yet. Use: password-generator retrieve <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/retrieve.log"
            local total=$(wc -l < "$DATA_DIR/retrieve.log")
            echo "  [Password Generator] retrieve: $input"
    ...
    expire)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/expire.log" 2>/dev/null || echo "  No entries yet. Use: password-generator expire <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/expire.log"
            local total=$(wc -l < "$DATA_DIR/expire.log")
            echo "  [Password Generator] expire: $input"
    ...
    policy)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/policy.log" 2>/dev/null || echo "  No entries yet. Use: password-generator policy <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/policy.log"
            local total=$(wc -l < "$DATA_DIR/policy.log")
            echo "  [Password Generator] policy: $input"
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: password-generator report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [Password Generator] report: $input"
    ...
    hash)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/hash.log" 2>/dev/null || echo "  No entries yet. Use: password-generator hash <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/hash.log"
            local total=$(wc -l < "$DATA_DIR/hash.log")
            echo "  [Password Generator] hash: $input"
    ...
    verify)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/verify.log" 2>/dev/null || echo "  No entries yet. Use: password-generator verify <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/verify.log"
            local total=$(wc -l < "$DATA_DIR/verify.log")
            echo "  [Password Generator] verify: $input"
    ...
    revoke)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/revoke.log" 2>/dev/null || echo "  No entries yet. Use: password-generator revoke <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/revoke.log"
            local total=$(wc -l < "$DATA_DIR/revoke.log")
            echo "  [Password Generator] revoke: $input"
    ...
    *)
        echo "Unknown: $1 — run 'password-generator help'"
        exit 1
        ;;
esac