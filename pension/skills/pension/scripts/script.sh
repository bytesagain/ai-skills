#!/usr/bin/env bash
# pension -- Track pension contributions, balances, and growth forecasts. Use when recording deposits, categorizing funds, reviewing balances, forecasting payouts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PENSION_DIR:-$HOME/.pension}"
...
show_help() {
    cat << EOF
pension v$VERSION -- Track pension contributions, balances, and growth forecasts. Use when recording deposits, categorizing funds, reviewing balances, forecasting payouts.

Usage: pension <command> [args]
...
cmd_add() {
    local value="${1:?Usage: pension add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: pension search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: pension remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "pension v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: pension help"; exit 1 ;;
esac
