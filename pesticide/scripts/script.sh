#!/usr/bin/env bash
# pesticide -- Run pesticide operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PESTICIDE_DIR:-$HOME/.pesticide}"
...
show_help() {
    cat << EOF
pesticide v$VERSION -- Run pesticide operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: pesticide <command> [args]
...
cmd_add() {
    local value="${1:?Usage: pesticide add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: pesticide search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: pesticide remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="pesticide-export.$fmt"
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: pesticide help"; exit 1 ;;
esac
