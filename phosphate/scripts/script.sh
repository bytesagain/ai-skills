#!/usr/bin/env bash
# phosphate -- Run phosphate operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PHOSPHATE_DIR:-$HOME/.phosphate}"
...
show_help() {
    cat << EOF
phosphate v$VERSION -- Run phosphate operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: phosphate <command> [args]
...
cmd_add() {
    local value="${1:?Usage: phosphate add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: phosphate search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: phosphate remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="phosphate-export.$fmt"
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: phosphate help"; exit 1 ;;
esac
