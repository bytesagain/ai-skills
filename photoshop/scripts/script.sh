#!/usr/bin/env bash
# photoshop -- Image editing and graphic design assistant
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PHOTOSHOP_DIR:-$HOME/.photoshop}"
...
show_help() {
    cat << EOF
photoshop v$VERSION -- Image editing and graphic design assistant

Usage: photoshop <command> [args]
...
cmd_add() {
    local value="${1:?Usage: photoshop add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: photoshop search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: photoshop remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="photoshop-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "photoshop v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: photoshop help"; exit 1 ;;
esac
