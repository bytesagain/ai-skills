#!/usr/bin/env bash
# pico — Pico reference tool. Use when working with pico in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
pico v$VERSION — Pico Reference Tool
...
## What is Pico?
Pico (pico) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with pico.

## Key Concepts
- Core pico principles and fundamentals
- How pico fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Pico Matters
Understanding pico is critical for:
...
## Getting Started
1. Understand the basic pico concepts
...
## Installation
1. Download or clone the pico package
...
cmd_patterns() {
    cat << 'EOF'
# Pico — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for pico
...
case "$CMD" in
    ...
    version|--version|-v) echo "pico v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: pico help"; exit 1 ;;
esac
