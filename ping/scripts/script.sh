#!/usr/bin/env bash
# ping -- Monitor network connectivity and diagnose latency issues using ping and traceroute. Use when troubleshooting network problems or checking host availability.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PING_DIR:-$HOME/.ping}"
...
show_help() {
    cat << EOF
ping v$VERSION -- Monitor network connectivity and diagnose latency issues using ping and traceroute. Use when troubleshooting network problems or checking host availability.

Usage: ping <command> [args]
...
cmd_add() {
    local value="${1:?Usage: ping add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: ping search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: ping remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "ping v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: ping help"; exit 1 ;;
esac
