#!/usr/bin/env bash
# pivot — Pivot reference tool. Use when working with pivot in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
pivot v$VERSION — Pivot Reference Tool
...
## What is Pivot?
Pivot (pivot) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with pivot.

## Key Concepts
- Core pivot principles and fundamentals
- How pivot fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Pivot Matters
Understanding pivot is critical for:
...
## Getting Started
1. Understand the basic pivot concepts
...
cmd_formulas() {
    cat << 'EOF'
# Pivot — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Pivot — Instruments & Tools Overview

## Primary Instruments
- Core tools used in pivot operations
...
cmd_glossary() {
    cat << 'EOF'
# Pivot — Key Terms & Definitions

## Core Terminology
- **Pivot**: The primary subject of this reference
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: pivot help"; exit 1 ;;
esac
