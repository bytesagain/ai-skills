#!/usr/bin/env bash
# pkcs — Pkcs reference tool. Use when working with pkcs in security contexts.
...
show_help() {
    cat << 'HELPEOF'
pkcs v$VERSION — Pkcs Reference Tool
...
## What is Pkcs?
Pkcs (pkcs) is a specialized tool/concept in the security domain.
It provides essential capabilities for professionals working with pkcs.

## Key Concepts
- Core pkcs principles and fundamentals
- How pkcs fits into the broader security ecosystem  
- Essential terminology every practitioner should know

## Why Pkcs Matters
Understanding pkcs is critical for:
...
## Getting Started
1. Understand the basic pkcs concepts
...
## Installation
1. Download or clone the pkcs package
...
cmd_patterns() {
    cat << 'EOF'
# Pkcs — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for pkcs
...
case "$CMD" in
    ...
    version|--version|-v) echo "pkcs v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: pkcs help"; exit 1 ;;
esac
