#!/usr/bin/env bash
# plant -- Manage houseplant care with watering and fertilizer schedules. Use when adding plants, tracking watering, scheduling fertilizing, setting care reminders.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PLANT_DIR:-$HOME/.plant}"
...
show_help() {
    cat << EOF
plant v$VERSION -- Manage houseplant care with watering and fertilizer schedules. Use when adding plants, tracking watering, scheduling fertilizing, setting care reminders.

Usage: plant <command> [args]
...
cmd_add() {
    local value="${1:?Usage: plant add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: plant search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: plant remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "plant v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: plant help"; exit 1 ;;
esac
