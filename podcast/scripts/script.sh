#!/usr/bin/env bash
# podcast -- Manage podcast production workflows, episodes, and publishing using CLI tools. Use when you need to plan, script, schedule, and track podcast episodes.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PODCAST_DIR:-$HOME/.podcast}"
...
show_help() {
    cat << EOF
podcast v$VERSION -- Manage podcast production workflows, episodes, and publishing using CLI tools. Use when you need to plan, script, schedule, and track podcast episodes.

Usage: podcast <command> [args]
...
cmd_add() {
    local value="${1:?Usage: podcast add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: podcast search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: podcast remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "podcast v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: podcast help"; exit 1 ;;
esac
