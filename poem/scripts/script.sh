#!/usr/bin/env bash
# poem -- Compose and refine poems across multiple styles. Use when drafting verse, checking meter and rhyme, converting prose to poetry, generating themed poems.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${POEM_DIR:-$HOME/.poem}"
...
show_help() {
    cat << EOF
poem v$VERSION -- Compose and refine poems across multiple styles. Use when drafting verse, checking meter and rhyme, converting prose to poetry, generating themed poems.

Usage: poem <command> [args]
...
cmd_add() {
    local value="${1:?Usage: poem add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: poem remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "poem v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: poem help"; exit 1 ;;
esac
