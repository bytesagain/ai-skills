#!/usr/bin/env bash
# polymer — Polymer reference tool. Use when working with polymer in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
polymer v$VERSION — Polymer Reference Tool
...
## What is Polymer?
Polymer (polymer) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with polymer.

## Key Concepts
- Core polymer principles and fundamentals
- How polymer fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Polymer Matters
Understanding polymer is critical for:
...
## Getting Started
1. Understand the basic polymer concepts
...
## Installation
1. Download or clone the polymer package
...
cmd_patterns() {
    cat << 'EOF'
# Polymer — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for polymer
...
cmd_security() {
    cat << 'EOF'
# Polymer — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Polymer — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "polymer v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: polymer help"; exit 1 ;;
esac
