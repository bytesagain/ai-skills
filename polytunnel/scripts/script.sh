#!/usr/bin/env bash
# polytunnel -- Run polytunnel operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${POLYTUNNEL_DIR:-$HOME/.polytunnel}"
...
show_help() {
    cat << EOF
polytunnel v$VERSION -- Run polytunnel operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: polytunnel <command> [args]
...
cmd_add() {
    local value="${1:?Usage: polytunnel add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: polytunnel search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: polytunnel remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="polytunnel-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "polytunnel v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: polytunnel help"; exit 1 ;;
esac
