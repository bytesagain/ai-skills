#!/usr/bin/env bash
# potash -- Run potash operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${POTASH_DIR:-$HOME/.potash}"
...
show_help() {
    cat << EOF
potash v$VERSION -- Run potash operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: potash <command> [args]
...
cmd_add() {
    local value="${1:?Usage: potash add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: potash search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: potash remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "potash v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: potash help"; exit 1 ;;
esac
