#!/usr/bin/env bash
# preact — Preact reference tool. Use when working with preact in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
preact v$VERSION — Preact Reference Tool

Usage: preact <command>
...
## What is Preact?
Preact (preact) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with preact.

## Key Concepts
- Core preact principles and fundamentals
- How preact fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Preact Matters
Understanding preact is critical for:
...
## Getting Started
1. Understand the basic preact concepts
...
## Installation
1. Download or clone the preact package
...
cmd_patterns() {
    cat << 'EOF'
# Preact — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for preact
...
cmd_security() {
    cat << 'EOF'
# Preact — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Preact — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "preact v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: preact help"; exit 1 ;;
esac
