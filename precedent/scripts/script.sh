#!/usr/bin/env bash
# precedent -- Run precedent operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PRECEDENT_DIR:-$HOME/.precedent}"
...
show_help() {
    cat << EOF
precedent v$VERSION -- Run precedent operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: precedent <command> [args]
...
cmd_add() {
    local value="${1:?Usage: precedent add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: precedent search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: precedent remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="precedent-export.$fmt"
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: precedent help"; exit 1 ;;
esac
