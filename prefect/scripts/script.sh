#!/usr/bin/env bash
# prefect — Prefect reference tool. Use when working with prefect in data contexts.
...
show_help() {
    cat << 'HELPEOF'
prefect v$VERSION — Prefect Reference Tool

Usage: prefect <command>
...
## What is Prefect?
Prefect (prefect) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with prefect.

## Key Concepts
- Core prefect principles and fundamentals
- How prefect fits into the broader data ecosystem  
- Essential terminology every practitioner should know

## Why Prefect Matters
Understanding prefect is critical for:
...
## Getting Started
1. Understand the basic prefect concepts
...
## Installation
1. Download or clone the prefect package
...
cmd_patterns() {
    cat << 'EOF'
# Prefect — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for prefect
...
cmd_security() {
    cat << 'EOF'
# Prefect — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Prefect — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "prefect v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: prefect help"; exit 1 ;;
esac
