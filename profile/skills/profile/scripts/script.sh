#!/usr/bin/env bash
# profile -- Analyze code and system performance profiles using profiling tools. Use when identifying bottlenecks, hotspots, or memory issues in applications.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PROFILE_DIR:-$HOME/.profile}"
...
show_help() {
    cat << EOF
profile v$VERSION -- Analyze code and system performance profiles using profiling tools. Use when identifying bottlenecks, hotspots, or memory issues in applications.
...
cmd_add() {
    local value="${1:?Usage: profile add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: profile search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: profile remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "profile v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: profile help"; exit 1 ;;
esac
