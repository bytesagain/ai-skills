#!/usr/bin/env bash
# profit -- Calculate profit margins, breakeven points, and revenue forecasts using CLI tools. Use when analyzing business financials.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PROFIT_DIR:-$HOME/.profit}"
...
show_help() {
    cat << EOF
profit v$VERSION -- Calculate profit margins, breakeven points, and revenue forecasts using CLI tools. Use when analyzing business financials.

Usage: profit <command> [args]
...
cmd_add() {
    local value="${1:?Usage: profit add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: profit search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: profit remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "profit v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: profit help"; exit 1 ;;
esac
