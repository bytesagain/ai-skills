#!/usr/bin/env bash
# prometheus — Prometheus reference tool. Use when working with prometheus in sysops contexts.
...
show_help() {
    cat << 'HELPEOF'
prometheus v$VERSION — Prometheus Reference Tool
...
## What is Prometheus?
Prometheus (prometheus) is a specialized tool/concept in the sysops domain.
It provides essential capabilities for professionals working with prometheus.

## Key Concepts
- Core prometheus principles and fundamentals
- How prometheus fits into the broader sysops ecosystem  
- Essential terminology every practitioner should know

## Why Prometheus Matters
Understanding prometheus is critical for:
...
## Getting Started
1. Understand the basic prometheus concepts
...
## Installation
1. Download or clone the prometheus package
...
cmd_patterns() {
    cat << 'EOF'
# Prometheus — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for prometheus
...
cmd_migration() {
    cat << 'EOF'
# Prometheus — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "prometheus v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: prometheus help"; exit 1 ;;
esac
