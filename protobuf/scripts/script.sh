#!/usr/bin/env bash
# protobuf -- Handle protobuf operations. Use when you need to parse protobuf structures, migrate data formats, or run analytics pipelines.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PROTOBUF_DIR:-$HOME/.protobuf}"
...
show_help() {
    cat << EOF
protobuf v$VERSION -- Handle protobuf operations. Use when you need to parse protobuf structures, migrate data formats, or run analytics pipelines.

Usage: protobuf <command> [args]
...
cmd_add() {
    local value="${1:?Usage: protobuf add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: protobuf search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: protobuf remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
