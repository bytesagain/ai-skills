#!/usr/bin/env bash
# puzzle -- Brain teasers, riddles, and logic puzzles with streak tracking. Use when exercising your brain.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PUZZLE_DIR:-$HOME/.puzzle}"
...
show_help() {
    cat << EOF
puzzle v$VERSION -- Brain teasers, riddles, and logic puzzles with streak tracking. Use when exercising your brain.

Usage: puzzle <command> [args]
...
cmd_add() {
    local value="${1:?Usage: puzzle add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: puzzle remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "puzzle v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: puzzle help"; exit 1 ;;
esac
