#!/usr/bin/env bash
# query -- Build and format SQL queries interactively using a CLI query builder. Use when constructing complex database queries.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${QUERY_DIR:-$HOME/.query}"
...
show_help() {
    cat << EOF
query v$VERSION -- Build and format SQL queries interactively using a CLI query builder. Use when constructing complex database queries.
...
cmd_add() {
    local value="${1:?Usage: query add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: query search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: query remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "query v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: query help"; exit 1 ;;
esac
