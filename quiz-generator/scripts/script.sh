#!/usr/bin/env bash
# quiz-generator - Learning and study assistant
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${QUIZ_GENERATOR_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/quiz-generator}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
quiz-generator v$VERSION

Learning and study assistant

Usage: quiz-generator <command> [args]
...
case "${1:-help}" in
    ...
    test) shift; cmd_test "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "quiz-generator v$VERSION" ;;
