#!/usr/bin/env bash
# quote -- Record, search, and display quotes and sayings. Use when saving favorites, checking attribution, converting collections, analyzing themes, generating picks.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${QUOTE_DIR:-$HOME/.quote}"
...
show_help() {
    cat << EOF
quote v$VERSION -- Record, search, and display quotes and sayings. Use when saving favorites, checking attribution, converting collections, analyzing themes, generating picks.
...
cmd_add() {
    local value="${1:?Usage: quote add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: quote search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: quote remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "quote v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: quote help"; exit 1 ;;
esac
