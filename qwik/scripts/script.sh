#!/usr/bin/env bash
# qwik — Qwik reference tool. Use when working with qwik in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
qwik v$VERSION — Qwik Reference Tool
...
## What is Qwik?
Qwik (qwik) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with qwik.

## Key Concepts
- Core qwik principles and fundamentals
- How qwik fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Qwik Matters
Understanding qwik is critical for:
...
## Getting Started
1. Understand the basic qwik concepts
...
## Installation
1. Download or clone the qwik package
...
cmd_patterns() {
    cat << 'EOF'
# Qwik — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for qwik
...
case "$CMD" in
    ...
    version|--version|-v) echo "qwik v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: qwik help"; exit 1 ;;
esac
