#!/usr/bin/env bash
# rating -- Calculate scores, rankings, and review aggregations. Use when computing star ratings, checking distributions, converting scales, analyzing sentiment.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${RATING_DIR:-$HOME/.rating}"
...
show_help() {
    cat << EOF
rating v$VERSION -- Calculate scores, rankings, and review aggregations. Use when computing star ratings, checking distributions, converting scales, analyzing sentiment.

Usage: rating <command> [args]
...
cmd_add() {
    local value="${1:?Usage: rating add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: rating search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: rating remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "rating v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: rating help"; exit 1 ;;
esac
