#!/usr/bin/env bash
# receipt -- Scan, categorize, and total receipts for expenses. Use when recording purchases, categorizing spending, balancing monthly totals, forecasting budgets.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${RECEIPT_DIR:-$HOME/.receipt}"
...
show_help() {
    cat << EOF
receipt v$VERSION -- Scan, categorize, and total receipts for expenses. Use when recording purchases, categorizing spending, balancing monthly totals, forecasting budgets.

Usage: receipt <command> [args]
...
cmd_add() {
    local value="${1:?Usage: receipt add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: receipt remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "receipt v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: receipt help"; exit 1 ;;
esac
