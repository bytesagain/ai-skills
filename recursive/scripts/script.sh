#!/usr/bin/env bash
# recursive -- Analyze recursive operations. Use when you need to understand recursive mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${RECURSIVE_DIR:-$HOME/.recursive}"
...
show_help() {
    cat << EOF
recursive v$VERSION -- Analyze recursive operations. Use when you need to understand recursive mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: recursive <command> [args]
...
cmd_add() {
    local value="${1:?Usage: recursive add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: recursive search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: recursive remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="recursive-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "recursive v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: recursive help"; exit 1 ;;
esac
