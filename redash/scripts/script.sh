#!/usr/bin/env bash
# redash — Redash reference tool. Use when working with redash in data contexts.
...
show_help() {
    cat << 'HELPEOF'
redash v$VERSION — Redash Reference Tool

Usage: redash <command>
...
## What is Redash?
Redash (redash) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with redash.

## Key Concepts
- Core redash principles and fundamentals
- How redash fits into the broader data ecosystem  
- Essential terminology every practitioner should know

## Why Redash Matters
Understanding redash is critical for:
...
## Getting Started
1. Understand the basic redash concepts
...
## Installation
1. Download or clone the redash package
...
cmd_patterns() {
    cat << 'EOF'
# Redash — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for redash
...
cmd_performance() {
    cat << 'EOF'
# Redash — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Redash — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Redash — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "redash v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: redash help"; exit 1 ;;
esac
