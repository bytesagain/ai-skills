#!/usr/bin/env bash
# reduce -- Handle reduce operations. Use when you need to parse reduce structures, migrate data formats, or run analytics pipelines.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${REDUCE_DIR:-$HOME/.reduce}"
...
show_help() {
    cat << EOF
reduce v$VERSION -- Handle reduce operations. Use when you need to parse reduce structures, migrate data formats, or run analytics pipelines.

Usage: reduce <command> [args]
...
cmd_add() {
    local value="${1:?Usage: reduce add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: reduce search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: reduce remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "reduce v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: reduce help"; exit 1 ;;
esac
