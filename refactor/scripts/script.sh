#!/usr/bin/env bash
# refactor — Refactor reference tool. Use when working with refactor in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
refactor v$VERSION — Refactor Reference Tool

Usage: refactor <command>
...
## What is Refactor?
Refactor (refactor) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with refactor.

## Key Concepts
- Core refactor principles and fundamentals
- How refactor fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Refactor Matters
Understanding refactor is critical for:
...
## Getting Started
1. Understand the basic refactor concepts
...
## Installation
1. Download or clone the refactor package
...
cmd_patterns() {
    cat << 'EOF'
# Refactor — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for refactor
...
cmd_migration() {
    cat << 'EOF'
# Refactor — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "refactor v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: refactor help"; exit 1 ;;
esac
