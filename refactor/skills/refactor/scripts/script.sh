#!/usr/bin/env bash
# refactor -- Analyze code quality and suggest refactoring improvements using static analysis. Use when cleaning up or improving codebases.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${REFACTOR_DIR:-$HOME/.refactor}"
...
show_help() {
    cat << EOF
refactor v$VERSION -- Analyze code quality and suggest refactoring improvements using static analysis. Use when cleaning up or improving codebases.

Usage: refactor <command> [args]
...
cmd_add() {
    local value="${1:?Usage: refactor add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: refactor search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: refactor remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
