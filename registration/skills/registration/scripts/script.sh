#!/usr/bin/env bash
# registration -- Manage registration records and attendee data. Use when logging sign-ups, checking capacity, converting export formats, generating confirmation reports.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${REGISTRATION_DIR:-$HOME/.registration}"
...
show_help() {
    cat << EOF
registration v$VERSION -- Manage registration records and attendee data. Use when logging sign-ups, checking capacity, converting export formats, generating confirmation reports.

Usage: registration <command> [args]
...
cmd_add() {
    local value="${1:?Usage: registration add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: registration search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: registration remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="registration-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "registration v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: registration help"; exit 1 ;;
esac
