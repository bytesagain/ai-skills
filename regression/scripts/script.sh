...
VERSION="1.0.0"
DATA_DIR="${REGRESSION_DIR:-$HOME/.regression}"
...
show_help() {
    cat << EOF
regression v$VERSION -- Compute regression operations. Use when you need to solve regression equations, run statistical analysis, or simulate numerical models.

Usage: regression <command> [args]
...
cmd_add() {
    local value="${1:?Usage: regression add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: regression search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_calculate() {
    local expr="${*:?Usage: regression calculate <expression>}"
...
cmd_remove() {
    local num="${1:?Usage: regression remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="regression-export.$fmt"
