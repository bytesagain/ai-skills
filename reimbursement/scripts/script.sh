#!/usr/bin/env bash
# reimbursement -- Calculate reimbursement financial metrics and business data. Use when tracking expenses, analyzing investments, or generating financial reports.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${REIMBURSEMENT_DIR:-$HOME/.reimbursement}"
...
show_help() {
    cat << EOF
reimbursement v$VERSION -- Calculate reimbursement financial metrics and business data. Use when tracking expenses, analyzing investments, or generating financial reports.

Usage: reimbursement <command> [args]
...
cmd_add() {
    local value="${1:?Usage: reimbursement add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: reimbursement search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: reimbursement remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="reimbursement-export.$fmt"
