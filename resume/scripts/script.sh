#!/usr/bin/env bash
# resume -- Generate and manage professional resumes with sections and templates using CLI tools. Use when you need to create, tailor,
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${RESUME_DIR:-$HOME/.resume}"
...
show_help() {
    cat << EOF
resume v$VERSION -- Generate and manage professional resumes with sections and templates using CLI tools. Use when you need to create, tailor,

Usage: resume <command> [args]
...
cmd_add() {
    local value="${1:?Usage: resume add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: resume search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: resume remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "resume v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: resume help"; exit 1 ;;
esac
