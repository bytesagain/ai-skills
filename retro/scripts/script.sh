#!/usr/bin/env bash
# retro — Retro reference tool. Use when working with retro in life contexts.
...
show_help() {
    cat << 'HELPEOF'
retro v$VERSION — Retro Reference Tool
...
## What is Retro?
Retro (retro) is a specialized tool/concept in the life domain.
It provides essential capabilities for professionals working with retro.

## Key Concepts
- Core retro principles and fundamentals
- How retro fits into the broader life ecosystem  
- Essential terminology every practitioner should know

## Why Retro Matters
Understanding retro is critical for:
...
## Getting Started
1. Understand the basic retro concepts
...
cmd_guide() {
    cat << 'EOF'
# Retro — Step-by-Step Guide

## Overview
This guide walks you through the essential retro workflows.
...
cmd_mistakes() {
    cat << 'EOF'
# Retro — Common Mistakes to Avoid
...
cmd_examples() {
    cat << 'EOF'
# Retro — Real-World Examples

## Example 1: Basic Setup
A typical retro setup for a small team:
- Standard configuration with defaults
- Basic monitoring enabled
- Manual backup schedule

## Example 2: Production Deployment
An enterprise retro deployment:
...
cmd_faq() {
    cat << 'EOF'
# Retro — Frequently Asked Questions

## General
**Q: What is Retro?**
A: Retro is a reference tool for retro in the life domain.

**Q: Who should use this?**
A: Anyone working with retro who needs quick reference material.
...
case "$CMD" in
    ...
    version|--version|-v) echo "retro v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: retro help"; exit 1 ;;
esac
