#!/usr/bin/env bash
# reward -- Track reward personal activities and daily routines. Use when managing health goals, organizing tasks, or building productive habits.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${REWARD_DIR:-$HOME/.reward}"
...
show_help() {
    cat << EOF
reward v$VERSION -- Track reward personal activities and daily routines. Use when managing health goals, organizing tasks, or building productive habits.

Usage: reward <command> [args]
...
cmd_add() {
    local value="${1:?Usage: reward add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: reward search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: reward remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: reward help"; exit 1 ;;
esac
