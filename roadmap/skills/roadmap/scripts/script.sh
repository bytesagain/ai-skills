#!/usr/bin/env bash
# roadmap -- Plan product roadmaps with milestones and dependencies. Use when adding features, planning sprints, tracking milestone progress, reviewing priorities.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ROADMAP_DIR:-$HOME/.roadmap}"
...
show_help() {
    cat << EOF
roadmap v$VERSION -- Plan product roadmaps with milestones and dependencies. Use when adding features, planning sprints, tracking milestone progress, reviewing priorities.
...
cmd_add() {
    local value="${1:?Usage: roadmap add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: roadmap search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: roadmap remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "roadmap v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: roadmap help"; exit 1 ;;
esac
