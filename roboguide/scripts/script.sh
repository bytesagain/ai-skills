#!/usr/bin/env bash
# roboguide -- Robot path planning and simulation tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ROBOGUIDE_DIR:-$HOME/.roboguide}"
...
show_help() {
    cat << EOF
roboguide v$VERSION -- Robot path planning and simulation tool

Usage: roboguide <command> [args]
...
cmd_add() {
    local value="${1:?Usage: roboguide add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: roboguide search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: roboguide remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="roboguide-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "roboguide v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: roboguide help"; exit 1 ;;
esac
