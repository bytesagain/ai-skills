#!/usr/bin/env bash
# robotstudio -- Robot simulation and programming helper
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ROBOTSTUDIO_DIR:-$HOME/.robotstudio}"
...
show_help() {
    cat << EOF
robotstudio v$VERSION -- Robot simulation and programming helper

Usage: robotstudio <command> [args]
...
cmd_add() {
    local value="${1:?Usage: robotstudio add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: robotstudio search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: robotstudio remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="robotstudio-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "robotstudio v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: robotstudio help"; exit 1 ;;
esac
