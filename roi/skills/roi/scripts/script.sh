#!/usr/bin/env bash
# roi -- Calculate and compare return on investment across projects using financial models. Use when evaluating investment decisions.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ROI_DIR:-$HOME/.roi}"
...
show_help() {
    cat << EOF
roi v$VERSION -- Calculate and compare return on investment across projects using financial models. Use when evaluating investment decisions.

Usage: roi <command> [args]
...
cmd_add() {
    local value="${1:?Usage: roi add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: roi search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: roi remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "roi v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: roi help"; exit 1 ;;
esac
