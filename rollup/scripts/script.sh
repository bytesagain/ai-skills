#!/usr/bin/env bash
# rollup — Rollup reference tool. Use when working with rollup in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
rollup v$VERSION — Rollup Reference Tool

Usage: rollup <command>
...
## What is Rollup?
Rollup (rollup) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with rollup.

## Key Concepts
- Core rollup principles and fundamentals
- How rollup fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Rollup Matters
Understanding rollup is critical for:
...
## Getting Started
1. Understand the basic rollup concepts
...
## Installation
1. Download or clone the rollup package
...
cmd_patterns() {
    cat << 'EOF'
# Rollup — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for rollup
...
cmd_performance() {
    cat << 'EOF'
# Rollup — Performance Optimization
...
cmd_migration() {
    cat << 'EOF'
# Rollup — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "rollup v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: rollup help"; exit 1 ;;
esac
