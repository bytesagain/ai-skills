#!/usr/bin/env bash
# rollup -- Analyze rollup operations. Use when you need to understand rollup mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ROLLUP_DIR:-$HOME/.rollup}"
...
show_help() {
    cat << EOF
rollup v$VERSION -- Analyze rollup operations. Use when you need to understand rollup mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: rollup <command> [args]
...
cmd_add() {
    local value="${1:?Usage: rollup add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: rollup search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: rollup remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
