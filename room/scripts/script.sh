#!/usr/bin/env bash
# room -- Organize room inventory and maintenance schedules. Use when adding furniture items, tracking appliances, scheduling deep cleans, setting repair reminders.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ROOM_DIR:-$HOME/.room}"
...
show_help() {
    cat << EOF
room v$VERSION -- Organize room inventory and maintenance schedules. Use when adding furniture items, tracking appliances, scheduling deep cleans, setting repair reminders.

Usage: room <command> [args]
...
cmd_add() {
    local value="${1:?Usage: room add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: room search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: room remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "room v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: room help"; exit 1 ;;
esac
