#!/usr/bin/env bash
# scaffold — Scaffold reference tool. Use when working with scaffold in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
scaffold v$VERSION — Scaffold Reference Tool
...
## What is Scaffold?
Scaffold (scaffold) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with scaffold.

## Key Concepts
- Core scaffold principles and fundamentals
- How scaffold fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Scaffold Matters
Understanding scaffold is critical for:
...
## Getting Started
1. Understand the basic scaffold concepts
...
## Installation
1. Download or clone the scaffold package
...
cmd_patterns() {
    cat << 'EOF'
# Scaffold — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for scaffold
...
cmd_performance() {
    cat << 'EOF'
# Scaffold — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Scaffold — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Scaffold — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "scaffold v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: scaffold help"; exit 1 ;;
esac
