#!/usr/bin/env bash
# schema -- Design and manage database schemas using CLI tools. Use when you need to create tables, columns, indexes, relations, validate, migrate, diff,
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SCHEMA_DIR:-$HOME/.schema}"
...
show_help() {
    cat << EOF
schema v$VERSION -- Design and manage database schemas using CLI tools. Use when you need to create tables, columns, indexes, relations, validate, migrate, diff,

Usage: schema <command> [args]
...
cmd_add() {
    local value="${1:?Usage: schema add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: schema search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: schema remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "schema v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: schema help"; exit 1 ;;
esac
