#!/usr/bin/env bash
# scipy — Scipy reference tool. Use when working with scipy in data contexts.
...
show_help() {
    cat << 'HELPEOF'
scipy v$VERSION — Scipy Reference Tool
...
## What is Scipy?
Scipy (scipy) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with scipy.

## Key Concepts
- Core scipy principles and fundamentals
- How scipy fits into the broader data ecosystem  
- Essential terminology every practitioner should know

## Why Scipy Matters
Understanding scipy is critical for:
...
## Getting Started
1. Understand the basic scipy concepts
...
## Installation
1. Download or clone the scipy package
...
cmd_patterns() {
    cat << 'EOF'
# Scipy — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for scipy
...
cmd_performance() {
    cat << 'EOF'
# Scipy — Performance Optimization
...
case "$CMD" in
    ...
    version|--version|-v) echo "scipy v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: scipy help"; exit 1 ;;
esac
