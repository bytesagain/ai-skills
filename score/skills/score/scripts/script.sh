#!/usr/bin/env bash
# score -- Track game scores, brackets, and player statistics. Use when recording results, scoring rounds, ranking leaderboards, reviewing game history, computing stats.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SCORE_DIR:-$HOME/.score}"
...
show_help() {
    cat << EOF
score v$VERSION -- Track game scores, brackets, and player statistics. Use when recording results, scoring rounds, ranking leaderboards, reviewing game history, computing stats.
...
cmd_add() {
    local value="${1:?Usage: score add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: score remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "score v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: score help"; exit 1 ;;
esac
