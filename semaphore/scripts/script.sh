#!/usr/bin/env bash
# Semaphore — travel tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

DATA_DIR="${HOME}/.local/share/semaphore"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "semaphore v2.0.0"; }

_help() {
    echo "Semaphore v2.0.0 — travel toolkit"
    echo ""
    echo "Usage: semaphore <command> [args]"
    echo ""
    echo "Commands:"
    echo "  plan               Plan"
    echo "  search             Search"
...
_search() {
    local term="${1:?Usage: semaphore search <term>}"
...
case "${1:-help}" in
    plan)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/plan.log" 2>/dev/null || echo "  No entries yet. Use: semaphore plan <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/plan.log"
            local total=$(wc -l < "$DATA_DIR/plan.log")
            echo "  [Semaphore] plan: $input"
    ...
    search)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/search.log" 2>/dev/null || echo "  No entries yet. Use: semaphore search <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/search.log"
            local total=$(wc -l < "$DATA_DIR/search.log")
            echo "  [Semaphore] search: $input"
    ...
    book)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/book.log" 2>/dev/null || echo "  No entries yet. Use: semaphore book <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/book.log"
            local total=$(wc -l < "$DATA_DIR/book.log")
            echo "  [Semaphore] book: $input"
    ...
    pack-list)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/pack-list.log" 2>/dev/null || echo "  No entries yet. Use: semaphore pack-list <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/pack-list.log"
            local total=$(wc -l < "$DATA_DIR/pack-list.log")
            echo "  [Semaphore] pack-list: $input"
    ...
    budget)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/budget.log" 2>/dev/null || echo "  No entries yet. Use: semaphore budget <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/budget.log"
            local total=$(wc -l < "$DATA_DIR/budget.log")
            echo "  [Semaphore] budget: $input"
    ...
    convert)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/convert.log" 2>/dev/null || echo "  No entries yet. Use: semaphore convert <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/convert.log"
            local total=$(wc -l < "$DATA_DIR/convert.log")
            echo "  [Semaphore] convert: $input"
    ...
    weather)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/weather.log" 2>/dev/null || echo "  No entries yet. Use: semaphore weather <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/weather.log"
            local total=$(wc -l < "$DATA_DIR/weather.log")
            echo "  [Semaphore] weather: $input"
    ...
    route)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/route.log" 2>/dev/null || echo "  No entries yet. Use: semaphore route <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/route.log"
            local total=$(wc -l < "$DATA_DIR/route.log")
            echo "  [Semaphore] route: $input"
    ...
    checklist)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/checklist.log" 2>/dev/null || echo "  No entries yet. Use: semaphore checklist <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/checklist.log"
            local total=$(wc -l < "$DATA_DIR/checklist.log")
            echo "  [Semaphore] checklist: $input"
    ...
    journal)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/journal.log" 2>/dev/null || echo "  No entries yet. Use: semaphore journal <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/journal.log"
            local total=$(wc -l < "$DATA_DIR/journal.log")
            echo "  [Semaphore] journal: $input"
    ...
    compare)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/compare.log" 2>/dev/null || echo "  No entries yet. Use: semaphore compare <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/compare.log"
            local total=$(wc -l < "$DATA_DIR/compare.log")
            echo "  [Semaphore] compare: $input"
    ...
    remind)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/remind.log" 2>/dev/null || echo "  No entries yet. Use: semaphore remind <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/remind.log"
            local total=$(wc -l < "$DATA_DIR/remind.log")
            echo "  [Semaphore] remind: $input"
    ...
    *)
        echo "Unknown: $1 — run 'semaphore help'"
        exit 1
        ;;
esac