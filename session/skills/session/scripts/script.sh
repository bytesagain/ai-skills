#!/usr/bin/env bash
# session -- Manage session state and lifecycle using JSONL storage. Use when tracking user sessions, expiring stale data, or auditing session activity.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SESSION_DIR:-$HOME/.session}"
...
show_help() {
    cat << EOF
session v$VERSION -- Manage session state and lifecycle using JSONL storage. Use when tracking user sessions, expiring stale data, or auditing session activity.
...
cmd_add() {
    local value="${1:?Usage: session add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: session search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: session remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "session v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: session help"; exit 1 ;;
esac
