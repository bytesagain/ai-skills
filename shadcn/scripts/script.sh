#!/usr/bin/env bash
# shadcn — Shadcn reference tool. Use when working with shadcn in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
shadcn v$VERSION — Shadcn Reference Tool

Usage: shadcn <command>
...
## What is Shadcn?
Shadcn (shadcn) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with shadcn.

## Key Concepts
- Core shadcn principles and fundamentals
- How shadcn fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Shadcn Matters
Understanding shadcn is critical for:
...
## Getting Started
1. Understand the basic shadcn concepts
...
## Installation
1. Download or clone the shadcn package
...
cmd_patterns() {
    cat << 'EOF'
# Shadcn — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for shadcn
...
cmd_performance() {
    cat << 'EOF'
# Shadcn — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Shadcn — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Shadcn — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "shadcn v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: shadcn help"; exit 1 ;;
esac
