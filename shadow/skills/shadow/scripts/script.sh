#!/usr/bin/env bash
# shadow -- Generate and preview CSS shadow effects using CLI tools. Use when you need box-shadow, text-shadow, drop-shadow, layered shadows, presets, animations,
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SHADOW_DIR:-$HOME/.shadow}"
...
show_help() {
    cat << EOF
shadow v$VERSION -- Generate and preview CSS shadow effects using CLI tools. Use when you need box-shadow, text-shadow, drop-shadow, layered shadows, presets, animations,
...
cmd_add() {
    local value="${1:?Usage: shadow add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: shadow search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: shadow remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "shadow v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: shadow help"; exit 1 ;;
esac
