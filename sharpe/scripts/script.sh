#!/usr/bin/env bash
# sharpe -- Calculate sharpe financial metrics and business data. Use when tracking expenses, analyzing investments, or generating reports.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SHARPE_DIR:-$HOME/.sharpe}"
...
show_help() {
    cat << EOF
sharpe v$VERSION -- Calculate sharpe financial metrics and business data. Use when tracking expenses, analyzing investments, or generating reports.

Usage: sharpe <command> [args]
...
cmd_add() {
    local value="${1:?Usage: sharpe add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: sharpe search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: sharpe remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
