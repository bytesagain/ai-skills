#!/usr/bin/env bash
# sharpe — Sharpe reference tool. Use when working with sharpe in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
sharpe v$VERSION — Sharpe Reference Tool

Usage: sharpe <command>
...
## What is Sharpe?
Sharpe (sharpe) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with sharpe.

## Key Concepts
- Core sharpe principles and fundamentals
- How sharpe fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Sharpe Matters
Understanding sharpe is critical for:
...
## Getting Started
1. Understand the basic sharpe concepts
...
cmd_formulas() {
    cat << 'EOF'
# Sharpe — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Sharpe — Instruments & Tools Overview

## Primary Instruments
- Core tools used in sharpe operations
...
cmd_glossary() {
    cat << 'EOF'
# Sharpe — Key Terms & Definitions

## Core Terminology
- **Sharpe**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "sharpe v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: sharpe help"; exit 1 ;;
esac
