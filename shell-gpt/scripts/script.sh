#!/usr/bin/env bash
# shell-gpt — Shell Gpt reference tool. Use when working with shell gpt in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
shell-gpt v$VERSION — Shell Gpt Reference Tool

Usage: shell-gpt <command>
...
## What is Shell Gpt?
Shell Gpt (shell-gpt) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with shell gpt.

## Key Concepts
- Core shell gpt principles and fundamentals
- How shell gpt fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Shell Gpt Matters
Understanding shell gpt is critical for:
...
## Getting Started
1. Understand the basic shell gpt concepts
...
## Installation
1. Download or clone the shell gpt package
...
cmd_patterns() {
    cat << 'EOF'
# Shell Gpt — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for shell gpt
...
cmd_performance() {
    cat << 'EOF'
# Shell Gpt — Performance Optimization
...
cmd_migration() {
    cat << 'EOF'
# Shell Gpt — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "shell-gpt v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: shell-gpt help"; exit 1 ;;
esac
