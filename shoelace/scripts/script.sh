#!/usr/bin/env bash
# shoelace — Shoelace reference tool. Use when working with shoelace in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
shoelace v$VERSION — Shoelace Reference Tool
...
## What is Shoelace?
Shoelace (shoelace) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with shoelace.

## Key Concepts
- Core shoelace principles and fundamentals
- How shoelace fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Shoelace Matters
Understanding shoelace is critical for:
...
## Getting Started
1. Understand the basic shoelace concepts
...
## Installation
1. Download or clone the shoelace package
...
cmd_patterns() {
    cat << 'EOF'
# Shoelace — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for shoelace
...
cmd_performance() {
    cat << 'EOF'
# Shoelace — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Shoelace — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Shoelace — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "shoelace v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: shoelace help"; exit 1 ;;
esac
