#!/usr/bin/env bash
# sidechain -- Analyze sidechain operations. Use when you need to understand sidechain mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SIDECHAIN_DIR:-$HOME/.sidechain}"
...
show_help() {
    cat << EOF
sidechain v$VERSION -- Analyze sidechain operations. Use when you need to understand sidechain mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: sidechain <command> [args]
...
cmd_add() {
    local value="${1:?Usage: sidechain add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: sidechain search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: sidechain remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="sidechain-export.$fmt"
