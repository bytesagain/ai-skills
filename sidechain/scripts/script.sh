#!/usr/bin/env bash
# sidechain — Sidechain reference tool. Use when working with sidechain in blockchain contexts.
...
show_help() {
    cat << 'HELPEOF'
sidechain v$VERSION — Sidechain Reference Tool

Usage: sidechain <command>
...
## What is Sidechain?
Sidechain (sidechain) is a specialized tool/concept in the blockchain domain.
It provides essential capabilities for professionals working with sidechain.

## Key Concepts
- Core sidechain principles and fundamentals
- How sidechain fits into the broader blockchain ecosystem  
- Essential terminology every practitioner should know

## Why Sidechain Matters
Understanding sidechain is critical for:
...
## Getting Started
1. Understand the basic sidechain concepts
...
cmd_formulas() {
    cat << 'EOF'
# Sidechain — Key Formulas & Calculations
...
cmd_regulations() {
    cat << 'EOF'
# Sidechain — Regulatory Framework
...
cmd_instruments() {
    cat << 'EOF'
# Sidechain — Instruments & Tools Overview

## Primary Instruments
- Core tools used in sidechain operations
...
cmd_glossary() {
    cat << 'EOF'
# Sidechain — Key Terms & Definitions

## Core Terminology
- **Sidechain**: The primary subject of this reference
...
cmd_checklist() {
    cat << 'EOF'
# Sidechain — Inspection Checklist
...
case "$CMD" in
    ...
    version|--version|-v) echo "sidechain v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: sidechain help"; exit 1 ;;
esac
