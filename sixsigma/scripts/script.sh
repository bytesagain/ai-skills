#!/usr/bin/env bash
# sixsigma -- Six Sigma DMAIC project tracker
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SIXSIGMA_DIR:-$HOME/.sixsigma}"
...
show_help() {
    cat << EOF
sixsigma v$VERSION -- Six Sigma DMAIC project tracker

Usage: sixsigma <command> [args]
...
cmd_add() {
    local value="${1:?Usage: sixsigma add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: sixsigma search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: sixsigma remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: sixsigma help"; exit 1 ;;
esac
