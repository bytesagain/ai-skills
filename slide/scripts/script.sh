#!/usr/bin/env bash
# slide -- Create and manage presentation slides using JSONL storage. Use when building slide decks, applying themes, or exporting to HTML presentations.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SLIDE_DIR:-$HOME/.slide}"
...
show_help() {
    cat << EOF
slide v$VERSION -- Create and manage presentation slides using JSONL storage. Use when building slide decks, applying themes, or exporting to HTML presentations.

Usage: slide <command> [args]
...
cmd_add() {
    local value="${1:?Usage: slide add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: slide search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: slide remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "slide v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: slide help"; exit 1 ;;
esac
