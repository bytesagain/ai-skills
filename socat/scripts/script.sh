#!/usr/bin/env bash
# socat — Socat reference tool. Use when working with socat in network contexts.
...
show_help() {
    cat << 'HELPEOF'
socat v$VERSION — Socat Reference Tool
...
## What is Socat?
Socat (socat) is a specialized tool/concept in the network domain.
It provides essential capabilities for professionals working with socat.

## Key Concepts
- Core socat principles and fundamentals
- How socat fits into the broader network ecosystem  
- Essential terminology every practitioner should know

## Why Socat Matters
Understanding socat is critical for:
...
## Getting Started
1. Understand the basic socat concepts
...
## Installation
1. Download or clone the socat package
...
cmd_patterns() {
    cat << 'EOF'
# Socat — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for socat
...
cmd_performance() {
    cat << 'EOF'
# Socat — Performance Optimization
...
case "$CMD" in
    ...
    version|--version|-v) echo "socat v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: socat help"; exit 1 ;;
esac
