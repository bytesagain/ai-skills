#!/usr/bin/env bash
# solid — Solid reference tool. Use when working with solid in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
solid v$VERSION — Solid Reference Tool
...
## What is Solid?
Solid (solid) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with solid.

## Key Concepts
- Core solid principles and fundamentals
- How solid fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Solid Matters
Understanding solid is critical for:
...
## Getting Started
1. Understand the basic solid concepts
...
## Installation
1. Download or clone the solid package
...
cmd_patterns() {
    cat << 'EOF'
# Solid — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for solid
...
case "$CMD" in
    ...
    version|--version|-v) echo "solid v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: solid help"; exit 1 ;;
esac
