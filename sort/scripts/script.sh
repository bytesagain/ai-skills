#!/usr/bin/env bash
# sort -- Sort files, lines, and columns with custom ordering and dedup. Use when scanning duplicates, monitoring changes, reporting results, alerting anomalies.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SORT_DIR:-$HOME/.sort}"
...
show_help() {
    cat << EOF
sort v$VERSION -- Sort files, lines, and columns with custom ordering and dedup. Use when scanning duplicates, monitoring changes, reporting results, alerting anomalies.

Usage: sort <command> [args]
...
cmd_add() {
    local value="${1:?Usage: sort add <entry>}"
    shift || true
...
cmd_remove() {
    local num="${1:?Usage: sort remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "sort v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: sort help"; exit 1 ;;
esac
