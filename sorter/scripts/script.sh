#!/usr/bin/env bash
# sorter -- Run sorter operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SORTER_DIR:-$HOME/.sorter}"
...
show_help() {
    cat << EOF
sorter v$VERSION -- Run sorter operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: sorter <command> [args]
...
cmd_add() {
    local value="${1:?Usage: sorter add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: sorter search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: sorter remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: sorter help"; exit 1 ;;
esac
