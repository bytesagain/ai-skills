#!/usr/bin/env bash
# sox -- Run sox operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SOX_DIR:-$HOME/.sox}"
...
show_help() {
    cat << EOF
sox v$VERSION -- Run sox operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: sox <command> [args]
...
cmd_add() {
    local value="${1:?Usage: sox add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: sox search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: sox remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: sox help"; exit 1 ;;
esac
