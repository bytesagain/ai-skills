#!/usr/bin/env bash
# spark — Spark reference tool. Use when working with spark in data contexts.
...
show_help() {
    cat << 'HELPEOF'
spark v$VERSION — Spark Reference Tool
...
## What is Spark?
Spark (spark) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with spark.

## Key Concepts
- Core spark principles and fundamentals
- How spark fits into the broader data ecosystem  
- Essential terminology every practitioner should know

## Why Spark Matters
Understanding spark is critical for:
...
## Getting Started
1. Understand the basic spark concepts
...
## Installation
1. Download or clone the spark package
...
cmd_patterns() {
    cat << 'EOF'
# Spark — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for spark
...
case "$CMD" in
    ...
    version|--version|-v) echo "spark v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: spark help"; exit 1 ;;
esac
