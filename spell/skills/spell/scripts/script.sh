#!/usr/bin/env bash
# spell -- Log anything fast and find it later with search and export. Use when running lookups, checking entries, converting formats, generating summaries.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SPELL_DIR:-$HOME/.spell}"
...
show_help() {
    cat << EOF
spell v$VERSION -- Log anything fast and find it later with search and export. Use when running lookups, checking entries, converting formats, generating summaries.

Usage: spell <command> [args]
...
cmd_add() {
    local value="${1:?Usage: spell add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: spell search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: spell remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "spell v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: spell help"; exit 1 ;;
esac
