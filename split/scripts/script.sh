#!/usr/bin/env bash
# split -- Process and transform split data with fast CLI operations. Use when you need to manipulate, convert, or analyze content.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SPLIT_DIR:-$HOME/.split}"
...
show_help() {
    cat << EOF
split v$VERSION -- Process and transform split data with fast CLI operations. Use when you need to manipulate, convert, or analyze content.

Usage: split <command> [args]
...
cmd_add() {
    local value="${1:?Usage: split add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: split search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: split remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: split help"; exit 1 ;;
esac
