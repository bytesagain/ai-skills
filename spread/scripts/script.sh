#!/usr/bin/env bash
# spread -- Calculate spread financial metrics and business data. Use when tracking expenses, analyzing investments, or generating financial reports.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SPREAD_DIR:-$HOME/.spread}"
...
show_help() {
    cat << EOF
spread v$VERSION -- Calculate spread financial metrics and business data. Use when tracking expenses, analyzing investments, or generating financial reports.

Usage: spread <command> [args]
...
cmd_add() {
    local value="${1:?Usage: spread add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: spread search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: spread remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "spread v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: spread help"; exit 1 ;;
esac
