#!/usr/bin/env bash
# ssh-key -- Secure ssh key operations. Use when you need to audit ssh key configurations, manage credentials, or enforce access policies.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SSH_KEY_DIR:-$HOME/.ssh-key}"
...
show_help() {
    cat << EOF
ssh-key v$VERSION -- Secure ssh key operations. Use when you need to audit ssh key configurations, manage credentials, or enforce access policies.

Usage: ssh-key <command> [args]
...
cmd_add() {
    local value="${1:?Usage: ssh-key add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: ssh-key search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: ssh-key remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "ssh-key v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: ssh-key help"; exit 1 ;;
esac
