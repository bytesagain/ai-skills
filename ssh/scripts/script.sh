#!/usr/bin/env bash
# ssh -- Manage SSH connections, keys, and tunnels using CLI tools. Use when you need to add, remove, test, tunnel, or configure SSH hosts and key management.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SSH_DIR:-$HOME/.ssh}"
...
show_help() {
    cat << EOF
ssh v$VERSION -- Manage SSH connections, keys, and tunnels using CLI tools. Use when you need to add, remove, test, tunnel, or configure SSH hosts and key management.

Usage: ssh <command> [args]
...
cmd_add() {
    local value="${1:?Usage: ssh add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: ssh search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: ssh remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: ssh help"; exit 1 ;;
esac
