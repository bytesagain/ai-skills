#!/usr/bin/env bash
# standup — Standup reference tool. Use when working with standup in life contexts.
...
show_help() {
    cat << 'HELPEOF'
standup v$VERSION — Standup Reference Tool

Usage: standup <command>
...
## What is Standup?
Standup (standup) is a specialized tool/concept in the life domain.
It provides essential capabilities for professionals working with standup.

## Key Concepts
- Core standup principles and fundamentals
- How standup fits into the broader life ecosystem  
- Essential terminology every practitioner should know

## Why Standup Matters
Understanding standup is critical for:
...
## Getting Started
1. Understand the basic standup concepts
...
cmd_guide() {
    cat << 'EOF'
# Standup — Step-by-Step Guide

## Overview
This guide walks you through the essential standup workflows.
...
cmd_mistakes() {
    cat << 'EOF'
# Standup — Common Mistakes to Avoid
...
cmd_examples() {
    cat << 'EOF'
# Standup — Real-World Examples

## Example 1: Basic Setup
A typical standup setup for a small team:
- Standard configuration with defaults
- Basic monitoring enabled
- Manual backup schedule

## Example 2: Production Deployment
An enterprise standup deployment:
...
cmd_faq() {
    cat << 'EOF'
# Standup — Frequently Asked Questions

## General
**Q: What is Standup?**
A: Standup is a reference tool for standup in the life domain.

**Q: Who should use this?**
A: Anyone working with standup who needs quick reference material.
...
case "$CMD" in
    ...
    version|--version|-v) echo "standup v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: standup help"; exit 1 ;;
esac
