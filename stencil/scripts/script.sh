#!/usr/bin/env bash
# stencil — Stencil reference tool. Use when working with stencil in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
stencil v$VERSION — Stencil Reference Tool

Usage: stencil <command>
...
## What is Stencil?
Stencil (stencil) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with stencil.

## Key Concepts
- Core stencil principles and fundamentals
- How stencil fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Stencil Matters
Understanding stencil is critical for:
...
## Getting Started
1. Understand the basic stencil concepts
...
## Installation
1. Download or clone the stencil package
...
cmd_patterns() {
    cat << 'EOF'
# Stencil — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for stencil
...
cmd_performance() {
    cat << 'EOF'
# Stencil — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Stencil — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Stencil — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "stencil v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: stencil help"; exit 1 ;;
esac
