#!/usr/bin/env bash
# story -- Draft, edit, and schedule creative stories with content optimization. Use when drafting chapters, editing narratives, optimizing readability, scheduling posts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${STORY_DIR:-$HOME/.story}"
...
show_help() {
    cat << EOF
story v$VERSION -- Draft, edit, and schedule creative stories with content optimization. Use when drafting chapters, editing narratives, optimizing readability, scheduling posts.

Usage: story <command> [args]
...
cmd_add() {
    local value="${1:?Usage: story add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: story search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: story remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "story v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: story help"; exit 1 ;;
esac
