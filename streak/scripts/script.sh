#!/usr/bin/env bash
# streak -- Track streak personal activities and daily routines. Use when managing health goals, organizing tasks, or building productive habits.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${STREAK_DIR:-$HOME/.streak}"
...
show_help() {
    cat << EOF
streak v$VERSION -- Track streak personal activities and daily routines. Use when managing health goals, organizing tasks, or building productive habits.

Usage: streak <command> [args]
...
cmd_add() {
    local value="${1:?Usage: streak add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: streak search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: streak remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "streak v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: streak help"; exit 1 ;;
esac
