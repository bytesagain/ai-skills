...
VERSION="1.0.0"
DATA_DIR="${STRETCH_DIR:-$HOME/.stretch}"
...
show_help() {
    cat << EOF
stretch v$VERSION -- stretch tool

Usage: stretch <command> [args]
...
cmd_add() {
    local value="${1:?Usage: stretch add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: stretch search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: stretch remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: stretch help"; exit 1 ;;
esac
