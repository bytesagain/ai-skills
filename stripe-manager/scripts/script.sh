#!/usr/bin/env bash
# stripe-manager - Financial tracking and analysis tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${STRIPE_MANAGER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/stripe-manager}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
stripe-manager v$VERSION

Financial tracking and analysis tool

Usage: stripe-manager <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "stripe-manager v$VERSION" ;;
