#!/usr/bin/env bash
# subpoena -- Run subpoena operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SUBPOENA_DIR:-$HOME/.subpoena}"
...
show_help() {
    cat << EOF
subpoena v$VERSION -- Run subpoena operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: subpoena <command> [args]
...
cmd_add() {
    local value="${1:?Usage: subpoena add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: subpoena search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: subpoena remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: subpoena help"; exit 1 ;;
esac
