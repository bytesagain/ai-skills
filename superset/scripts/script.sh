#!/usr/bin/env bash
# superset — Superset reference tool. Use when working with superset in data contexts.
...
show_help() {
    cat << 'HELPEOF'
superset v$VERSION — Superset Reference Tool

Usage: superset <command>
...
## What is Superset?
Superset (superset) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with superset.

## Key Concepts
- Core superset principles and fundamentals
- How superset fits into the broader data ecosystem  
- Essential terminology every practitioner should know

## Why Superset Matters
Understanding superset is critical for:
...
## Getting Started
1. Understand the basic superset concepts
...
## Installation
1. Download or clone the superset package
...
cmd_patterns() {
    cat << 'EOF'
# Superset — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for superset
...
cmd_security() {
    cat << 'EOF'
# Superset — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Superset — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "superset v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: superset help"; exit 1 ;;
esac
