#!/usr/bin/env bash
# survey -- Build surveys, collect responses, and analyze results. Use when creating forms, checking responses, converting data, analyzing trends, generating reports.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SURVEY_DIR:-$HOME/.survey}"
...
show_help() {
    cat << EOF
survey v$VERSION -- Build surveys, collect responses, and analyze results. Use when creating forms, checking responses, converting data, analyzing trends, generating reports.

Usage: survey <command> [args]
...
cmd_add() {
    local value="${1:?Usage: survey add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: survey search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: survey remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "survey v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: survey help"; exit 1 ;;
esac
