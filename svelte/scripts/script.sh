#!/usr/bin/env bash
# svelte — Svelte reference tool. Use when working with svelte in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
svelte v$VERSION — Svelte Reference Tool

Usage: svelte <command>
...
## What is Svelte?
Svelte (svelte) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with svelte.

## Key Concepts
- Core svelte principles and fundamentals
- How svelte fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Svelte Matters
Understanding svelte is critical for:
...
## Getting Started
1. Understand the basic svelte concepts
...
## Installation
1. Download or clone the svelte package
...
cmd_patterns() {
    cat << 'EOF'
# Svelte — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for svelte
...
cmd_security() {
    cat << 'EOF'
# Svelte — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Svelte — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "svelte v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: svelte help"; exit 1 ;;
esac
