#!/usr/bin/env bash
# tachometer -- Diagnose tachometer operations. Use when you need to inspect tachometer components, plan maintenance schedules, or troubleshoot vehicle systems.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${TACHOMETER_DIR:-$HOME/.tachometer}"
...
show_help() {
    cat << EOF
tachometer v$VERSION -- Diagnose tachometer operations. Use when you need to inspect tachometer components, plan maintenance schedules, or troubleshoot vehicle systems.

Usage: tachometer <command> [args]
...
cmd_add() {
    local value="${1:?Usage: tachometer add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: tachometer search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: tachometer remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="tachometer-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "tachometer v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: tachometer help"; exit 1 ;;
esac
