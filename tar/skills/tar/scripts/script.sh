#!/usr/bin/env bash
# tar -- Create, extract, list, and compress tar archives with format support. Use when scanning contents, monitoring sizes, reporting results, alerting corruption.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${TAR_DIR:-$HOME/.tar}"
...
show_help() {
    cat << EOF
tar v$VERSION -- Create, extract, list, and compress tar archives with format support. Use when scanning contents, monitoring sizes, reporting results, alerting corruption.
...
cmd_add() {
    local value="${1:?Usage: tar add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: tar search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: tar remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "tar v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: tar help"; exit 1 ;;
esac
