#!/usr/bin/env bash
# tariff -- Run tariff operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${TARIFF_DIR:-$HOME/.tariff}"
...
show_help() {
    cat << EOF
tariff v$VERSION -- Run tariff operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: tariff <command> [args]
...
cmd_add() {
    local value="${1:?Usage: tariff add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: tariff search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: tariff remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: tariff help"; exit 1 ;;
esac
