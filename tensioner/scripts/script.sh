#!/usr/bin/env bash
# tensioner -- Diagnose tensioner operations. Use when you need to inspect tensioner components, plan maintenance schedules, or troubleshoot vehicle systems.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${TENSIONER_DIR:-$HOME/.tensioner}"
...
show_help() {
    cat << EOF
tensioner v$VERSION -- Diagnose tensioner operations. Use when you need to inspect tensioner components, plan maintenance schedules, or troubleshoot vehicle systems.

Usage: tensioner <command> [args]
...
cmd_add() {
    local value="${1:?Usage: tensioner add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: tensioner search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: tensioner remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="tensioner-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "tensioner v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: tensioner help"; exit 1 ;;
esac
