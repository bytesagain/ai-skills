#!/usr/bin/env bash
# terraform-helper — Terraform Helper reference tool. Use when working with terraform helper in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
terraform-helper v$VERSION — Terraform Helper Reference Tool

Usage: terraform-helper <command>
...
cmd_intro() {
    cat << 'EOF'
# Terraform Helper — Overview

## What is Terraform Helper?
Terraform Helper (terraform-helper) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with terraform helper.

## Key Concepts
- Core terraform helper principles and fundamentals
- How terraform helper fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Terraform Helper Matters
Understanding terraform helper is critical for:
...
## Getting Started
1. Understand the basic terraform helper concepts
...
cmd_quickstart() {
    cat << 'EOF'
# Terraform Helper — Quick Start Guide
...
## Installation
1. Download or clone the terraform helper package
...
cmd_patterns() {
    cat << 'EOF'
# Terraform Helper — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for terraform helper
...
cmd_debugging() {
    cat << 'EOF'
# Terraform Helper — Debugging Guide
...
cmd_performance() {
    cat << 'EOF'
# Terraform Helper — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Terraform Helper — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Terraform Helper — Migration & Upgrade Guide
...
cmd_cheatsheet() {
    cat << 'EOF'
# Terraform Helper — Quick Reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "terraform-helper v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: terraform-helper help"; exit 1 ;;
esac
