#!/usr/bin/env bash
# thesaurus -- Look up synonyms, antonyms, and related words with history and export. Use when finding alternatives, checking usage, running drills, analyzing frequency.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${THESAURUS_DIR:-$HOME/.thesaurus}"
...
show_help() {
    cat << EOF
thesaurus v$VERSION -- Look up synonyms, antonyms, and related words with history and export. Use when finding alternatives, checking usage, running drills, analyzing frequency.

Usage: thesaurus <command> [args]
...
cmd_add() {
    local value="${1:?Usage: thesaurus add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: thesaurus search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: thesaurus remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="thesaurus-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "thesaurus v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: thesaurus help"; exit 1 ;;
esac
