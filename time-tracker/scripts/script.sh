#!/usr/bin/env bash
# time-tracker - Productivity and task management tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${TIME_TRACKER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/time-tracker}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
time-tracker v$VERSION

Productivity and task management tool

Usage: time-tracker <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "time-tracker v$VERSION" ;;
