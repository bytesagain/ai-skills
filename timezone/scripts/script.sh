#!/usr/bin/env bash
# timezone -- Convert times across world timezones and compare availability. Use when converting meetings, checking offsets, comparing zones, generating tables.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${TIMEZONE_DIR:-$HOME/.timezone}"
...
show_help() {
    cat << EOF
timezone v$VERSION -- Convert times across world timezones and compare availability. Use when converting meetings, checking offsets, comparing zones, generating tables.

Usage: timezone <command> [args]
...
cmd_add() {
    local value="${1:?Usage: timezone add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: timezone search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: timezone remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "timezone v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: timezone help"; exit 1 ;;
esac
