#!/usr/bin/env bash
# tire -- Track tire maintenance, rotation, and tread wear for vehicles. Use when logging changes, checking wear, running reminders, generating reports.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${TIRE_DIR:-$HOME/.tire}"
...
show_help() {
    cat << EOF
tire v$VERSION -- Track tire maintenance, rotation, and tread wear for vehicles. Use when logging changes, checking wear, running reminders, generating reports.

Usage: tire <command> [args]
...
cmd_add() {
    local value="${1:?Usage: tire add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: tire search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: tire remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "tire v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: tire help"; exit 1 ;;
esac
