#!/usr/bin/env bash
# tls — Tls reference tool. Use when working with tls in security contexts.
...
show_help() {
    cat << 'HELPEOF'
tls v$VERSION — Tls Reference Tool
...
## What is Tls?
Tls (tls) is a specialized tool/concept in the security domain.
It provides essential capabilities for professionals working with tls.

## Key Concepts
- Core tls principles and fundamentals
- How tls fits into the broader security ecosystem  
- Essential terminology every practitioner should know

## Why Tls Matters
Understanding tls is critical for:
...
## Getting Started
1. Understand the basic tls concepts
...
## Installation
1. Download or clone the tls package
...
cmd_patterns() {
    cat << 'EOF'
# Tls — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for tls
...
case "$CMD" in
    ...
    version|--version|-v) echo "tls v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: tls help"; exit 1 ;;
esac
