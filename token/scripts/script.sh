#!/usr/bin/env bash
# token -- Generate, validate, and manage secure tokens using HMAC and hashing. Use when creating API keys, session tokens, or auditing token lifecycle.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${TOKEN_DIR:-$HOME/.token}"
...
show_help() {
    cat << EOF
token v$VERSION -- Generate, validate, and manage secure tokens using HMAC and hashing. Use when creating API keys, session tokens, or auditing token lifecycle.

Usage: token <command> [args]
...
cmd_add() {
    local value="${1:?Usage: token add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: token search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: token remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "token v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: token help"; exit 1 ;;
esac
