#!/usr/bin/env bash
# trace -- Track and debug application logs using CLI tracing tools. Use when you need to start trace sessions, search logs, filter by level, tail output,
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${TRACE_DIR:-$HOME/.trace}"
...
show_help() {
    cat << EOF
trace v$VERSION -- Track and debug application logs using CLI tracing tools. Use when you need to start trace sessions, search logs, filter by level, tail output,

Usage: trace <command> [args]
...
cmd_add() {
    local value="${1:?Usage: trace add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: trace search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: trace remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
