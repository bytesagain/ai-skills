#!/usr/bin/env bash
# trivia -- Host trivia rounds with question banks, scoring, and boards. Use when running quizzes, checking answers, analyzing scores, generating rounds.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${TRIVIA_DIR:-$HOME/.trivia}"
...
show_help() {
    cat << EOF
trivia v$VERSION -- Host trivia rounds with question banks, scoring, and boards. Use when running quizzes, checking answers, analyzing scores, generating rounds.

Usage: trivia <command> [args]
...
cmd_add() {
    local value="${1:?Usage: trivia add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: trivia search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: trivia remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "trivia v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: trivia help"; exit 1 ;;
esac
