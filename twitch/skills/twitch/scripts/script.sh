#!/usr/bin/env bash
# twitch -- Manage Twitch channel data, scores, and rankings from CLI. Use when rolling highlights, scoring streams, ranking metrics, tracking stats.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${TWITCH_DIR:-$HOME/.twitch}"
...
show_help() {
    cat << EOF
twitch v$VERSION -- Manage Twitch channel data, scores, and rankings from CLI. Use when rolling highlights, scoring streams, ranking metrics, tracking stats.

Usage: twitch <command> [args]
...
cmd_add() {
    local value="${1:?Usage: twitch add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: twitch search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: twitch remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "twitch v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: twitch help"; exit 1 ;;
esac
