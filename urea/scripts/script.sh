#!/usr/bin/env bash
# urea -- Run urea operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${UREA_DIR:-$HOME/.urea}"
...
show_help() {
    cat << EOF
urea v$VERSION -- Run urea operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: urea <command> [args]
...
cmd_add() {
    local value="${1:?Usage: urea add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: urea search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: urea remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: urea help"; exit 1 ;;
esac
