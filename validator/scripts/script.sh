#!/usr/bin/env bash
# validator -- Validate emails, URLs, phones, dates, and custom patterns. Use when sanitizing input, verifying form fields, checking formats, or enforcing rules.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${VALIDATOR_DIR:-$HOME/.validator}"
...
show_help() {
    cat << EOF
validator v$VERSION -- Validate emails, URLs, phones, dates, and custom patterns. Use when sanitizing input, verifying form fields, checking formats, or enforcing rules.

Usage: validator <command> [args]
...
cmd_add() {
    local value="${1:?Usage: validator add <entry>}"
    shift || true
...
cmd_check() {
    local target="${1:?Usage: validator check <file-or-value>}"
    if [ -f "$target" ]; then
...
cmd_remove() {
    local num="${1:?Usage: validator remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="validator-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "validator v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: validator help"; exit 1 ;;
esac
