#!/usr/bin/env bash
# valuation -- Build DCF models and run comparable analysis for company valuation. Use when modeling cash flows, benchmarking peers, projecting growth, or pitching.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${VALUATION_DIR:-$HOME/.valuation}"
...
show_help() {
    cat << EOF
valuation v$VERSION -- Build DCF models and run comparable analysis for company valuation. Use when modeling cash flows, benchmarking peers, projecting growth, or pitching.

Usage: valuation <command> [args]
...
cmd_add() {
    local value="${1:?Usage: valuation add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: valuation search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: valuation remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="valuation-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "valuation v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: valuation help"; exit 1 ;;
esac
