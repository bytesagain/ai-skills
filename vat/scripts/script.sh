#!/usr/bin/env bash
# vat -- Calculate vat financial metrics and business data. Use when tracking expenses, analyzing investments, or generating financial reports.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${VAT_DIR:-$HOME/.vat}"
...
show_help() {
    cat << EOF
vat v$VERSION -- Calculate vat financial metrics and business data. Use when tracking expenses, analyzing investments, or generating financial reports.

Usage: vat <command> [args]
...
cmd_add() {
    local value="${1:?Usage: vat add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: vat search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: vat remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "vat v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: vat help"; exit 1 ;;
esac
