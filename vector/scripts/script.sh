#!/usr/bin/env bash
# vector -- Perform vector math and similarity operations using CLI tools. Use when you need dot products, cosine similarity, distance metrics, normalization,
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${VECTOR_DIR:-$HOME/.vector}"
...
show_help() {
    cat << EOF
vector v$VERSION -- Perform vector math and similarity operations using CLI tools. Use when you need dot products, cosine similarity, distance metrics, normalization,

Usage: vector <command> [args]
...
cmd_add() {
    local value="${1:?Usage: vector add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: vector search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: vector remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "vector v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: vector help"; exit 1 ;;
esac
