#!/usr/bin/env bash
# vega — Vega reference tool. Use when working with vega in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
vega v$VERSION — Vega Reference Tool
...
## What is Vega?
Vega (vega) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with vega.

## Key Concepts
- Core vega principles and fundamentals
- How vega fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Vega Matters
Understanding vega is critical for:
...
## Getting Started
1. Understand the basic vega concepts
...
cmd_formulas() {
    cat << 'EOF'
# Vega — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Vega — Instruments & Tools Overview

## Primary Instruments
- Core tools used in vega operations
...
cmd_glossary() {
    cat << 'EOF'
# Vega — Key Terms & Definitions

## Core Terminology
- **Vega**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "vega v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: vega help"; exit 1 ;;
esac
