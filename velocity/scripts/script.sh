#!/usr/bin/env bash
# velocity — Velocity reference tool. Use when working with velocity in life contexts.
...
show_help() {
    cat << 'HELPEOF'
velocity v$VERSION — Velocity Reference Tool
...
## What is Velocity?
Velocity (velocity) is a specialized tool/concept in the life domain.
It provides essential capabilities for professionals working with velocity.

## Key Concepts
- Core velocity principles and fundamentals
- How velocity fits into the broader life ecosystem  
- Essential terminology every practitioner should know

## Why Velocity Matters
Understanding velocity is critical for:
...
## Getting Started
1. Understand the basic velocity concepts
...
cmd_guide() {
    cat << 'EOF'
# Velocity — Step-by-Step Guide

## Overview
This guide walks you through the essential velocity workflows.
...
cmd_resources() {
    cat << 'EOF'
# Velocity — Recommended Resources
...
cmd_mistakes() {
    cat << 'EOF'
# Velocity — Common Mistakes to Avoid
...
cmd_examples() {
    cat << 'EOF'
# Velocity — Real-World Examples

## Example 1: Basic Setup
A typical velocity setup for a small team:
- Standard configuration with defaults
- Basic monitoring enabled
- Manual backup schedule

## Example 2: Production Deployment
An enterprise velocity deployment:
...
## General
**Q: What is Velocity?**
A: Velocity is a reference tool for velocity in the life domain.

**Q: Who should use this?**
A: Anyone working with velocity who needs quick reference material.
...
case "$CMD" in
    ...
    version|--version|-v) echo "velocity v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: velocity help"; exit 1 ;;
esac
