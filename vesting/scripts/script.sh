#!/usr/bin/env bash
# vesting -- Analyze vesting operations. Use when you need to understand vesting mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${VESTING_DIR:-$HOME/.vesting}"
...
show_help() {
    cat << EOF
vesting v$VERSION -- Analyze vesting operations. Use when you need to understand vesting mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: vesting <command> [args]
...
cmd_add() {
    local value="${1:?Usage: vesting add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: vesting search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: vesting remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "vesting v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: vesting help"; exit 1 ;;
esac
