#!/usr/bin/env bash
# victoriametrics — Victoriametrics reference tool. Use when working with victoriametrics in database contexts.
...
show_help() {
    cat << 'HELPEOF'
victoriametrics v$VERSION — Victoriametrics Reference Tool

Usage: victoriametrics <command>
...
Victoriametrics (victoriametrics) is a specialized tool/concept in the database domain.
It provides essential capabilities for professionals working with victoriametrics.

## Key Concepts
- Core victoriametrics principles and fundamentals
- How victoriametrics fits into the broader database ecosystem  
- Essential terminology every practitioner should know

## Why Victoriametrics Matters
Understanding victoriametrics is critical for:
- Improving efficiency in database workflows
...
## Getting Started
1. Understand the basic victoriametrics concepts
...
cmd_quickstart() {
    cat << 'EOF'
# Victoriametrics — Quick Start Guide
...
## Installation
1. Download or clone the victoriametrics package
...
cmd_patterns() {
    cat << 'EOF'
# Victoriametrics — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for victoriametrics
...
cmd_debugging() {
    cat << 'EOF'
# Victoriametrics — Debugging Guide
...
cmd_performance() {
    cat << 'EOF'
# Victoriametrics — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Victoriametrics — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Victoriametrics — Migration & Upgrade Guide
...
cmd_cheatsheet() {
    cat << 'EOF'
# Victoriametrics — Quick Reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "victoriametrics v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: victoriametrics help"; exit 1 ;;
esac
