#!/usr/bin/env bash
# vitamin -- Track vitamin and supplement intake with goals and reminders. Use when logging supplements, setting nutrition goals, reviewing history, or scheduling doses.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${VITAMIN_DIR:-$HOME/.vitamin}"
...
show_help() {
    cat << EOF
vitamin v$VERSION -- Track vitamin and supplement intake with goals and reminders. Use when logging supplements, setting nutrition goals, reviewing history, or scheduling doses.

Usage: vitamin <command> [args]
...
cmd_add() {
    local value="${1:?Usage: vitamin add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: vitamin search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: vitamin remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "vitamin v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: vitamin help"; exit 1 ;;
esac
