#!/usr/bin/env bash
# vite — Vite reference tool. Use when working with vite in frontend contexts.
...
show_help() {
    cat << 'HELPEOF'
vite v$VERSION — Vite Reference Tool
...
## What is Vite?
Vite (vite) is a specialized tool/concept in the frontend domain.
It provides essential capabilities for professionals working with vite.

## Key Concepts
- Core vite principles and fundamentals
- How vite fits into the broader frontend ecosystem  
- Essential terminology every practitioner should know

## Why Vite Matters
Understanding vite is critical for:
...
## Getting Started
1. Understand the basic vite concepts
...
## Installation
1. Download or clone the vite package
...
cmd_patterns() {
    cat << 'EOF'
# Vite — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for vite
...
case "$CMD" in
    ...
    version|--version|-v) echo "vite v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: vite help"; exit 1 ;;
esac
