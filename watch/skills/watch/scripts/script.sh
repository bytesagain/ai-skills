#!/usr/bin/env bash
# watch -- Monitor file and directory changes using CLI watchers. Use when you need to watch for file modifications, track change history, filter events,
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${WATCH_DIR:-$HOME/.watch}"
...
show_help() {
    cat << EOF
watch v$VERSION -- Monitor file and directory changes using CLI watchers. Use when you need to watch for file modifications, track change history, filter events,
...
Commands:
  ...
  monitor         Monitor target for changes
...
cmd_add() {
    local value="${1:?Usage: watch add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: watch search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_monitor() {
    local target="${1:-.}"
    local secs="${2:-5}"
    echo "Monitoring $target every ${secs}s (Ctrl+C to stop)..."
    local prev=""
    for i in $(seq 1 60); do
        local cur=$(ls -la "$target" 2>/dev/null | md5sum | cut -d" " -f1)
        if [ -n "$prev" ] && [ "$cur" != "$prev" ]; then
            echo "  [$(date +%H:%M:%S)] CHANGED"
            _save_entry "monitor" "change:$target"
        fi
        prev="$cur"; sleep "$secs"
    done
}
...
cmd_remove() {
    local num="${1:?Usage: watch remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "watch v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: watch help"; exit 1 ;;
esac
