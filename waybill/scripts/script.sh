#!/usr/bin/env bash
# waybill -- Run waybill operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${WAYBILL_DIR:-$HOME/.waybill}"
...
show_help() {
    cat << EOF
waybill v$VERSION -- Run waybill operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: waybill <command> [args]
...
cmd_add() {
    local value="${1:?Usage: waybill add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: waybill search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: waybill remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "waybill v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: waybill help"; exit 1 ;;
esac
