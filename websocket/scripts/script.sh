#!/usr/bin/env bash
# websocket — Websocket reference tool. Use when working with websocket in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
websocket v$VERSION — Websocket Reference Tool
...
## What is Websocket?
Websocket (websocket) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with websocket.

## Key Concepts
- Core websocket principles and fundamentals
- How websocket fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Websocket Matters
Understanding websocket is critical for:
...
## Getting Started
1. Understand the basic websocket concepts
...
## Installation
1. Download or clone the websocket package
...
cmd_patterns() {
    cat << 'EOF'
# Websocket — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for websocket
...
cmd_migration() {
    cat << 'EOF'
# Websocket — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "websocket v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: websocket help"; exit 1 ;;
esac
