#!/usr/bin/env bash
# whois — Whois reference tool. Use when working with whois in network contexts.
...
show_help() {
    cat << 'HELPEOF'
whois v$VERSION — Whois Reference Tool
...
## What is Whois?
Whois (whois) is a specialized tool/concept in the network domain.
It provides essential capabilities for professionals working with whois.

## Key Concepts
- Core whois principles and fundamentals
- How whois fits into the broader network ecosystem  
- Essential terminology every practitioner should know

## Why Whois Matters
Understanding whois is critical for:
...
## Getting Started
1. Understand the basic whois concepts
...
## Installation
1. Download or clone the whois package
...
cmd_patterns() {
    cat << 'EOF'
# Whois — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for whois
...
cmd_performance() {
    cat << 'EOF'
# Whois — Performance Optimization
...
case "$CMD" in
    ...
    version|--version|-v) echo "whois v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: whois help"; exit 1 ;;
esac
