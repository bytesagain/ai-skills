#!/usr/bin/env bash
# wine -- Record, search, and analyze your wine collection and tasting notes. Use when logging tastings, searching vintages, comparing ratings, or tracking inventory.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${WINE_DIR:-$HOME/.wine}"
...
show_help() {
    cat << EOF
wine v$VERSION -- Record, search, and analyze your wine collection and tasting notes. Use when logging tastings, searching vintages, comparing ratings, or tracking inventory.

Usage: wine <command> [args]
...
cmd_add() {
    local value="${1:?Usage: wine add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: wine search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: wine remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "wine v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: wine help"; exit 1 ;;
esac
