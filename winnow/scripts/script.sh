#!/usr/bin/env bash
# winnow -- Run winnow operations with simple CLI commands. Use when you need quick data processing or automation.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${WINNOW_DIR:-$HOME/.winnow}"
...
show_help() {
    cat << EOF
winnow v$VERSION -- Run winnow operations with simple CLI commands. Use when you need quick data processing or automation.

Usage: winnow <command> [args]
...
cmd_add() {
    local value="${1:?Usage: winnow add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: winnow search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: winnow remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "winnow v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: winnow help"; exit 1 ;;
esac
