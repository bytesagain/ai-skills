#!/usr/bin/env bash
# withhold -- Calculate withhold financial metrics and business data. Use when tracking expenses, analyzing investments, or generating financial reports.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${WITHHOLD_DIR:-$HOME/.withhold}"
...
show_help() {
    cat << EOF
withhold v$VERSION -- Calculate withhold financial metrics and business data. Use when tracking expenses, analyzing investments, or generating financial reports.

Usage: withhold <command> [args]
...
cmd_add() {
    local value="${1:?Usage: withhold add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: withhold search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: withhold remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "withhold v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: withhold help"; exit 1 ;;
esac
