#!/usr/bin/env bash
# xml -- Handle xml format data for parsing, conversion, and analysis. Use when working with data pipelines or format migration.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${XML_DIR:-$HOME/.xml}"
...
show_help() {
    cat << EOF
xml v$VERSION -- Handle xml format data for parsing, conversion, and analysis. Use when working with data pipelines or format migration.

Usage: xml <command> [args]
...
cmd_add() {
    local value="${1:?Usage: xml add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: xml search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: xml remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "xml v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: xml help"; exit 1 ;;
esac
