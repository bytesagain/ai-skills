#!/usr/bin/env bash
# yaml -- Handle yaml format data for parsing, conversion, and analysis. Use when working with data pipelines or format migration.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${YAML_DIR:-$HOME/.yaml}"
...
show_help() {
    cat << EOF
yaml v$VERSION -- Handle yaml format data for parsing, conversion, and analysis. Use when working with data pipelines or format migration.
...
cmd_add() {
    local value="${1:?Usage: yaml add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: yaml search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: yaml remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "yaml v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: yaml help"; exit 1 ;;
esac
