#!/usr/bin/env bash
# zip -- Compress, extract, list, and encrypt ZIP archives in batch. Use when archiving files, extracting packages, listing contents, encrypting backups, or batching.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ZIP_DIR:-$HOME/.zip}"
...
show_help() {
    cat << EOF
zip v$VERSION -- Compress, extract, list, and encrypt ZIP archives in batch. Use when archiving files, extracting packages, listing contents, encrypting backups, or batching.
...
cmd_add() {
    local value="${1:?Usage: zip add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: zip search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: zip remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "zip v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: zip help"; exit 1 ;;
esac
