#!/usr/bin/env bash
# zk-proof -- Analyze zk proof operations. Use when you need to understand zk proof mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ZK_PROOF_DIR:-$HOME/.zk-proof}"
...
show_help() {
    cat << EOF
zk-proof v$VERSION -- Analyze zk proof operations. Use when you need to understand zk proof mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: zk-proof <command> [args]
...
cmd_add() {
    local value="${1:?Usage: zk-proof add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: zk-proof search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: zk-proof remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "zk-proof v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: zk-proof help"; exit 1 ;;
esac
